...
| File | Purpose |
|------|---------|
| `.agent/memory/codebase_map.md` | Structural index of the entire repo |
| `.agent/memory/experiment_journal.md` | Log of all past experiments and insights |
| `.agent/memory/evaluation_registry.md` | Available metrics and their status |

## Step 3: Check for Existing Skills & SOPs

Before writing new code or procedures:

1. **Skills**: Browse `.agent/skills/` — each `.md` file is a self-contained skill with instructions.
2. **Workflows/SOPs**: Browse `.agent/workflows/` — step-by-step procedures for common tasks.
3. **Lessons**: Browse `.agent/lessons/` — known pitfalls and their fixes.
...
- Browse `.agent/memory/related_work/` for indexed papers and repos.
...
1. Document your progress in `.agent/exploration/<topic>.md` using the template in `.agent/exploration/README.md`.
2. At the end of your session, reflect:
   - **What worked** → propose a new skill or SOP in the exploration log.
   - **What failed** → create a lesson in `.agent/lessons/`.
