...
## Purpose
Search through `.agent/memory/related_work/` to find indexed papers, repos,
...
## Prerequisites
- The related work index has entries (`.agent/memory/related_work/*.md`).
...
Use grep-based search through `.agent/memory/related_work/`:

```bash
# Search by content
grep -rl "<query>" .agent/memory/related_work/

# Search by tags (in frontmatter)
grep -l "tags:.*<tag>" .agent/memory/related_work/*.md
```
...
- **File**: `.agent/memory/related_work/<slug>.md`
...
## References
- `.agent/memory/related_work/README.md` — index schema
