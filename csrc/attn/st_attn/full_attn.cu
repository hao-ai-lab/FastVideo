...
void fwd_attend_ker(const __grid_constant__ fwd_globals<D> g) {
    ...
    if (threadIdx.x == 0) {
        ...
        coord<k_tile_half> k_tile_idx_0 = {blockIdx.z, kv_head_idx, 0, 0};
        coord<k_tile_half> k_tile_idx_1 = {blockIdx.z, kv_head_idx, 1, 0};
        ...
        coord<v_tile_half> v_tile_idx_0 = {blockIdx.z, kv_head_idx, 0, 0};
        coord<v_tile_half> v_tile_idx_1 = {blockIdx.z, kv_head_idx, 1, 0};
    ...
    for (int kv_idx = 0; kv_idx < kv_blocks - 1; kv_idx++) {
        ...
        if (threadIdx.x == 0) {
            ...
            // tma::load_async(k_smem[0], g.k, k_tile_idx, k_smem_arrived);
            coord<k_tile_half> k_tile_idx_0 = {blockIdx.z, kv_head_idx, 2 * (kv_idx + 1), 0};
            coord<k_tile_half> k_tile_idx_1 = {blockIdx.z, kv_head_idx, 2 * (kv_idx + 1) + 1, 0};
            tma::load_async(k_smem_0[0], g.k, k_tile_idx_0, k_smem_arrived);
            tma::load_async(k_smem_1[0], g.k, k_tile_idx_1, k_smem_arrived);
        }
        ...
        if (threadIdx.x == 0) {
            ...
            // tma::load_async(v_smem[0], g.v, v_tile_idx, v_smem_arrived);
            coord<v_tile_half> v_tile_idx_0 = {blockIdx.z, kv_head_idx, 2 * (kv_idx + 1), 0};
            coord<v_tile_half> v_tile_idx_1 = {blockIdx.z, kv_head_idx, 2 * (kv_idx + 1) + 1, 0};
            tma::load_async(v_smem_0[0], g.v, v_tile_idx_0, v_smem_arrived);
            tma::load_async(v_smem_1[0], g.v, v_tile_idx_1, v_smem_arrived);
        }
    }
...
template<int D>
__global__ __launch_bounds__(128, 3) // encourage compiler to reduce register usage so that an SM can hold 3 CTAs
...
mha_forward(torch::Tensor q, torch::Tensor k, torch::Tensor v)
{
    CHECK_INPUT(q);
    CHECK_INPUT(k);
    CHECK_INPUT(v);
