...
def sliding_tile_attention(q_all, k_all, v_all, window_size, text_length,
    ...
    for head_index, (t_kernel, h_kernel, w_kernel) in enumerate(window_size):
        for batch in range(q_all.shape[0]):
            q_head, k_head, v_head, o_head = q_all[
                batch:batch + 1, head_index:head_index +
                1], k_all[batch:batch + 1, head_index:head_index +
                          1], v_all[batch:batch + 1, head_index:head_index +
    ...
    if has_text:
        _ = sta_fwd(q_all, k_all, v_all, hidden_states, 3, 3, 3, text_length,
                    True)
