...
template<int D> struct fwd_globals {
    ...
    uint8_t *__restrict__ block_sparse_map;
};
...
void fwd_attend_ker(const __grid_constant__ fwd_globals<D> g) { // use block size of 64
    ...
    int kv_blocks   = g.N / 64;
    int kv_head_idx = blockIdx.y / g.hr;
    int seq_idx     = blockIdx.x;

    const int q_blk = (seq_idx * 4) + kittens::warpid();
    uint8_t *warp_sparse_mask = g.block_sparse_map + blockIdx.z * gridDim.y * (gridDim.x * 4) * (g.N / 16) + 
                             blockIdx.y * (gridDim.x * 4) * (g.N / 16) + 
                             q_blk * (g.N / 16);
    ...
    if (threadIdx.x == 0) {
        ...
        coord<k_tile> k_tile_idx = {blockIdx.z, kv_head_idx, 0, 0};
        tma::load_async(k_smem[0], g.k, k_tile_idx, k_smem_arrived);

        tma::expect_bytes(v_smem_arrived, sizeof(v_tile));
        coord<v_tile> v_tile_idx = {blockIdx.z, kv_head_idx, 0, 0};
    ...
    for (int kv_idx = 0; kv_idx < kv_blocks - 1; kv_idx++) {
        // load sparse mask
        uint8_t sparse_mask[4];
        reinterpret_cast<uint32_t*>(sparse_mask)[0] = *(reinterpret_cast<uint32_t*>(warp_sparse_mask + kv_idx * 4)); 
        ...
        // if (q_blk == 0 && threadIdx.x == 0) {
        //     printf("%d, %d, %d, %d, ", sparse_mask[0], sparse_mask[1], sparse_mask[2], sparse_mask[3]);
        // }

        // block mask of granularity 16x16, which is exactly the tile size of TK
        #pragma unroll
        for (int i = 0; i < 4; i++) {
            if (sparse_mask[i] == 0) {
                auto &attn_subtile = reinterpret_cast<rt_fl<16, 16>&>(att_block.tiles[0][i]);
                // neg_infty(attn_subtile);
                add(attn_subtile, attn_subtile, -5000000.0f);
                __syncwarp();
            }
        }
        ...
        if (threadIdx.x == 0) {
            tma::expect_bytes(k_smem_arrived, sizeof(k_tile));
            coord<k_tile> k_tile_idx = {blockIdx.z, kv_head_idx, kv_idx + 1, 0};
            tma::load_async(k_smem[0], g.k, k_tile_idx, k_smem_arrived);
        }
...
struct bwd_globals {
    ...
    const int N;
    const int hr;
    uint8_t *__restrict__ block_sparse_map_trans;
};
...
void bwd_attend_ker(const __grid_constant__ bwd_globals<D> g) {
    ...
    attn_tile (&ds_smem)[1] = al.allocate<attn_tile, 1>();
    ...
    const int k_idx = blockIdx.x * 4 + kittens::warpid();
    uint8_t *warp_sparse_mask = g.block_sparse_map_trans + blockIdx.z * gridDim.y * (gridDim.x * 4) * (g.N / 16) + 
                             blockIdx.y * (gridDim.x * 4) * (g.N / 16) + 
                             k_idx * (g.N / 16);
    ...
    if (threadIdx.x == 0) {
        ...
        coord<q_tile> tile_idx_qo = {blockIdx.z, blockIdx.y, 0, 0};
        coord<l_tile> vec_idx = {blockIdx.z, blockIdx.y, 0, 0};
    ...
    for (int qo_idx = 0; qo_idx < qo_blocks - 1; qo_idx++) {
        // load sparse mask
        uint8_t sparse_mask[4];
        reinterpret_cast<uint32_t*>(sparse_mask)[0] = *(reinterpret_cast<uint32_t*>(warp_sparse_mask + qo_idx * 4));
        ...
        for (int i = 0; i < 4; i++) {
            if (sparse_mask[i] == 0) {
                auto &s_subtile = reinterpret_cast<rt_fl<16, 16>&>(s_block_t.tiles[0][i]);
                add(s_subtile, s_subtile, -5000000.0f);
                __syncwarp();
            }
        }
        ...
        warpgroup::store(ds_smem[0], ds_block_t);
        warpgroup::mma_async_wait();
        ...
        warpgroup::mm_AtB(qg_reg, ds_smem[0], k_smem[0]); //delat dQ = dSK
        warpgroup::mma_commit_group();
        tma::store_async_wait();
        warpgroup::mma_async_wait();
...
block_sparse_attention_forward(torch::Tensor q, torch::Tensor k, torch::Tensor v, torch::Tensor block_sparse_map)
{
    CHECK_INPUT(q);
    CHECK_INPUT(k);
    CHECK_INPUT(v);
    ...
    if (head_dim == 64) {
        ...
        globals g{qg_arg, kg_arg, vg_arg, lg_arg, og_arg, static_cast<int>(seq_len), static_cast<int>(hr), reinterpret_cast<uint8_t*>(block_sparse_map.data_ptr())};
...
block_sparse_attention_backward(torch::Tensor q, 
                   ...
                   torch::Tensor l_vec,
                   torch::Tensor og,
                   torch::Tensor block_sparse_map_trans)
{
    CHECK_INPUT(q);
    CHECK_INPUT(k);
    CHECK_INPUT(v);
    ...
    if (head_dim == 64)  {
        ...
        bwd_global_args bwd_global{bwd_q_arg, 
                        ...
                        static_cast<int>(hr),
                        reinterpret_cast<uint8_t*>(block_sparse_map_trans.data_ptr()),
                        };
