...
def flex_test(Q, K, V, kernel_size):
    mask = get_sliding_tile_attention_mask(kernel_size,  (6, 8, 8), (30, 48, 80), 39, 'cuda')
...
def check_correctness(b, h, n, d, causal, mean, std, num_iterations=50, error_mode='all'):
    ...
    for kernel_size in tqdm(kernel_size_ls):
        ...
        print("avg_diff", torch.sum(abs_diff).item() / (b * h * n * d * (1 if error_mode == 'output' else 3 if error_mode == 'backward' else 4)))

    total_elements = b * h * n * d * num_iterations * (1 if error_mode == 'output' else 3 if error_mode == 'backward' else 4) * len(kernel_size_ls)
