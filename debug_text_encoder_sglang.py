...
def main():
    ...
    if first_divergent is not None:
        ...
        if fv_o_in and ref_o_in and fv_o and ref_o and fv_w is not None and ref_w is not None:
            print("\n--- 7. Manual F.linear sanity check ---")
            try:
                fv_in = fv_o_in[0].float()
                ref_in = ref_o_in[0].float()
                fv_w_local = _to_local_tensor(fv_w).to(fv_in.device)
                ref_w_local = _to_local_tensor(ref_w).to(ref_in.device)
                fv_out_manual = torch.nn.functional.linear(fv_in, fv_w_local, None)
                ref_out_manual = torch.nn.functional.linear(ref_in, ref_w_local, None)
                hooked_fv = fv_o[0].float()
                hooked_ref = ref_o[0].float()
