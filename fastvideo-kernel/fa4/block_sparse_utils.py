...
@cute.jit
def dual_stream_block_count(total_block_count, stream_idx: Int32):
    """Number of ordinals assigned to one stream by even/odd list position."""
    return (total_block_count + Int32(1) - stream_idx) // Int32(2)


@cute.jit
def _block_from_reverse_concat_ordinal(
    ordinal: Int32,
    mask_block_indices: cute.Tensor,
    mask_begin: Int32,
    mask_end: Int32,
    full_block_indices: Optional[cute.Tensor],
    full_begin: Int32,
    full_end: Int32,
):
    """Lookup an ordinal in ``reverse(masked) + reverse(full)``.

    This is the single canonical order used by the dual-stream loader,
    softmax, and MMA block counts. Stream 0 owns even ordinals and stream 1
    owns odd ordinals.
    """
    mask_count = mask_end - mask_begin
    n_block = Int32(0)
    if ordinal < mask_count:
        n_block = mask_block_indices[mask_end - Int32(1) - ordinal]
    else:
        full_ordinal = ordinal - mask_count
        if const_expr(full_block_indices is not None):
            n_block = full_block_indices[full_end - Int32(1) - full_ordinal]
    return n_block


@cute.jit
def get_dual_stream_block_counts(
    blocksparse_tensors: BlockSparseTensors,
    batch_idx,
    head_idx,
    m_block,
    ...
    """Return stream-0/stream-1 counts for canonical ordinal parity."""
    total = get_total_block_count(
        blocksparse_tensors,
        batch_idx,
        head_idx,
        m_block,
    ...
    return dual_stream_block_count(total, Int32(0)), dual_stream_block_count(total, Int32(1))


@cute.jit
def produce_block_sparse_loads_dual_stream_sm100(
    blocksparse_tensors: BlockSparseTensors,
    batch_idx,
    head_idx,
    m_block,
    ...
    """Load two sparse streams through one interleaved KV FIFO.

    After the initial K0/K1, each stream pipelines ``V(i), K(i + 1)``.
    This matches the consumer's PV/QK alternation and exposes the same load/MMA
    overlap as the original single-stream FA4 schedule.
    """
    m_block_sparse = sparse_tensor_m_block(m_block, qhead_per_kvhead, q_subtile_factor)
    (
        curr_mask_block_cnt,
    ...
    total_block_count = mask_end - mask_begin + full_end - full_begin

    if total_block_count > Int32(0):
        stream0_count = dual_stream_block_count(total_block_count, Int32(0))
        stream1_count = dual_stream_block_count(total_block_count, Int32(1))
        n_block0 = _block_from_reverse_concat_ordinal(
            Int32(0),
            curr_mask_block_idx,
            mask_begin,
            mask_end,
        ...
        load_K(block=n_block0, producer_state=kv_producer_state, page_idx=None)
        load_Q(block=0, stage=0)
        q_producer_phase ^= 1
        kv_producer_state.advance()

        if stream1_count > Int32(0):
            n_block1 = _block_from_reverse_concat_ordinal(
                Int32(1),
                curr_mask_block_idx,
                mask_begin,
                mask_end,
            ...
            load_K(block=n_block1, producer_state=kv_producer_state, page_idx=None)
            kv_producer_state.advance()

        for stream_idx in cutlass.range(stream0_count, unroll=1):
            ordinal0 = stream_idx * Int32(2)
            n_block0 = _block_from_reverse_concat_ordinal(
                ordinal0,
                curr_mask_block_idx,
                mask_begin,
            ...
            load_V(block=n_block0, producer_state=kv_producer_state, page_idx=None)
            kv_producer_state.advance()

            next_stream_idx = stream_idx + Int32(1)
            if next_stream_idx < stream0_count:
                n_block0_next = _block_from_reverse_concat_ordinal(
                    next_stream_idx * Int32(2),
                    curr_mask_block_idx,
                    mask_begin,
                    mask_end,
                    curr_full_block_idx,
            ...
            if stream_idx < stream1_count:
                ...
                load_V(block=n_block1, producer_state=kv_producer_state, page_idx=None)
                kv_producer_state.advance()

                if next_stream_idx < stream1_count:
                    n_block1_next = _block_from_reverse_concat_ordinal(
                        next_stream_idx * Int32(2) + Int32(1),
                        curr_mask_block_idx,
                        mask_begin,
                        mask_end,
                        curr_full_block_idx,
...
@cute.jit
def produce_block_sparse_loads_dual_stream_spdb_sm100(
    blocksparse_tensors: BlockSparseTensors,
    batch_idx,
    head_idx,
    m_block,
    ...
    After the initial K0/K1, each stream pipelines ``K(i + 1), V(i)``.
    This matches the score-double-buffered consumer, which can issue the next
    QK into an alternate score slot before waiting for the current P/PV.
    """
    m_block_sparse = sparse_tensor_m_block(m_block, qhead_per_kvhead, q_subtile_factor)
    (
        curr_mask_block_cnt,
...
def softmax_block_sparse_sm100(
    ...
    if total_block_cnt == 0:
        sm_stats_barrier.arrive_w_index(
            index=(
                stage_idx
                if const_expr(sm_stats_barrier_cta_wide)
                else stage_idx * 4 + warp_idx
            )
        )
    else:
...
@cute.jit
def softmax_block_sparse_dual_stream_sm100(
    blocksparse_tensors: BlockSparseTensors,
    batch_idx,
    head_idx,
    m_block,
    ...
    """Softmax traversal for one canonical even/odd KV stream."""
    warp_idx = cute.arch.make_warp_uniform(cute.arch.warp_idx()) % 4
    m_block_sparse = sparse_tensor_m_block(m_block, qhead_per_kvhead, q_subtile_factor)
    (
        curr_mask_block_cnt,
    ...
    mask_begin, mask_end = split_block_range(curr_mask_block_cnt, split_idx, num_splits)
    full_begin, full_end = split_block_range(curr_full_block_cnt, split_idx, num_splits)
    split_mask_count = mask_end - mask_begin
    split_full_count = full_end - full_begin

    mask_first_ordinal = stage_idx
    stream_mask_count = dual_stream_block_count(split_mask_count, mask_first_ordinal)
    # Full-list ordinals follow all masked-list ordinals, so carry parity over
    # the masked/full boundary rather than restarting it for the full list.
    full_first_ordinal = (stage_idx + split_mask_count) % Int32(2)
    stream_full_count = dual_stream_block_count(split_full_count, full_first_ordinal)
    stream_block_count = stream_mask_count + stream_full_count

    if stream_block_count == Int32(0):
        sm_stats_barrier.arrive_w_index(
            index=(
                stage_idx
                if const_expr(sm_stats_barrier_cta_wide)
        ...
        if stream_mask_count > Int32(0):
            local_ordinal = mask_first_ordinal
            n_block = curr_mask_block_idx[mask_end - Int32(1) - local_ordinal]
            (
                mma_si_consumer_phase,
                si_corr_producer_phase,
                s0_s1_sequence_phase,
            ...
            for i in cutlass.range(1, stream_mask_count, unroll=1):
                local_ordinal = mask_first_ordinal + i * Int32(2)
                n_block = curr_mask_block_idx[mask_end - Int32(1) - local_ordinal]
                (
                    mma_si_consumer_phase,
                    si_corr_producer_phase,
        ...
        if const_expr(curr_full_block_idx is not None):
            if stream_full_count > Int32(0):
                local_ordinal = full_first_ordinal
                n_block = curr_full_block_idx[full_end - Int32(1) - local_ordinal]
                if stream_mask_count == Int32(0):
                    (
                        mma_si_consumer_phase,
                        si_corr_producer_phase,
                ...
                for i in cutlass.range(1, stream_full_count, unroll=1):
                    local_ordinal = full_first_ordinal + i * Int32(2)
                    n_block = curr_full_block_idx[full_end - Int32(1) - local_ordinal]
                    (
                        mma_si_consumer_phase,
                        si_corr_producer_phase,
