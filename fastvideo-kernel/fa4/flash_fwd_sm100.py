...
from flash_attn.cute.block_sparse_utils import (
    dual_stream_block_count,
    get_dual_stream_block_counts,
    get_total_block_count,
    produce_block_sparse_loads_dual_stream_sm100,
    produce_block_sparse_loads_dual_stream_spdb_sm100,
    produce_block_sparse_loads_sm100,
    softmax_block_sparse_dual_stream_sm100,
...
def supports_sm100_tile_m64_head_dims(head_dim: int, head_dim_v: int) -> bool:
    return (
        8 <= head_dim <= 128
        and 8 <= head_dim_v <= 128
        and head_dim % 8 == 0
        and head_dim_v % 8 == 0
    ) or (head_dim, head_dim_v) == (192, 128)
...
class FlashAttentionForwardSm100:
    ...
    def __init__(
        ...
        num_kv_streams: cutlass.Constexpr[int] = 1,
        use_sp_double_buffer: cutlass.Constexpr[bool] = False,
        ...
        self.p_in_smem = m_block_size == 64
        self.q_stage = 1 if self.p_in_smem else q_stage
        assert self.q_stage in [1, 2]
        if m_block_size not in (64, 128) or (
            m_block_size == 64 and n_block_size not in (64, 128)
        ):
            raise NotImplementedError(
                "FlashAttentionForwardSm100 supports m_block_size=128, or m_block_size=64 "
                f"with n_block_size in (64, 128); got ({m_block_size}, {n_block_size})."
            )
        if self.p_in_smem and (
            not supports_sm100_tile_m64_head_dims(head_dim, head_dim_v)
            or use_2cta_instrs
            or use_clc_scheduler
            or paged_kv_non_tma
        ):
            raise NotImplementedError(
                "SM100 tile_m=64 forward requires supported head dimensions, 1CTA, "
                "the static scheduler, and TMA KV loads"
            )
        self.num_kv_streams = num_kv_streams
        assert self.num_kv_streams in [1, 2]
        self.num_compute_slots = self.q_stage * self.num_kv_streams
        assert self.num_compute_slots in [1, 2]
        self.use_sp_double_buffer = use_sp_double_buffer
        if self.use_sp_double_buffer:
            assert self.num_kv_streams == 2
            assert self.q_stage == 1
            assert self.m_block_size == 128 and self.n_block_size == 64
            assert self.head_dim_padded == 128 and self.head_dim_v_padded == 128
            assert not is_causal and not is_local
            assert not is_varlen_q and not is_split_kv and not pack_gqa
            assert self.use_tma_KV and not use_2cta_instrs and not use_clc_scheduler
        self.num_sp_tmem_slots = 4 if self.use_sp_double_buffer else 2
        self.num_sp_pipeline_stages = (
            4 if self.use_sp_double_buffer else self.num_compute_slots
        )
        ...
        if self.p_in_smem and self.arch not in (Arch.sm_100a, Arch.sm_100f):
        ...
        self.tmem_s_offset = (
            [i * self.n_block_size for i in range(self.num_sp_tmem_slots)]
            if self.use_sp_double_buffer
            else [0, self.n_block_size]
        )
        self.tmem_o_offset = [
            self.tmem_s_offset[-1] + self.n_block_size + i * self.head_dim_v_padded
            for i in range(self.num_compute_slots)
        ]
        self.tmem_total = self.tmem_o_offset[-1] + self.head_dim_v_padded
        assert self.tmem_total <= self.tmem_alloc_cols
        self.tmem_s_to_p_offset = self.n_block_size // 2
        self.tmem_p_offset = [
            self.tmem_s_offset[i] + self.tmem_s_to_p_offset
            for i in range(self.num_sp_tmem_slots)
        ]
        ...
        if self.p_in_smem and self.num_kv_streams == 2:
    ...
    def _setup_attributes(self):
        ...
        self.s_stage = self.num_sp_tmem_slots
        smem_size_p = (
        ...
        assert self.s_stage >= self.num_compute_slots
    ...
    def __call__(
        ...
        if const_expr(
            self.p_in_smem and self.q_dtype not in (cutlass.Float16, cutlass.BFloat16)
        ):
            raise TypeError("SM100 tile_m=64 forward requires FP16 or BF16 tensors")
        ...
        if const_expr(
            self.num_kv_streams == 2
            and self.m_block_size == 128
            and self.n_block_size == 64
        ...
        if const_expr(
            ...
            self.ex2_emu_freq = 24 if self.use_sp_double_buffer else 8
        ...
        tiled_mma_pv = sm100_utils_basic.make_trivial_tiled_mma(
            ...
            cta_group,
            self.mma_tiler_pv[:2],
            tcgen05.OperandSource.SMEM if self.p_in_smem else tcgen05.OperandSource.TMEM,
        )
        ...
        smem_p_size = (
            self.s_stage * self.m_block_size * self.n_block_size if self.p_in_smem else 0
        )
        ...
        class SharedStorage:
            ...
            mbar_S_full_P_full_O_rescaled: cute.struct.MemRange[
                Int64, self.num_sp_pipeline_stages * 2
            ]
            mbar_P_full_lastsplit: cute.struct.MemRange[
                Int64, self.num_sp_pipeline_stages * 2
            ]
            mbar_O_full: cute.struct.MemRange[Int64, self.num_compute_slots * 2]
            mbar_softmax_stats: cute.struct.MemRange[Int64, self.num_compute_slots * 2]
            # mbar_softmax_stats: cute.struct.MemRange[Int64, self.num_compute_slots * 4 * 2]
            ...
            sScale: cute.struct.MemRange[
                Float32, self.num_compute_slots * self.m_block_size * 2
            ]
            ...
            sP: cute.struct.Align[
                cute.struct.MemRange[self.q_dtype, smem_p_size],
                self.buffer_align_bytes,
            ]
    ...
    def kernel(
        ...
        pipeline_s_p_o = pipeline_custom.PipelineUmmaAsync.create(
            ...
            num_stages=self.num_sp_pipeline_stages,
        ...
        pipeline_o_acc = pipeline_custom.PipelineUmmaAsync.create(
            ...
            num_stages=self.num_compute_slots,
            producer_group=mma_warp,
        ...
        if const_expr(self.s0_s1_barrier and self.num_compute_slots > 1):
        ...
        sm_stats_barrier = pipeline_custom.NamedBarrier(
            ...
            num_threads=cute.arch.WARP_SIZE * (8 if self.p_in_smem else 2),
        )
        smem_p_barrier = pipeline_custom.NamedBarrier(
            barrier_id=int(NamedBarrierFwdSm100.SoftmaxSmemP0),
        ...
        sScale = storage.sScale.get_tensor(
            cute.make_layout(self.num_compute_slots * self.m_block_size * 2)
        )
        ...
        if const_expr(self.p_in_smem):
            tStS = thr_mma_qk.make_fragment_C(qk_acc_shape)
            tStS = cute.make_tensor(
                tStS.iterator,
                cute.append(
                    tStS.layout,
                    cute.make_layout(
                        (self.s_stage,), stride=(self.n_block_size,)
                    ),
                ),
            )
        else:
            tStS = thr_mma_qk.make_fragment_C(
                cute.append(qk_acc_shape, self.s_stage)
            )
        pv_acc_shape = thr_mma_pv.partition_shape_C(self.mma_tiler_pv[:2])
        if const_expr(self.p_in_smem):
            tOtO = thr_mma_pv.make_fragment_C(pv_acc_shape)
            # The compact codomain of the M64 accumulator layout includes TMEM
            # datapath bits. Letting cute.append infer a stage stride would move
            # stage 1 across datapaths instead of to the next column range.
            tOtO = cute.make_tensor(
                tOtO.iterator,
                cute.append(
                    tOtO.layout,
                    cute.make_layout(
                        (self.num_compute_slots,),
                        stride=(self.head_dim_v_padded,),
                    ),
                ),
            )
            tOtO_addr = (tOtO.iterator + self.tmem_o_offset[0]).toint()
            tOtO_ptr = cute.make_ptr(
                self.pv_acc_dtype,
                tOtO_addr,
                cute.AddressSpace.tmem,
                assumed_align=8,
            )
            tOtO = cute.make_tensor(tOtO_ptr, tOtO.layout)
        else:
            tOtO = thr_mma_pv.make_fragment_C(
                cute.append(pv_acc_shape, self.num_compute_slots)
            )
        ...
        if warp_idx == self.mma_warp_id:
            ...
            mma_fn = (
                self.mma_dual_stream_spdb
                if const_expr(self.use_sp_double_buffer)
                else self.mma_dual_stream
                if const_expr(self.num_kv_streams == 2)
                else self.mma
            )
            mma_fn(
                tiled_mma_qk,
        ...
        if (
            (
                const_expr(self.num_compute_slots == 2)
                and warp_idx <= self.softmax1_warp_ids[-1]
            )
            or (
                const_expr(self.num_compute_slots == 1)
                and warp_idx <= self.softmax0_warp_ids[-1]
            )
        ):
            ...
            if const_expr(not self.s0_s1_barrier):
                stage = Int32(
                    0
                    if const_expr(self.num_compute_slots == 1)
        ...
        if warp_idx >= self.correction_warp_ids[0] and warp_idx < self.mma_warp_id:
            ...
            tmem_ptr = tmem.retrieve_ptr(self.qk_acc_dtype)
            correction_fn = (
                self.correction_loop_dual_stream_spdb
                if const_expr(self.use_sp_double_buffer)
                else self.correction_loop_dual_stream
                if const_expr(self.num_kv_streams == 2)
                else self.correction_loop
            )
            correction_fn(
                thr_mma_qk,
                thr_mma_pv,
            ...
            if const_expr(not self.use_block_sparsity):
                ...
                if const_expr(self.num_kv_streams == 2):
                    dual_stream_load = (
                        produce_block_sparse_loads_dual_stream_spdb_sm100
                        if const_expr(self.use_sp_double_buffer)
                        else produce_block_sparse_loads_dual_stream_sm100
                    )
                    kv_producer_state, q_producer_phase = dual_stream_load(
                            blocksparse_tensors,
                            batch_idx,
                            head_idx,
                            m_block,
    ...
    def mma(
        ...
        gemm_Si = [
            partial(
                ...
                smem_offset=(
                    -sQ_stage_stride
                    if slot // self.num_kv_streams == 0
                    else sQ_stage_stride
                ),
                zero_init=True,
        ...
        gemm_Pi = [
            partial(
                ...
                pv_mma_op,
                self.tmem_o_offset[slot],
                tOrP[None, None, None, slot],
                sA=sP[None, None, None, slot] if self.p_in_smem else None,
        ...
        while work_tile.is_valid_tile:
            ...
            if process_tile and is_leader_cta:
                for stage in cutlass.range_constexpr(self.num_compute_slots):
                    # GEMM_QK00 (Q0 * K0 -> S0) or GEMM_QK01 (Q1 * K0 -> S1)
                    # 1. wait for Q0 / Q1
                    if const_expr(stage % self.num_kv_streams == 0):
                        pipeline_q.consumer_wait_w_index_phase(
                            stage // self.num_kv_streams, mma_q_consumer_phase
                        )
                    # 2. wait for K0
                ...
                for i in cutlass.range(block_loop_count, unroll=1):
                    ...
                    for stage in cutlass.range_constexpr(self.num_compute_slots):
                        ...
                        # 4. release V(i-1)
                        if const_expr(stage == self.num_compute_slots - 1):
    ...
    def mma_dual_stream(
        ...
        """MMA consumer for the q-stage-1, two-KV-stream sparse schedule.

        The load warp writes one FIFO pair as K(even), K(odd), V(even),
        V(odd).  The two score/output slots share Q but keep independent P/O
        barrier phases so an odd tail does not advance the inactive stream.
        """
        assert const_expr(self.q_stage == 1)
        assert const_expr(self.num_kv_streams == 2)
        assert const_expr(self.use_block_sparsity)
        ...
        while work_tile.is_valid_tile:
            m_block, head_idx, batch_idx, split_idx = work_tile.tile_idx
            seqlen = SeqlenInfoCls(batch_idx)
            stream0_count, stream1_count = get_dual_stream_block_counts(
                blocksparse_tensors,
                batch_idx,
                head_idx,
                m_block,
            ...
            if stream0_count > Int32(0) and is_leader_cta:
                pipeline_q.consumer_wait_w_index_phase(0, mma_q_consumer_phase)
                o0_should_accumulate = False
                o1_should_accumulate = False

                # Initial score MMAs consume the two prefetched K blocks.
                pipeline_kv.consumer_wait(mma_kv_consumer_state)
                k0_idx, k0_phase = (
                    mma_kv_consumer_state.index,
                    mma_kv_consumer_state.phase,
                )
                sK0 = sK[None, None, None, k0_idx]
                if const_expr(self.uneven_kv_smem):
                    sK0 = self.offset_kv_smem(sK0, k0_idx, k0_phase)
                gemm_Si[0](
                    smem_desc_start_b=sm100_desc.make_smem_desc_start_addr(sK0.iterator)
                )
                pipeline_s_p_o.producer_commit_w_index(0)
                pipeline_kv.consumer_release(mma_kv_consumer_state)
                mma_kv_consumer_state.advance()

                if stream1_count > Int32(0):
                    pipeline_kv.consumer_wait(mma_kv_consumer_state)
                    k1_idx, k1_phase = (
                        mma_kv_consumer_state.index,
                        mma_kv_consumer_state.phase,
                    )
                    sK1 = sK[None, None, None, k1_idx]
                    if const_expr(self.uneven_kv_smem):
                        sK1 = self.offset_kv_smem(sK1, k1_idx, k1_phase)
                    gemm_Si[1](
                        smem_desc_start_b=sm100_desc.make_smem_desc_start_addr(sK1.iterator)
                    )
                    pipeline_s_p_o.producer_commit_w_index(1)
                    pipeline_kv.consumer_release(mma_kv_consumer_state)
                    mma_kv_consumer_state.advance()

                if stream0_count == Int32(1):
                    pipeline_q.consumer_release_w_index(0)
                    mma_q_consumer_phase ^= 1

                for stream_idx in cutlass.range(stream0_count, unroll=1):
                    # PV(s0, i), followed immediately by QK(s0, i + 1).
                    pipeline_kv.consumer_wait(mma_kv_consumer_state)
                    v0_idx, v0_phase = (
                        mma_kv_consumer_state.index,
                        mma_kv_consumer_state.phase,
                    )
                    sV0 = sV[None, None, None, v0_idx]
                    if const_expr(self.uneven_kv_smem):
                        sV0 = self.offset_kv_smem(sV0, v0_idx, v0_phase)
                    pipeline_s_p_o.producer_acquire_w_index_phase(0, p_phase0)
                    gemm_Pi[0](
                        tCrB=tOrV[None, None, None, v0_idx],
                        sB=sV0,
                        zero_init=not o0_should_accumulate,
                        mbar_ptr=(
                            pipeline_p_lastsplit.sync_object_full.get_barrier(0)
                            if self.split_P_arrive > 0
                            else None
                        ),
                        mbar_phase=p_phase0,
                    )
                    if stream_idx == stream0_count - Int32(1):
                        pipeline_o_acc.producer_commit_w_index(0)
                    pipeline_kv.consumer_release(mma_kv_consumer_state)
                    mma_kv_consumer_state.advance()
                    p_phase0 ^= 1
                    o0_should_accumulate = True

                    next_stream_idx = stream_idx + Int32(1)
                    if next_stream_idx < stream0_count:
                        pipeline_kv.consumer_wait(mma_kv_consumer_state)
                        k0_next_idx, k0_next_phase = (
                            mma_kv_consumer_state.index,
                            mma_kv_consumer_state.phase,
                        )
                        sK0_next = sK[None, None, None, k0_next_idx]
                        if const_expr(self.uneven_kv_smem):
                            sK0_next = self.offset_kv_smem(
                                sK0_next, k0_next_idx, k0_next_phase
                            )
                        gemm_Si[0](
                        ...
                        mma_kv_consumer_state.advance()
                        if (
                            next_stream_idx == stream0_count - Int32(1)
                            and stream0_count > stream1_count
                        ):
                            pipeline_q.consumer_release_w_index(0)
                            mma_q_consumer_phase ^= 1

                    # PV(s1, i), followed immediately by QK(s1, i + 1).
                    if stream_idx < stream1_count:
                        pipeline_kv.consumer_wait(mma_kv_consumer_state)
                        v1_idx, v1_phase = (
                            mma_kv_consumer_state.index,
                            mma_kv_consumer_state.phase,
                        )
                        sV1 = sV[None, None, None, v1_idx]
                        if const_expr(self.uneven_kv_smem):
                            sV1 = self.offset_kv_smem(sV1, v1_idx, v1_phase)
                        pipeline_s_p_o.producer_acquire_w_index_phase(1, p_phase1)
                        gemm_Pi[1](
                            tCrB=tOrV[None, None, None, v1_idx],
                            sB=sV1,
                            zero_init=not o1_should_accumulate,
                            mbar_ptr=(
                                pipeline_p_lastsplit.sync_object_full.get_barrier(1)
                                if self.split_P_arrive > 0
                                else None
                            ),
                            mbar_phase=p_phase1,
                        )
                        if stream_idx == stream1_count - Int32(1):
                            pipeline_o_acc.producer_commit_w_index(1)
                        pipeline_kv.consumer_release(mma_kv_consumer_state)
                        mma_kv_consumer_state.advance()
                        p_phase1 ^= 1
                        o1_should_accumulate = True

                        if next_stream_idx < stream1_count:
                            pipeline_kv.consumer_wait(mma_kv_consumer_state)
                            k1_next_idx, k1_next_phase = (
                                mma_kv_consumer_state.index,
                                mma_kv_consumer_state.phase,
                            )
                            sK1_next = sK[None, None, None, k1_next_idx]
                            if const_expr(self.uneven_kv_smem):
                                sK1_next = self.offset_kv_smem(
                                    sK1_next, k1_next_idx, k1_next_phase
                                )
                            gemm_Si[1](
                                smem_desc_start_b=sm100_desc.make_smem_desc_start_addr(
                                    sK1_next.iterator
                            ...
                            mma_kv_consumer_state.advance()
                            if (
                                next_stream_idx == stream1_count - Int32(1)
                                and stream0_count == stream1_count
                            ):
                                pipeline_q.consumer_release_w_index(0)
                                mma_q_consumer_phase ^= 1
    ...
    def mma_dual_stream_spdb(
        ...
        With score double buffering, each stream alternates between two S/P
        slots while retaining one O accumulator.  The load and MMA FIFOs both
        order K(i + 1) before V(i), allowing QK(i + 1) to run ahead of the
        P(i)/PV(i) dependency.
        """
        assert const_expr(self.q_stage == 1)
        assert const_expr(self.num_kv_streams == 2)
        assert const_expr(self.use_sp_double_buffer)
        assert const_expr(self.use_block_sparsity)
        ...
        while work_tile.is_valid_tile:
            ...
            if stream0_count > Int32(0) and is_leader_cta:
                ...
                if const_expr(self.uneven_kv_smem):
                    sK0 = self.offset_kv_smem(sK0, k0_idx, k0_phase)
                sp_slot0 = Int32(0) + Int32(2) * (qk_count0 % Int32(2))
                gemm_S(
                    sp_slot0 * self.n_block_size,
                    smem_desc_start_b=sm100_desc.make_smem_desc_start_addr(sK0.iterator)
                )
                pipeline_s_p_o.producer_commit_w_index(sp_slot0)
                pipeline_kv.consumer_release(mma_kv_consumer_state)
                mma_kv_consumer_state.advance()
                qk_count0 += Int32(1)
                ...
                if stream1_count > Int32(0):
                    ...
                    if const_expr(self.uneven_kv_smem):
                        sK1 = self.offset_kv_smem(sK1, k1_idx, k1_phase)
                    sp_slot1 = Int32(1) + Int32(2) * (qk_count1 % Int32(2))
                    gemm_S(
                        sp_slot1 * self.n_block_size,
                        smem_desc_start_b=sm100_desc.make_smem_desc_start_addr(sK1.iterator)
                    )
                    pipeline_s_p_o.producer_commit_w_index(sp_slot1)
                    pipeline_kv.consumer_release(mma_kv_consumer_state)
                    mma_kv_consumer_state.advance()
                    qk_count1 += Int32(1)
                ...
                for stream_idx in cutlass.range(stream0_count, unroll=1):
                    ...
                    if const_expr(self.uneven_kv_smem):
                        sV0 = self.offset_kv_smem(sV0, v0_idx, v0_phase)
                    sp_slot0 = Int32(0) + Int32(2) * (pv_count0 % Int32(2))
                    sp_phase0 = pv_count0 // Int32(2)
                    pipeline_s_p_o.producer_acquire_w_index_phase(sp_slot0, sp_phase0)
                    gemm_P(
                        self.tmem_o_offset[0],
                        tOrP[None, None, None, sp_slot0],
                        tCrB=tOrV[None, None, None, v0_idx],
                        sA=None,
                        sB=sV0,
                        zero_init=not o0_should_accumulate,
                        mbar_ptr=(
                            pipeline_p_lastsplit.sync_object_full.get_barrier(sp_slot0)
                            if self.split_P_arrive > 0
                            else None
                        ),
                        mbar_phase=sp_phase0,
                    )
                    pipeline_o_acc.producer_commit_w_index(0)
                    pipeline_kv.consumer_release(mma_kv_consumer_state)
                    mma_kv_consumer_state.advance()
                    ...
                    if stream_idx < stream1_count:
                        ...
                        if const_expr(self.uneven_kv_smem):
                            sV1 = self.offset_kv_smem(sV1, v1_idx, v1_phase)
                        sp_slot1 = Int32(1) + Int32(2) * (pv_count1 % Int32(2))
                        sp_phase1 = pv_count1 // Int32(2)
                        pipeline_s_p_o.producer_acquire_w_index_phase(sp_slot1, sp_phase1)
                        gemm_P(
                            self.tmem_o_offset[1],
                            tOrP[None, None, None, sp_slot1],
                            tCrB=tOrV[None, None, None, v1_idx],
                            sA=None,
                            sB=sV1,
                            zero_init=not o1_should_accumulate,
                            mbar_ptr=(
                                pipeline_p_lastsplit.sync_object_full.get_barrier(sp_slot1)
                                if self.split_P_arrive > 0
                                else None
                            ),
                            mbar_phase=sp_phase1,
                        )
                        pipeline_o_acc.producer_commit_w_index(1)
                        pipeline_kv.consumer_release(mma_kv_consumer_state)
                        mma_kv_consumer_state.advance()
    ...
    def softmax_loop(
        ...
        descale_tensors: Optional[DescaleTensors],
        thr_mma_qk: cute.ThrMma,
        tStS: cute.Tensor,  # ((TILE_M, TILE_N), 1, 1, num_sp_tmem_slots)
        sP: cute.Tensor | None,
        sScale: cute.Tensor,
        mLSE: Optional[cute.Tensor],
        pipeline_s_p_o: pipeline.PipelineAsync,
        ...
        smem_p_barrier: pipeline.NamedBarrier,
        ...
        q_stage_idx = stage // self.num_kv_streams
        ...
        if const_expr(self.p_in_smem):
            ...
            if const_expr(self.use_sp_double_buffer):
                tSAcc_pair = cute.make_tensor(
                    tSAcc.iterator,
                    cute.append(
                        tSAcc.layout,
                        cute.make_layout(
                            (2,), stride=(2 * self.n_block_size,)
                        ),
                    ),
                )
                tStS_t2r = thr_tmem_load.partition_S(tSAcc_pair)
            else:
                tStS_t2r = thr_tmem_load.partition_S(tSAcc)
            ...
            thr_tmem_store = tcgen05.make_tmem_copy(tmem_store_atom, tStP_base).get_slice(tidx)
            if const_expr(self.use_sp_double_buffer):
                ...
                tStP_r2t = thr_tmem_store.partition_D(tStP_base)
        ...
        while work_tile.is_valid_tile:
            ...
            shared_mask_kwargs = dict(
                m_block=(self.q_stage * m_block + q_stage_idx) * self.cta_group_size,
                thr_mma=thr_mma_qk,
            ...
            softmax = SoftmaxSm100.create(
                ...
                row_pair_stride=16 if const_expr(self.p_in_smem) else 0,
            )
            softmax.reset()
            ...
            if const_expr(self.use_block_sparsity):
                if const_expr(self.num_kv_streams == 2):
                    ...
                    tile_block_count = (
                        stream0_count if stage == Int32(0) else stream1_count
                    )
                else:
            ...
            softmax_step = partial(
                ...
                tStS_t2r=tStS_t2r,
                tStP_r2t=tStP_r2t,
                smem_store_thr=smem_store_thr,
                sP_smem_view=sP_smem_view,
                sScale=sScale,
                stage=stage,
                batch_idx=batch_idx,
                head_idx=head_idx,
            ...
            if const_expr(self.use_block_sparsity):
                ...
                if const_expr(aux_tensors is not None):
                    m_tile_end = (
                        (self.q_stage * m_block + q_stage_idx + 1) * self.cta_group_size
                    ) * self.m_block_size
                ...
                if not empty_tile:
                    my_row = self._softmax_tidx_to_row(tidx)
                    sScale[my_row + stage * self.m_block_size] = softmax.row_sum[0]
                    if const_expr(
                        self.num_kv_streams == 2
                        or mLSE is not None
                        ...
                        sScale[
                            ...
                            + self.num_compute_slots * self.m_block_size
                        ] = softmax.row_max[0]
                    # if tidx == 0:
                    ...
                    sm_stats_barrier.arrive_w_index(
                        index=stage if self.p_in_smem else stage * 4 + warp_idx
                    )
    ...
    def softmax_step(
        ...
        smem_store_thr: cute.ThrCopy | None,
        sP_smem_view: cute.Tensor | None,
        sScale: cute.Tensor,
        stage: int | Int32,
        batch_idx: Int32,
        head_idx: Int32,
        ...
        if const_expr(self.use_sp_double_buffer):
            sp_parity = mma_si_consumer_phase % Int32(2)
            sp_slot = stage + Int32(2) * sp_parity
            sp_phase = mma_si_consumer_phase // Int32(2)
            tStS_t2r_cur = tStS_t2r[None, None, None, sp_parity]
            tStP_r2t_cur = tStP_r2t[None, None, None, sp_parity]
        else:
            sp_slot = stage
            sp_phase = mma_si_consumer_phase
            tStS_t2r_cur = tStS_t2r
            tStP_r2t_cur = tStP_r2t

        # Wait for Si
        pipeline_s_p_o.consumer_wait_w_index_phase(sp_slot, sp_phase)
        tSrS_t2r = cute.make_rmem_tensor(thr_tmem_load.partition_D(tScS).shape, self.qk_acc_dtype)
        cute.copy(thr_tmem_load, tStS_t2r_cur, tSrS_t2r)
        ...
        if const_expr(not is_first):
            ...
            sScale[
                self._softmax_tidx_to_row(thread_idx) + stage * self.m_block_size
            ] = acc_scale
        ...
        if const_expr(self.p_in_smem):
            rP_smem_view = smem_store_thr.retile(tSrP_r2t)
            cute.copy(
                smem_store_thr,
                rP_smem_view,
                sP_smem_view[None, None, None, stage],
            )
            cute.arch.fence_view_async_shared()
            # The S/P/O pipeline already counts every softmax and correction
            # consumer; its release is the visibility handshake for this P tile.
            pipeline_s_p_o.consumer_release_w_index(sp_slot)
        else:
            for i in cutlass.range_constexpr(cute.size(tStP_r2t_cur.shape[2])):
                cute.copy(
                    thr_tmem_store,
                    tSrP_r2t_f32[None, None, i],
                    tStP_r2t_cur[None, None, i],
                )
                if const_expr(self.split_P_arrive > 0):
                    split_P_arrive_idx = (
                        cute.size(tStP_r2t_cur.shape[2])
                        * self.split_P_arrive
                        // self.n_block_size
                    )
            ...
            if const_expr(self.split_P_arrive > 0):
                ...
                with cute.arch.elect_one():
                    pipeline_p_lastsplit.producer_commit_w_index(sp_slot)
            else:
                pipeline_s_p_o.consumer_release_w_index(sp_slot)
        pipeline_sm_stats.producer_acquire_w_index_phase(stage, sm_stats_producer_phase)
        ...
        mma_si_consumer_phase = (
            mma_si_consumer_phase + Int32(1)
            if const_expr(self.use_sp_double_buffer)
            else mma_si_consumer_phase ^ 1
        )
        return mma_si_consumer_phase, sm_stats_producer_phase ^ 1, s0_s1_sequence_phase ^ 1
        ...
        while work_tile.is_valid_tile:
            ...
            if has_work:
                ...
                sm_stats_barrier.arrive_and_wait_w_index(
                    index=0 if self.p_in_smem else 0 * 4 + warp_idx
                )
                pipeline_sm_stats.consumer_release_w_index(0)
                if const_expr(self.num_compute_slots == 2):
                    # pipeline_sm_stats.consumer_wait_w_index_phase(1, sm_stats_consumer_phase)
                    sm_stats_barrier.arrive_and_wait_w_index(
                        index=1 if self.p_in_smem else 1 * 4 + warp_idx
                    )
                ...
                for i in cutlass.range(total_block_count - 1, unroll=1):
                    for stage in cutlass.range_constexpr(self.num_compute_slots):
                        ...
                        if const_expr(self.p_in_smem):
                            self.correction_rescale_smem_p(
                                thr_mma_pv,
                            ...
                            scale = sScale[row_tidx + stage * self.m_block_size]
                        ...
                        pipeline_sm_stats.consumer_release_w_index(
                            self.num_compute_slots - 1 - stage
                        )
                    sm_stats_consumer_phase ^= 1
                ...
                if const_expr(learnable_sink is not None):
                    if const_expr(not self.pack_gqa):
                        ...
                        for stage in cutlass.range_constexpr(self.q_stage):
                            q_head_idx = (
                                ((m_block * self.q_stage + stage) * self.cta_group_size + mma_tile_coord_v) * self.m_block_size + row_tidx
                ...
                for stage in cutlass.range_constexpr(self.q_stage):
                    ...
                    # cute.arch.fence_view_async_tmem_load()
                    # scale = tSrScale_t2r[0]
                    row_sum = sScale[row_tidx + stage * self.m_block_size]
                    if const_expr(mLSE is not None or learnable_sink is not None):
                        row_max = sScale[
                            row_tidx
                            + (stage + self.num_compute_slots) * self.m_block_size
                        ]
                    else:
                        row_max = None
                    ...
                    if const_expr(self.p_in_smem):
                        correction_threads = (
                            len(self.correction_warp_ids) * cute.arch.WARP_SIZE
                        )
                        cute.arch.barrier(
                            barrier_id=int(NamedBarrierFwdSm100.CorrectionScale),
                            number_of_threads=correction_threads,
                        )
                        if tidx < self.m_block_size:
                            sScale[row_tidx + stage * self.m_block_size] = scale
                        cute.arch.barrier(
                            barrier_id=int(NamedBarrierFwdSm100.CorrectionScale),
                            number_of_threads=correction_threads,
                        )
                    ...
                    self.correction_epilogue(
                        ...
                        gO_stage,
                        gmem_tiled_copy_O,
                        sScale=sScale if const_expr(self.p_in_smem) else None,
                    )
                ...
                if const_expr(self.use_block_sparsity):
                    (
                        ...
                        split_idx,
                        stats,
                        partial(self.correction_epilogue, zero_output=True),
                        thr_mma_pv,
                        tOtO,
                        sO,
                        pipeline_sm_stats,
            ...
            if const_expr(mLSE is not None):
                ...
                for stage in cutlass.range_constexpr(self.q_stage):
                    ...
                    if const_expr(not self.pack_gqa or self.m_block_size % self.qhead_per_kvhead == 0):
                        ...
                        if (
                            tidx < self.m_block_size
                            and row_tidx < seqlen_q - m_tile_idx * self.m_block_size
                        ):
                            # This actually just works with PackGQA too
                            gLSE[row_tidx] = lse
                    else:
                        idx = m_tile_idx * self.m_block_size + row_tidx
                        if tidx < self.m_block_size and idx < seqlen_q:
    ...
    @cute.jit
    def correction_loop_dual_stream(
        self,
        thr_mma_qk: cute.ThrMma,
        thr_mma_pv: cute.ThrMma,
        tStS: cute.Tensor,
        ...
        """Correct and merge two independently-normalized sparse KV streams."""
        assert const_expr(self.num_kv_streams == 2)
        assert const_expr(self.q_stage == 1)
        assert const_expr(self.use_block_sparsity)
        assert const_expr(self.use_tma_O)
        assert const_expr(not self.use_correction_warps_for_epi)
        assert const_expr(not self.pack_gqa)
        assert const_expr(not self.is_split_kv)
        assert const_expr(not self.is_varlen_q)
        assert const_expr(learnable_sink is None)
        ...
        # Seed the first P/O acquire for each stream.  Thereafter a stream is
        # released only when it has a real PV event; empty tiles leave this
        # seed untouched for the next active tile.
        pipeline_s_p_o.consumer_release_w_index(0)
        pipeline_s_p_o.consumer_release_w_index(1)

        o_corr_phase0 = Int32(0)
        o_corr_phase1 = Int32(0)
        corr_epi_producer_phase = Int32(1)
        ...
        while work_tile.is_valid_tile:
            ...
            has_stream0 = stream0_count > Int32(0)
            has_stream1 = stream1_count > Int32(0)

            # Every stream emits one first signal: the first score block when
            # active, or a synthetic empty-stream signal otherwise.
            sm_stats_barrier.arrive_and_wait_w_index(
                index=0 if self.p_in_smem else 0 * 4 + warp_idx
            )
            pipeline_sm_stats.consumer_release_w_index(0)
            ...
            # Signals 1..count-1 carry the online-softmax correction scale.
            if stream0_count > Int32(1):
                for corr_idx in cutlass.range(stream0_count - Int32(1), unroll=1):
                    sm_stats_barrier.arrive_and_wait_w_index(
                        index=0 if self.p_in_smem else 0 * 4 + warp_idx
                    )
                    if const_expr(self.p_in_smem):
                        ...
                        scale0 = sScale[row_tidx]
                        if cute.arch.vote_ballot_sync(scale0 < Float32(1.0)) != 0:
                            self.correction_rescale(
                                thr_mma_pv,
                                tOtO[None, None, None, 0],
                                tidx,
                    ...
                    if corr_idx + Int32(1) < stream1_count:
                        sm_stats_barrier.arrive_and_wait_w_index(
                            index=1 if self.p_in_smem else 1 * 4 + warp_idx
                        )
                        if const_expr(self.p_in_smem):
                            ...
                            scale1 = sScale[row_tidx + self.m_block_size]
                            if cute.arch.vote_ballot_sync(scale1 < Float32(1.0)) != 0:
                                self.correction_rescale(
                                    thr_mma_pv,
                                    tOtO[None, None, None, 1],
                                    tidx,
            ...
            # Active streams emit one additional final-stat signal.  Empty
            # streams have already completed their sole synthetic signal above.
            ...
            if has_stream0:
                ...
                row_sum0 = sScale[row_tidx]
                row_max0 = sScale[
                    row_tidx + self.num_compute_slots * self.m_block_size
                ]
                pipeline_sm_stats.consumer_release_w_index(0)
                pipeline_o_acc.consumer_wait_w_index_phase(0, o_corr_phase0)
            if has_stream1:
                sm_stats_barrier.arrive_and_wait_w_index(
                    index=1 if self.p_in_smem else 1 * 4 + warp_idx
                )
                row_sum1 = sScale[row_tidx + self.m_block_size]
                row_max1 = sScale[
                    row_tidx
                    + self.m_block_size
                    + self.num_compute_slots * self.m_block_size
                ...
                pipeline_sm_stats.consumer_release_w_index(1)
                pipeline_o_acc.consumer_wait_w_index_phase(1, o_corr_phase1)

            valid0 = has_stream0 and row_sum0 != Float32(0.0) and row_sum0 == row_sum0
            valid1 = has_stream1 and row_sum1 != Float32(0.0) and row_sum1 == row_sum1
            merged_row_max = -Float32.inf
            if valid0:
                merged_row_max = row_max0
            if valid1:
                merged_row_max = (
                    max(merged_row_max, row_max1) if valid0 else row_max1
                )
            ...
            if valid0:
                alpha0 = cute.math.exp2(
                    (row_max0 - merged_row_max) * softmax_scale_log2_eff,
                    fastmath=True,
                )
            if valid1:
                alpha1 = cute.math.exp2(
                    (row_max1 - merged_row_max) * softmax_scale_log2_eff,
                    fastmath=True,
                )
            merged_row_sum = alpha0 * row_sum0 + alpha1 * row_sum1
            merged_invalid = (
                merged_row_sum == Float32(0.0) or merged_row_sum != merged_row_sum
            )
            inv_sum = cute.arch.rcp_approx(
                merged_row_sum if not merged_invalid else Float32(1.0)
            )
            output_scale0 = (
                alpha0 * inv_sum * v_descale if not merged_invalid else Float32(0.0)
            )
            output_scale1 = (
                alpha1 * inv_sum * v_descale if not merged_invalid else Float32(0.0)
            )
            if const_expr(self.p_in_smem):
                correction_threads = (
                    len(self.correction_warp_ids) * cute.arch.WARP_SIZE
                ...
                if tidx < self.m_block_size:
                    sScale[row_tidx] = output_scale0
                    sScale[row_tidx + self.m_block_size] = output_scale1
                cute.arch.barrier(
                    barrier_id=int(NamedBarrierFwdSm100.CorrectionScale),
                    number_of_threads=correction_threads,
                )
            ...
            self.correction_merge_epilogue(
                thr_mma_pv,
                tOtO[None, None, None, 0],
                tOtO[None, None, None, 1],
                tidx,
            ...
            if const_expr(mLSE is not None):
                ...
                lse = -Float32.inf
                if not merged_invalid:
                    lse = (
                        merged_row_max * softmax_scale_log2_eff
                        + cute.math.log2(merged_row_sum, fastmath=True)
                        - max_offset
                    ) * Float32(math.log(2.0))
                if (
                    tidx < self.m_block_size
                    and row_tidx < seqlen.seqlen_q - m_block * self.m_block_size
                ):
                    gLSE[row_tidx] = lse
    ...
    @cute.jit
    def correction_loop_dual_stream_spdb(
        self,
        thr_mma_qk: cute.ThrMma,
        thr_mma_pv: cute.ThrMma,
        tStS: cute.Tensor,
        ...
        o_corr_phase0 = Int32(0)
        o_corr_phase1 = Int32(0)
        sp_corr_count0 = Int32(0)
        sp_corr_count1 = Int32(0)
        corr_epi_producer_phase = Int32(1)
        ...
        while work_tile.is_valid_tile:
            ...
            pipeline_sm_stats.consumer_release_w_index(0)
            if has_stream0:
                sp_corr_count0 += Int32(1)
            sm_stats_barrier.arrive_and_wait_w_index(
                index=1 if self.p_in_smem else 1 * 4 + warp_idx
            )
            pipeline_sm_stats.consumer_release_w_index(1)
            if has_stream1:
                sp_corr_count1 += Int32(1)
            ...
            if stream0_count > Int32(1):
                for corr_idx in cutlass.range(stream0_count - Int32(1), unroll=1):
                    ...
                    sp_slot0 = Int32(0) + Int32(2) * (sp_corr_count0 % Int32(2))
                    pipeline_s_p_o.consumer_release_w_index(sp_slot0)
                    sp_corr_count0 += Int32(1)
                    pipeline_sm_stats.consumer_release_w_index(0)
                    ...
                    if corr_idx + Int32(1) < stream1_count:
                        ...
                        sp_slot1 = Int32(1) + Int32(2) * (sp_corr_count1 % Int32(2))
                        pipeline_s_p_o.consumer_release_w_index(sp_slot1)
                        sp_corr_count1 += Int32(1)
                        pipeline_sm_stats.consumer_release_w_index(1)
            ...
            if has_stream0:
                next_sp_slot0 = Int32(0) + Int32(2) * (sp_corr_count0 % Int32(2))
                pipeline_s_p_o.consumer_release_w_index(next_sp_slot0)
            if has_stream1:
                next_sp_slot1 = Int32(1) + Int32(2) * (sp_corr_count1 % Int32(2))
                pipeline_s_p_o.consumer_release_w_index(next_sp_slot1)
            pipeline_o_epi.producer_commit_w_index(0)
            corr_epi_producer_phase ^= 1
    ...
    def correction_merge_epilogue(
        ...
        has_stream1: Boolean,
        sO: cute.Tensor,
        sScale: cute.Tensor | None = None,
    ):
        """Merge two FP32 TMEM numerators and convert/store only once."""
        ...
        tile_layout = cute.make_layout((self.m_block_size, corr_tile_size))
        tOtO0_i = cute.logical_divide(tOtO0, tile_layout)
        tOtO1_i = cute.logical_divide(tOtO1, tile_layout)
        tOcO_i = cute.logical_divide(tOcO, tile_layout)
        tOsO_i = cute.logical_divide(tOsO, tile_layout)
        ...
        if const_expr(self.p_in_smem):
            smem_copy_atom = cute.make_copy_atom(
                cute.nvgpu.CopyUniversalOp(), self.o_dtype
            )
        else:
        ...
        use_per_elem_scale = const_expr(self.p_in_smem) and sScale is not None
        ...
        for i in cutlass.range(
            ...
            if has_stream0:
                cute.copy(
                    tiled_tmem_load, tOtO0_t2r[None, 0, 0, i], tOrO
                )
                if const_expr(use_per_elem_scale):
                    for k in cutlass.range(cute.size(tOrO), unroll_full=True):
                        row_idx = tOcO_t2r_i[k][0]
                        tOrO[k] = tOrO[k] * sScale[row_idx]
                else:
                    for j in cutlass.range(0, cute.size(tOrO), 2, unroll_full=True):
                        tOrO[j], tOrO[j + 1] = cute.arch.mul_packed_f32x2(
                            (tOrO[j], tOrO[j + 1]), (scale0, scale0)
                        )
                if has_stream1:
                    tOrO1 = cute.make_rmem_tensor_like(tOrO, self.pv_acc_dtype)
                    cute.copy(
                        tiled_tmem_load,
                    ...
                    if const_expr(use_per_elem_scale):
                        for k in cutlass.range(cute.size(tOrO), unroll_full=True):
                            row_idx = tOcO_t2r_i[k][0]
                            tOrO[k] = tOrO[k] + tOrO1[k] * sScale[
                                row_idx + self.m_block_size
                            ]
                    else:
                        for j in cutlass.range(0, cute.size(tOrO), 2, unroll_full=True):
                            tOrO[j], tOrO[j + 1] = cute.arch.fma_packed_f32x2(
                                (tOrO1[j], tOrO1[j + 1]),
                                (scale1, scale1),
                                (tOrO[j], tOrO[j + 1]),
                            )
                ...
                cute.copy(
                    tiled_tmem_load, tOtO1_t2r[None, 0, 0, i], tOrO
                )
                if const_expr(use_per_elem_scale):
                    for k in cutlass.range(cute.size(tOrO), unroll_full=True):
                        row_idx = tOcO_t2r_i[k][0]
                        tOrO[k] = tOrO[k] * sScale[
                            row_idx + self.m_block_size
                        ]
                else:
                    for j in cutlass.range(0, cute.size(tOrO), 2, unroll_full=True):
                        tOrO[j], tOrO[j + 1] = cute.arch.mul_packed_f32x2(
                            (tOrO[j], tOrO[j + 1]), (scale1, scale1)
                        )
            copy_utils.cvt_copy(tiled_smem_store, tOrO, tOsO_r2s_i)

        cute.arch.fence_view_async_tmem_load()
        cute.arch.fence_view_async_shared()

    @cute.jit
    def _softmax_tidx_to_row(self, tidx):
        if cutlass.const_expr(self.p_in_smem):
            return (tidx // 32) * 16 + tidx % 16
        return tidx

    @cute.jit
    def correction_rescale_smem_p(
        self,
        thr_mma: cute.ThrMma,
        tOtO: cute.Tensor,
        tidx: Int32,
        ...
        tmem_store_atom = cute.make_copy_atom(
            tcgen05.copy.St16x256bOp(tcgen05.copy.Repetition(2)),
            self.pv_acc_dtype,
        )
        thr_tmem_load = tcgen05.make_tmem_copy(tmem_load_atom, tOtO).get_slice(tidx)
        thr_tmem_store = tcgen05.make_tmem_copy(tmem_store_atom, tOtO).get_slice(tidx)
        tOtO_t2r = thr_tmem_load.partition_S(tOtO)
        tOcO_t2r = thr_tmem_load.partition_D(tOcO)
        tOtO_r2t = thr_tmem_store.partition_D(tOtO)
        # An M64 Ld16x256 fragment spans exactly two unique output rows.
        # Online-softmax correction scales are <= 1, so skip the TMEM
        # round-trip only when every lane's two row scales remain identity.
        row_scale0 = sScale[tOcO_t2r[0][0] + stage * self.m_block_size]
        row_scale1 = sScale[tOcO_t2r[2][0] + stage * self.m_block_size]
        should_rescale = cute.arch.vote_ballot_sync(
            min(row_scale0, row_scale1) < Float32(1.0)
        ) != 0
        if should_rescale:
            tOrO = cute.make_rmem_tensor(tOcO_t2r.shape, self.pv_acc_dtype)
            cute.copy(thr_tmem_load, tOtO_t2r, tOrO)
            cute.arch.fence_view_async_tmem_load()
            for i in cutlass.range(cute.size(tOrO), unroll_full=True):
                row_idx = tOcO_t2r[i][0]
                tOrO[i] = tOrO[i] * sScale[
                    row_idx + stage * self.m_block_size
                ]
            cute.copy(thr_tmem_store, tOrO, tOtO_r2t)
            cute.arch.fence_view_async_tmem_store()
    ...
    def correction_epilogue(
        ...
        for i in cutlass.range(self.head_dim_v_padded // corr_tile_size, unroll_full=True):
            ...
            tOsO_r2s_i = tOsO_s2r[None, 0, 0, i]
            tOcO_t2r_i = tOcO_t2r[None, 0, 0, i]
            tOrO_frg = cute.make_rmem_tensor(tOcO_t2r_i.shape, self.pv_acc_dtype)
            if const_expr(zero_output):
                tOrO_frg.fill(0.0)
            else:
                cute.copy(tiled_tmem_load, tOtO_t2r_i, tOrO_frg)
                if const_expr(use_per_elem_scale):
                    for k in cutlass.range(cute.size(tOrO_frg), unroll_full=True):
                        row_idx = tOcO_t2r_i[k][0]
                        tOrO_frg[k] = (
                            tOrO_frg[k]
                            * sScale[row_idx + stage * self.m_block_size]
                        )
                else:
                    for j in cutlass.range(0, cute.size(tOrO_frg), 2, unroll_full=True):
                        tOrO_frg[j], tOrO_frg[j + 1] = cute.arch.mul_packed_f32x2(
