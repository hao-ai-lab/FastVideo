...
def _flash_attn_fwd(
    ...
    if arch // 10 in [10, 11]:
        sparse_q = get_sparse_q_block_size(block_sparse_tensors, seqlen_q)
        # Dense FA4 amortizes one CTA across two 128-row Q stages.  A sparse
        # mask whose declared Q block is exactly one tile must retain that
        # granularity: combining two rows would union distinct VSA selections.
        # The SM100 kernel already has a q_stage=1 specialization; selecting it
        # here also creates the base schedule for FastVideo's dual-KV-stream
        # single-Q-block path.
        single_q_sparse_tile = use_block_sparsity and sparse_q == tile_m
        q_stage = 1 if p_in_smem or single_q_sparse_tile or seqlen_q_packgqa <= tile_m else 2
    else:
        q_stage = 1
    ...
    dual_stream_enabled = os.environ.get("FASTVIDEO_FA4_VSA_DUAL_STREAM", "1") != "0"
    sp_double_buffer_enabled = (
        os.environ.get("FASTVIDEO_FA4_VSA_SP_DOUBLE_BUFFER", "1") != "0"
    )
    num_kv_streams = (
        2
    ...
    use_sp_double_buffer = (
        sp_double_buffer_enabled
        and num_kv_streams == 2
        and arch == 100
        and tile_m == 128
        and tile_n == 64
            ...
            if qv is not None:
                ...
                kv_stream_kwargs = (
                    ...
                    if use_dedicated_hd256_kernel
                        ...
                        "num_kv_streams": num_kv_streams,
                        "use_sp_double_buffer": use_sp_double_buffer,
                    }
                )
