...
def calc_chunks(cu_seqlen, moba_chunk_size):
    ...
    chunk_sizes = torch.full((num_chunk + 1, ),
                             moba_chunk_size,
                             dtype=torch.int32,
                             device=cu_seqlen.device)
    chunk_sizes[0] = 0
    batch_last_chunk_size = batch_sizes - (batch_num_chunk -
                                           1) * moba_chunk_size
    chunk_sizes[cu_num_chunk[1:]] = batch_last_chunk_size
    cu_chunk = chunk_sizes.cumsum(dim=-1, dtype=torch.int32)
    chunk_to_batch = torch.zeros((num_chunk, ),
                                 dtype=torch.int32,
                                 device=cu_seqlen.device)
    ...
    filtered_chunk_indices = torch.arange(num_chunk,
                                          device=cu_seqlen.device,
                                          dtype=torch.int32)
...
def _select_threshold_query_head(gate: torch.Tensor,
    ...
    gate_masked = torch.where(valid_gate_mask, gate,
                              -torch.inf)  # Use -inf for max
    gate_min_val = torch.where(valid_gate_mask, gate,
                               torch.inf)  # Use +inf for min

    row_min = gate_min_val.amin(dim=0)  # (H, S)
    row_max = gate_masked.amax(dim=0)  # (H, S)
    denom = row_max - row_min
    denom = torch.where(denom <= eps, torch.ones_like(denom),
                        denom)  # avoid divide‑by‑zero
    ...
    remain_ratio = simsum_threshold - self_norm / (total_norm_sum + eps
                                                   )  # (H, S)
    remain_ratio = torch.clamp(remain_ratio,
                               min=0.0)  # if already ≥ thresh, no extra needed
    ...
    sorted_norm, sorted_idx = torch.sort(others_norm, descending=True,
                                         dim=0)  # (C, H, S)
    ...
    cutoff = torch.where(any_cond,
                         cond.float().argmax(dim=0),
                         torch.full_like(any_cond, fill_value=C - 1))  # (H, S)
...
def _select_threshold_block(gate: torch.Tensor, valid_gate_mask: torch.Tensor,
                            gate_self_chunk_mask: torch.Tensor,
                            simsum_threshold: float) -> torch.Tensor:
    """
    ...
    block_denom = torch.where(block_denom <= eps, torch.ones_like(block_denom),
                              block_denom)  # (C, 1, 1)
    ...
    remain_ratio = simsum_threshold - self_norm_sum_per_block / (
        total_norm_sum_per_block + eps)  # (C,)
    ...
    sorted_others_flat, sorted_indices_flat = torch.sort(
        others_flat, dim=1, descending=True)  # (C, H*S)
    ...
    ratio_flat = cumsum_others_flat / (
        total_norm_sum_per_block.unsqueeze(1) + eps)  # (C, H*S)
...
def _select_threshold_overall(gate: torch.Tensor, valid_gate_mask: torch.Tensor,
                              gate_self_chunk_mask: torch.Tensor,
                              simsum_threshold: float) -> torch.Tensor:
    """
    ...
    overall_denom = torch.where(
        overall_denom <= eps,
        torch.tensor(1.0, device=gate.device, dtype=gate.dtype), overall_denom)
    ...
    remain_ratio = simsum_threshold - self_norm_sum_overall / (
        total_norm_sum_overall + eps)  # scalar
    ...
    valid_others_mask_flat = valid_gate_mask.flatten(
    ) & ~gate_self_chunk_mask.flatten()  # Mask for valid, non-self entries
    ...
    sorted_others_values, sort_perm = torch.sort(
        valid_others_values, descending=True)  # (N_valid_others,)
    sorted_original_indices = valid_others_indices[
        sort_perm]  # Original indices in C*H*S space, sorted by value

    # 5) cumulative‑sum the sorted valid 'other' norms globally
    cumsum_others_values = sorted_others_values.cumsum(
        dim=0)  # (N_valid_others,)

    # 6) find the smallest k where cumsum_ratio ≥ remain_ratio globally
    ratio_values = cumsum_others_values / (total_norm_sum_overall + eps
                                           )  # (N_valid_others,)
    ...
    cutoff_idx_in_sorted = torch.where(
        ...
        torch.tensor(len(sorted_others_values) - 1,
                     device=gate.device,
                     dtype=torch.long))
    ...
    others_mask_flat = torch.zeros_like(others_flat,
                                        dtype=torch.bool)  # (C*H*S,)
    if selected_other_indices.numel(
    ) > 0:  # Check if any 'other' indices were selected
...
def _select_threshold_head_global(gate: torch.Tensor,
                                  valid_gate_mask: torch.Tensor,
                                  gate_self_chunk_mask: torch.Tensor,
                                  simsum_threshold: float) -> torch.Tensor:
    """
    ...
    remain_ratio = simsum_threshold - self_norm_sum / (total_norm_sum + eps
                                                       )  # (H,)
    ...
    masked_flat = torch.where(valid_flat, others_flat,
                              torch.zeros_like(others_flat))
    sorted_vals, sorted_idx = torch.sort(masked_flat, dim=1,
                                         descending=True)  # (H, C*S)
    ...
    cutoff = torch.where(has_cutoff,
                         cond.float().argmax(dim=1), default)  # (H,)
...
class MixedAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        output = torch.zeros((q.shape[0], q.shape[1], q.shape[2]),
                             device=q.device,
                             dtype=torch.float32)
        output_2d = output.view(-1, q.shape[2])

        max_lse_1d = self_attn_lse_sh.view(-1)
        max_lse_1d = max_lse_1d.index_reduce(0, moba_q_sh_indices,
                                             moba_attn_lse.view(-1), "amax")
        self_attn_lse_sh = self_attn_lse_sh - max_lse_1d.view_as(
            self_attn_lse_sh)
        moba_attn_lse = (moba_attn_lse.view(-1).sub(
            max_lse_1d.index_select(
                0, moba_q_sh_indices)).reshape_as(moba_attn_lse))

        mixed_attn_se_sh = self_attn_lse_sh.exp()
        moba_attn_se = moba_attn_lse.exp()

        mixed_attn_se_sh.view(-1).index_add_(0, moba_q_sh_indices,
                                             moba_attn_se.view(-1))
        ...
        mixed_attn_lse = (mixed_attn_lse_sh.view(-1).index_select(
            0, moba_q_sh_indices).view_as(moba_attn_lse))
        factor = (moba_attn_lse - mixed_attn_lse).exp()  # [S, H]
        moba_attn_out = moba_attn_out * factor.unsqueeze(-1)
        raw_attn_out = moba_attn_out.view(-1, moba_attn_out.shape[-1])
        output_2d.index_add_(0, moba_q_sh_indices, raw_attn_out)
        output = output.to(q.dtype)
        mixed_attn_lse_sh = mixed_attn_lse_sh + max_lse_1d.view_as(
            mixed_attn_se_sh)
    ...
    def backward(ctx, d_output):
        ...
        headdim = q.shape[-1]
        d_moba_output = (d_output.view(-1, headdim).index_select(
            0, moba_q_sh_indices).unsqueeze(1))
        moba_output = (output.view(-1, headdim).index_select(
            0, moba_q_sh_indices).unsqueeze(1))

        mixed_attn_vlse = (mixed_attn_vlse_sh.view(-1).index_select(
            0, moba_q_sh_indices).view(1, -1))

        dmq = torch.empty_like(moba_q)
        dmkv = torch.empty_like(moba_kv)
        _ = _flash_attn_varlen_backward(dout=d_moba_output,
                                        q=moba_q,
                                        k=moba_kv[:, 0],
                                        v=moba_kv[:, 1],
                                        out=moba_output,
...
def moba_attn_varlen(
    ...
    range_tensor = torch.arange(max_chunk_len,
                                device=kv.device,
                                dtype=chunk_starts.dtype).unsqueeze(0)
    ...
    gathered = kv[indices.view(-1)].view(num_filtered_chunk, max_chunk_len,
                                         *kv.shape[1:])
    gathered = gathered * valid_mask.unsqueeze(-1).unsqueeze(-1).unsqueeze(
        -1).type_as(gathered)

    # Compute key_gate_weight over valid tokens.
    key_values = gathered[:, :, 0].float(
    )  # [num_filtered_chunk, max_chunk_len, num_head, head_dim]
    ...
    gate = torch.bmm(key_gate_weight.permute(1, 0, 2),
                     q_float.permute(1, 0, 2).transpose(1, 2)).permute(1, 0, 2)

    # Amplify the diagonal (self chunk) contributions.
    gate_seq_idx = torch.arange(seqlen, device=q.device,
                                dtype=torch.int32).unsqueeze(0).expand(
                                    num_filtered_chunk, seqlen)
    ...
    gate_self_chunk_mask = (
        ...
        (gate_seq_idx < chunk_end.unsqueeze(1))).unsqueeze(1).expand(
            -1, num_head, -1)
    ...
    if select_mode == 'topk':
        ...
        if threshold_type == 'query_head':
            ...
            # gate: (C, H, S)
            _, gate_topk_idx = torch.topk(gate,
                                          k=moba_topk,
                                          dim=0,
                                          largest=True,
                                          sorted=False)
            ...
            vals, idx = torch.topk(flat_gate_masked,
                                   k=moba_topk * H * S,
                                   largest=True,
                                   sorted=False)
            others_mask_flat = torch.zeros_like(flat_mask, dtype=torch.bool)
            others_mask_flat[idx] = True
            gate_mask = (valid_gate_mask.flatten() & others_mask_flat).view(
                gate.shape)
            ...
            flat_gate_masked = torch.where(
                flat_valid, flat_gate, torch.full_like(flat_gate,
                                                       -float('inf')))
            # pick top-k indices per head
            _, topk_idx = torch.topk(flat_gate_masked,
                                     k=moba_topk * S,
                                     dim=1,
                                     largest=True,
                                     sorted=False)
        ...
        if threshold_type == 'query_head':
            gate_mask = _select_threshold_query_head(gate, valid_gate_mask,
                                                     gate_self_chunk_mask,
                                                     simsum_threshold)
        elif threshold_type == 'block':
            gate_mask = _select_threshold_block(gate, valid_gate_mask,
                                                gate_self_chunk_mask,
                                                simsum_threshold)
        elif threshold_type == 'overall':
            gate_mask = _select_threshold_overall(gate, valid_gate_mask,
                                                  gate_self_chunk_mask,
                                                  simsum_threshold)
        elif threshold_type == 'head_global':
            gate_mask = _select_threshold_head_global(gate, valid_gate_mask,
                                                      gate_self_chunk_mask,
                                                      simsum_threshold)
        else:
            raise ValueError(
    ...
    if gate_mask.sum() == 0:
        return flash_attn_varlen_func(q,
                                      k,
                                      v,
                                      cu_seqlens,
                                      cu_seqlens,
    ...
    valid_expert_lengths = chunk_lengths.unsqueeze(1).expand(
        num_filtered_chunk, num_head).reshape(-1).to(torch.int32)
    if q_zero_mask.sum() > 0:
        experts_tensor = experts_tensor[valid_expert_mask]
        valid_expert_lengths = valid_expert_lengths[valid_expert_mask]

    seq_range = torch.arange(experts_tensor.shape[1],
                             device=experts_tensor.device).unsqueeze(0)
...
def process_moba_input(
    ...
    if isinstance(chunk_size, float) or isinstance(chunk_size, int):
        moba_chunk_size = int(chunk_size * patch_resolution[1] *
                              patch_resolution[2])
    else:
        assert isinstance(
        ...
        if len(chunk_size) == 2:
            assert patch_resolution[1] % chunk_size[
                0] == 0 and patch_resolution[2] % chunk_size[
                    1] == 0, f"spatial patch_resolution {patch_resolution[1:]} should be divisible by 2d chunk_size {chunk_size}"
            nch, ncw = patch_resolution[1] // chunk_size[0], patch_resolution[
                2] // chunk_size[1]
            x = rearrange(x,
            ...
            assert patch_resolution[0] % chunk_size[0] == 0 and patch_resolution[
                1] % chunk_size[1] == 0 and patch_resolution[2] % chunk_size[
                    2] == 0, f"patch_resolution {patch_resolution} should be divisible by 3d chunk_size {chunk_size}"
            nct, nch, ncw = patch_resolution[0] // chunk_size[
                0], patch_resolution[1] // chunk_size[1], patch_resolution[
                    2] // chunk_size[2]
            x = rearrange(
                x,
...
def generate_data(batch_size, seqlen, num_head, head_dim, dtype):
    ...
    q = torch.randn((batch_size, seqlen, num_head, head_dim),
                    requires_grad=True).to(dtype=dtype, device='cuda')
    k = torch.randn((batch_size, seqlen, num_head, head_dim),
                    requires_grad=True).to(dtype=dtype, device='cuda')
    v = torch.randn((batch_size, seqlen, num_head, head_dim),
                    requires_grad=True).to(dtype=dtype, device='cuda')
...
def test_attn_varlen_moba_speed(batch,
                                head,
                                seqlen,
                                head_dim,
                                moba_chunk_size,
    ...
    # Get data
    q, k, v, cu_seqlen, max_seqlen = generate_data(batch, seqlen, head,
                                                   head_dim, dtype)
    print(
