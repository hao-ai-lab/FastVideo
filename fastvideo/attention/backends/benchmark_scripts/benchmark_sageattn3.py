...
def benchmark_sageattn3(batch_size, num_heads, seq_len, head_dim, 
    ...
    def run_attention():
        with torch.cuda.stream(torch.cuda.Stream()):
            output = sageattn_blackwell(q, k, v, is_causal=is_causal)
    ...
    # Benchmark
    avg_time_ms = bench(run_attention, num_warmups=0, num_tests=num_tests)
...
def run_benchmark_suite():
    ...
    for config in configs:
        ...
        try:
            ...
            print(f"Error benchmarking configuration {config}: {e}")
            continue
