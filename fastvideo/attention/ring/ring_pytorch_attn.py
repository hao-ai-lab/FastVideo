...
#
# Vendored for FastVideo via yunchang (long-context-attention):
    ...
    def forward(ctx, group, q, k, v, sm_scale, is_causal):
        ...
        for step in range(comm.world_size):
            ...
            if not is_causal or step <= comm.rank:
                block_out, block_lse = pytorch_attn_forward(q,
                                                            k,
                                                            v,
    ...
    def backward(ctx, dout, *args):
        ...
        for step in range(kv_comm.world_size):
            ...
            if step <= kv_comm.rank or not is_causal:
                ...
                block_dq_buffer, block_dk_buffer, block_dv_buffer = pytorch_attn_backward(dout,
                                                                                          q,
                                                                                          k,
                                                                                          v,
                                                                                          out,
