...
class SeqAllToAll4D(torch.autograd.Function):
    ...
    def backward(ctx: Any, *grad_output:
                 Tensor) -> tuple[None, Tensor, None, None]:
        return (
            None,
            SeqAllToAll4D.apply(ctx.group, *grad_output, ctx.gather_idx,
                                ctx.scatter_idx, ctx.use_sync),
            None,
            None,
            None,
        )
...
def all_to_all_5D(input: torch.tensor,
                  scatter_idx: int = 3,
                  gather_idx: int = 1,
                  group=None,
                  use_sync: bool = False) -> torch.tensor:
    ...
    if scatter_idx == 3 and gather_idx == 1:
        ...
        input_t = (input.reshape(bs, shard_seqlen, 3, seq_world_size, shard_hc,
                                 hs).transpose(0, 3).contiguous())
        ...
        input_t = (input.reshape(bs, seq_world_size, shard_seqlen,
                                 3, shard_hc, hs).transpose(0, 4).transpose(
                                     0, 1).contiguous().reshape(
                                         seq_world_size, shard_hc, shard_seqlen,
                                         3, bs, hs))
        ...
        raise RuntimeError(
            "scatter_idx must be 1 or 3 and gather_idx must be 1 or 3")
...
class SeqAllToAll5D(torch.autograd.Function):
    ...
    def backward(ctx: Any, *grad_output:
        ...
        return (
            None,
            SeqAllToAll5D.apply(ctx.group, *grad_output, ctx.gather_idx,
                                ctx.scatter_idx, ctx.use_sync),
            None,
            None,
            None,
