...
**核心训练逻辑（单步）**
1) `adapter.prepare_batch(...)` 产出 `TrainingBatch`（包含 `x0` video latents + conditioning）。
2) 采样 `t_inhom`：
   - 先采样每个 chunk 的 timestep index（`method_config.chunk_size` 控制 chunk 划分）
   - 再 repeat 到每帧（`[B, T_lat]`）
3) 采样 `noise ~ N(0, I)`，得到 `x_t = adapter.add_noise(x0, noise, t_inhom_flat)`
4) 学生预测 `pred = adapter.predict_noise(student, x_t, t_inhom, batch, ...)`
5) loss：
...
- `method_config`（DFSFT 专属）
  ...
  - `attn_kind: dense|vsa`：选择 adapter 的 dense/VSA attention metadata 路径
...
- 当前 validator 仍是 “full-video pipeline” 语义；真正的 streaming/causal rollout
 仍需要在后续阶段实现（避免把 rollout policy 藏进 validator/adapter）。
