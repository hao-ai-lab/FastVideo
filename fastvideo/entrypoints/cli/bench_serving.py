...
class VBenchDataset(BaseDataset):
    ...
    def _load_data(self) -> list[dict[str, Any]]:
        if self.args.task_name in ("text-to-video", "text-to-image", "video-to-video"):
            ...
            raise ValueError(f"Illegal task name is found in VBenchDataset {self.args.task_name}")
        ...
        if not path:
            ...
            if not os.path.exists(path):
                ...
                try:
                    ...
                    logger.info("Failed to download VBench prompts: %s", e)
    ...
    def _auto_download_i2v_dataset(self) -> str | None:
        ...
        try:
            ...
            logger.info("https://github.com/Vchitect/VBench/tree/master/vbench2_beta_i2v#22-download")
            return None
    ...
    def _load_from_i2v_json(self, json_path: str) -> list[dict[str, Any]]:
        ...
        logger.info("Loaded %d I2V samples from VBench I2V dataset", len(data))
        return data
    ...
    def _create_dummy_data(self) -> list[dict[str, Any]]:
        ...
        if not os.path.exists(dummy_image):
            try:
                ...
                logger.info("Created dummy image at %s", dummy_image)
            except ImportError:
        ...
        for json_path in info_json_candidates:
            if json_path and os.path.exists(json_path):
                try:
                    ...
                    logger.info("Failed to load %s: %s", json_path, e)
...
async def async_request_video_sglang(
    ...
    if input.image_paths and len(input.image_paths) > 0:
        ...
        # Add extra body fields to form data if possible
        if input.extra_body:
...
def calculate_metrics(outputs: list[RequestFuncOutput], total_duration: float):
    ...
    peak_memories = [o.peak_memory_mb for o in success_outputs if o.peak_memory_mb is not None and o.peak_memory_mb > 0]
...
async def benchmark(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        logger.info("Failed to fetch model info: %s. Using default: %s", e, args.model)
    ...
    logger.info("Prepared %d requests from %s dataset.", len(requests_list), args.dataset)

    # Limit concurrency
    semaphore = asyncio.Semaphore(args.max_concurrency) if args.max_concurrency is not None else None
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="DEPRECATED: --backend is deprecated and will be ignored.",
    )
    parser.add_argument(
        "--base-url",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Maximum number of concurrent requests, default to `1`.",
    )
    parser.add_argument(
        "--request-rate",
        type=float,
