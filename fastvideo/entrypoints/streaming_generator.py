...
class IncrementalVideoWriter:
    ...
    def __init__(
        ...
        self._writer = imageio.get_writer(path, fps=fps, format="mp4", quality=self._quality)
    ...
    def _write_block(self, frames: list[np.ndarray], path: str) -> str:
        imageio.mimsave(path, frames, fps=self._fps, format="mp4", quality=self._quality)
        return path
...
class StreamingVideoGenerator(VideoGenerator):
    ...
    def reset(
        ...
        if "output_path" in kwargs:
            ...
            self.writer = IncrementalVideoWriter(output_path, fps=fps, block_dir=block_dir, quality=quality)
    ...
    def finalize(self, output_path: str = "streaming_output.mp4", fps: int = 24) -> str:
        ...
        if self.writer:
            ...
            imageio.mimsave(output_path, self.accumulated_frames, fps=fps, format="mp4", quality=8)
            logger.info("Saved video to %s", output_path)
    ...
    def _process_output_batch(self, output_batch: ForwardBatch) -> list[np.ndarray]:
        ...
        pad_for_writer = (self.writer is not None and target_h is not None and target_w is not None)
        for x in videos:
