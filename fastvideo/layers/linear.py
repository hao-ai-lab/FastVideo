...
from fastvideo.distributed import (divide, get_tensor_model_parallel_rank, get_tensor_model_parallel_world_size,
...
WEIGHT_LOADER_V2_SUPPORTED = [
    "CompressedTensorsLinearMethod", "AWQMarlinLinearMethod", "AWQLinearMethod", "GPTQMarlinLinearMethod",
    "Fp8LinearMethod", "MarlinLinearMethod", "QQQLinearMethod", "GPTQMarlin24LinearMethod", "TPUInt8LinearMethod",
    "GPTQLinearMethod", "FBGEMMFp8LinearMethod", "ModelOptFp8LinearMethod", "IPEXAWQLinearMethod",
...
def adjust_bitsandbytes_4bit_shard(param: Parameter, shard_offsets: dict[str, tuple[int, int]],
...
class LinearBase(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, Optional[Parameter]]:
        raise NotImplementedError
            ...
            for shard_id, shard_offset, shard_size in shard_offsets:
                ...
                if use_bitsandbytes_4bit:
                    orig_qkv_offsets = {
                        ...
                        "v": ((self.total_num_heads + self.total_num_kv_heads) * self.head_size,
                              self.total_num_kv_heads * self.head_size),
                        "total": ((self.total_num_heads + 2 * self.total_num_kv_heads) * self.head_size, 0)
                    }
        ...
        if output_dim is not None:
            ...
            if use_bitsandbytes_4bit:
                orig_qkv_offsets = {
                    ...
                    "total": ((self.num_heads + 2 * self.num_kv_heads) * self.head_size, 0)
                }
