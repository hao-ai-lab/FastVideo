...
from vllm.sequence import (VLLM_INVALID_TOKEN_ID, CompletionSequenceGroupOutput, Logprob, PromptLogprobs,
                           SampleLogprobs, SequenceOutput)
...
class SamplerOutput(
    ...
    def __repr__(self) -> str:
        ...
        """
        sampled_token_probs_repr = ("None" if self.sampled_token_probs is None else self.sampled_token_probs.shape)
        sampled_token_ids_repr = ("None" if self.sampled_token_ids is None else self.sampled_token_ids.shape)
...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        * Encapsulate arguments required for deferred Pythonization
        ...
        if do_penalties:
            logits = apply_penalties(logits, sampling_tensors.prompt_tokens, sampling_tensors.output_tokens,
        ...
        return _build_sampler_output(maybe_deferred_sample_results,
                                     sampling_metadata,
                                     prompt_logprobs,
                                     sample_logprobs,
    ...
    for seq_group in sampling_metadata.seq_groups:
        ...
        if min_tokens > 0 and token_ids_to_penalize:
            ...
            if seqs_to_penalize:
                ...
                logits_to_penalize.extend(itertools.product(seqs_to_penalize, token_ids_to_penalize))
...
def _multinomial(
    ...
    if seq_groups is None:
        ...
        for seq_group in seq_groups:
            ...
            q[sample_idx:sample_idx + stride].exponential_(generator=seq_group.generator)
            sample_idx += stride
    return probs.div_(q).argmax(dim=1).view(-1, num_samples)


def _top_k_top_p_multinomial_with_flashinfer(probs: torch.Tensor, top_ks: torch.Tensor, top_ps: torch.Tensor,
    ...
    if seq_groups is None:
        ...
        for seq_group in seq_groups:
            ...
            stride = len(seq_ids) * num_samples
            assert seq_group.generator is not None
            uniform_samples[:, sample_idx:sample_idx + stride].uniform_(generator=seq_group.generator)
            sample_idx += stride
    ...
    if not success.all():
        ...
        batch_next_token_ids = flashinfer.sampling.sampling_from_probs(probs, uniform_samples[0])
    return batch_next_token_ids.view(-1, num_samples)


def get_pythonized_sample_results(sample_result_args: SampleResultArgsType) -> SampleResultType:
    ...
    for sampling_type in SamplingType:
        ...
        if sampling_type == SamplingType.GREEDY:
            ...
            sample_results = _random_sample(seq_groups, multinomial_samples[sampling_type])
        sample_results_dict.update(zip(seq_group_id, sample_results))

    return [sample_results_dict.get(i, ([], [])) for i in range(len(sampling_metadata.seq_groups))]
...
def _sample_with_torch(
    ...
    * Defer Pythonization & preserve GPU-side
    ...
    categorized_seq_group_ids: Dict[SamplingType, List[int]] = {t: [] for t in SamplingType}
    ...
    for sampling_type in SamplingType:
        ...
        if sampling_type == SamplingType.GREEDY:
            ...
            if modify_greedy_probs:
                ...
                # token id.
                _modify_greedy_probs_inplace(logprobs, probs, long_sample_indices, greedy_samples)
            ...
            seq_groups_arg = (None if sampling_type == SamplingType.RANDOM else seq_groups)

            if flashinfer_top_k_top_p_sampling is not None:
                multinomial_samples[sampling_type] = _top_k_top_p_multinomial_with_flashinfer(
                ...
                multinomial_samples[sampling_type] = _multinomial(probs[long_sample_indices],
                                                                  max_n_in_batch,
    ...
    maybe_deferred_args = SampleResultArgsType(sampling_metadata=sampling_metadata,
    ...
    if not sampling_metadata.skip_sampler_cpu_output:
        ...
        return get_pythonized_sample_results(maybe_deferred_args), sampled_token_ids_tensor
    else:
...
def get_logprobs(
    ...
    for seq_group, sample_result in zip(sampling_metadata.seq_groups, sample_results):
        (prompt_logprobs, top_logprob_idx,
         selected_logprobs_idx) = _get_prompt_logprob_if_needed(seq_group, selected_logprobs, ranks, top_token_ids,
        ...
        (sampled_logprobs, top_logprob_idx,
         selected_logprobs_idx) = _get_sampled_logprob_if_needed(seq_group, sample_result, selected_logprobs, ranks,
...
def _get_prompt_logprob_if_needed(
    ...
    if is_prompt and sampling_params.prompt_logprobs is not None:
        ...
        # repetitively.
        selected_logprob_items = selected_logprobs[selected_logprobs_idx:selected_logprobs_idx +
                                                   len(next_prompt_tokens)].tolist()
        rank_items = ranks[selected_logprobs_idx:selected_logprobs_idx + len(next_prompt_tokens)].tolist()
...
def _get_sampled_logprob_if_needed(
    ...
    if seq_group.do_sample:
        ...
        if num_logprobs is None:
            ...
            rank_items = ranks[selected_logprobs_idx:selected_logprobs_idx + len(next_token_ids)].tolist()
            for idx, (next_token_id, parent_id) in enumerate(zip(next_token_ids, parent_seq_ids)):
                # Get the logprob of a sampled token.
                sampled_logprobs_dict = {next_token_id: (selected_logprob_items[idx], rank_items[idx])}
                if num_logprobs is not None and num_logprobs > 0:
                    # Get top K logprobs.
                    top_ids = top_token_ids[top_logprob_idx + parent_id, :num_logprobs].tolist()
                    top_probs = top_logprobs[top_logprob_idx + parent_id, :num_logprobs].tolist()
                    # Top K is already sorted by rank, so we can use 1 ~
                    # num_logprobs + 1 for rank.
                    top_ranks = range(1, num_logprobs + 1)
...
def _modify_greedy_probs_inplace(logprobs: torch.Tensor, probs: torch.Tensor, sample_indices: torch.Tensor,
...
def _build_sampler_output(
    ...
    if skip_sampler_cpu_output:
        ...
        for (seq_group, sample_result, group_prompt_logprobs,
             group_sample_logprobs) in zip(sampling_metadata.seq_groups, maybe_deferred_sample_results, prompt_logprobs,
                                           sample_logprobs):
            seq_ids = seq_group.seq_ids
...
def _get_next_prompt_tokens(seq_group: SequenceGroupToSample) -> List[int]:
    ...
    """
    assert seq_group.is_prompt, ("Caller should ensure the sequence group is in a prefill stage.")
    seq_ids = seq_group.seq_ids
