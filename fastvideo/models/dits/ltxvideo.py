...
class FastVideoFeedForward(nn.Module):
    def __init__(
        ...
        # TODO: add parallellism: ColumnParallelLinear/RowParallelLinear
...
class FastVideoLTXTransformerBlock(nn.Module):
    ...
    TODO: description of LTX specific changes
    """
    ...
    def forward(
        ...
        print(f"DEBUG: encoder_attention_mask shape before attn2: {encoder_attention_mask.shape if encoder_attention_mask is not None else None}")
        ...
        **kwargs
        ...
        if config is not None:
            ...
            if hasattr(config, 'arch_config'):
                ...
                self.in_channels = getattr(arch_config, 'in_channels', 128)
                self.out_channels = getattr(arch_config, 'out_channels', 128)
                self.patch_size = getattr(arch_config, 'patch_size', 1)
                self.patch_size_t = getattr(arch_config, 'patch_size_t', 1)
                self.num_attention_heads = getattr(arch_config, 'num_attention_heads', 32)
                self.attention_head_dim = getattr(arch_config, 'attention_head_dim', 64)
                self.cross_attention_dim = getattr(arch_config, 'cross_attention_dim', 2048)
                self.num_layers = getattr(arch_config, 'num_layers', 28)
                self.activation_fn = getattr(arch_config, 'activation_fn', 'gelu-approximate')
                self.qk_norm = getattr(arch_config, 'qk_norm', 'rms_norm_across_heads')
                self.norm_elementwise_affine = getattr(arch_config, 'norm_elementwise_affine', False)
                self.norm_eps = getattr(arch_config, 'norm_eps', 1e-6)
                self.caption_channels = getattr(arch_config, 'caption_channels', 4096)
                self.attention_bias = getattr(arch_config, 'attention_bias', True)
                self.attention_out_bias = getattr(arch_config, 'attention_out_bias', True)
            else:
                # Try to get from hf_config if provided
                if hf_config:
                    self.in_channels = hf_config.get('in_channels', 128)
                    self.out_channels = hf_config.get('out_channels', 128)
                    self.patch_size = hf_config.get('patch_size', 1)
                    self.patch_size_t = hf_config.get('patch_size_t', 1)
                    self.num_attention_heads = hf_config.get('num_attention_heads', 32)
                    self.attention_head_dim = hf_config.get('attention_head_dim', 64)
                    self.cross_attention_dim = hf_config.get('cross_attention_dim', 2048)
                    self.self.num_layers = hf_config.get('num_layers', 28)
                    self.activation_fn = hf_config.get('activation_fn', 'gelu-approximate')
                    self.qk_norm = hf_config.get('qk_norm', 'rms_norm_across_heads')
                    self.norm_elementwise_affine = hf_config.get('norm_elementwise_affine', False)
                    self.norm_eps = hf_config.get('norm_eps', 1e-6)
                    self.caption_channels = hf_config.get('caption_channels', 4096)
                    self.attention_bias = hf_config.get('attention_bias', True)
                    self.attention_out_bias = hf_config.get('attention_out_bias', True)
                else:
                    # Default values
        ...
        #can probably remove and replace all below with self.inner_dim
        ...
        #might be able to remove this if archconfig post init works
        ...
        # Caption projection
        self.caption_projection = PixArtAlphaTextProjection(in_features=self.caption_channels, hidden_size=self.inner_dim)
        ...
        self.norm_out = FP32LayerNorm(self.inner_dim, eps=1e-6, elementwise_affine=False)
