...
class WanGameActionTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # 1. Self-attention
        norm_hidden_states = (self.norm1(hidden_states.float()).unflatten(
...
class WanLingBotTransformer3DModel(BaseDiT):
    ...
    def forward(
        ...
        **kwargs
        ...
        if timestep.dim() == 1:
            timestep = timestep.unsqueeze(1).expand(-1, post_patch_num_frames)
        if timestep.dim() == 2:
        ...
        timestep_proj = timestep_proj.view(batch_size, post_patch_num_frames, 6,
                                           self.hidden_size)
