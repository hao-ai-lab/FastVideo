...
"""Stable Audio Open 1.0 VAE wrapper for daVinci-MagiHuman.

Mirrors the lazy-load pattern of `fastvideo/models/encoders/t5gemma.py`:
the underlying `AutoencoderOobleck` (Diffusers' name for Stable Audio's
VAE) is fetched from `stabilityai/stable-audio-open-1.0/vae/` on first
call, kept under `self._sa_audio_vae_model`, and hidden from the parent
`named_parameters()` traversal so FastVideo's weight loader does not
try to match these weights against the MagiHuman Diffusers repo.

Interface matches what `MagiHumanAudioDecodingStage` needs:

    model = SAAudioVAEModel(config)
    waveform = model.decode(audio_latent)   # (B, channels, samples)
    latent = model.encode(waveform)         # (B, L, decoder_input_channels)
...
class SAAudioVAEModel(nn.Module):
    """Thin lazy-loader around `diffusers.AutoencoderOobleck`."""
