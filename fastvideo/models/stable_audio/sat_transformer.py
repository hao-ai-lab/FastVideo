...
# Vendored from stable-audio-tools for in-repo Stable Audio DiT.
...
from fastvideo.models.stable_audio.sat_utils import compile
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, t):
        device = self.inv_freq.device
        seq_len = t.shape[0] if t.dim() > 0 else t.numel()
...
class Attention(nn.Module):
    ...
    def apply_attn(self, q, k, v, causal = None, flex_attention_block_mask = None, flex_attention_score_mod = None, flash_attn_sliding_window = None):
        ...
        if (flex_attention_compiled is not None
                and (flex_attention_block_mask is not None
                     or flex_attention_score_mod is not None)):
