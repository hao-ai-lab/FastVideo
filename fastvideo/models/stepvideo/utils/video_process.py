...
class VideoProcessor:
    ...
    def crop2standard540p(self, vid_array):
        ...
        if width_center > height_center:  ## horizon mode
            return vid_array[:, height_center - 270:height_center + 270,
                             width_center - 480:width_center + 480]
        elif width_center < height_center:  ## portrait mode
            return vid_array[:, height_center - 480:height_center + 480,
                             width_center - 270:width_center + 270]
        else:
            return vid_array

    def save_imageio_video(self,
        ...
        with imageio.get_writer(output_filename,
                                fps=fps,
                                codec=codec,
                                ffmpeg_params=ffmpeg_params) as vid_writer:
    ...
    def postprocess_video(self,
        ...
        if len(self.name_suffix) == 0:
            video_path = os.path.join(self.save_path,
                                      f"{output_file_name}.{output_type}")
        else:
            video_path = os.path.join(
                self.save_path,
                f"{output_file_name}-{self.name_suffix}.{output_type}")

        video_tensor = torch.cat([t for t in video_tensor], dim=-2)
        video_tensor = (video_tensor.cpu().clamp(-1, 1) + 1) * 127.5
        video_array = video_tensor.clamp(0, 255).to(
            torch.uint8).numpy().transpose(0, 2, 3, 1)
