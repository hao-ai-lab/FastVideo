...
def get_diffusion_perf_log_dir() -> str:
    ...
    """
    log_dir = os.environ.get("FASTVIDEO_PERF_LOG_DIR")
    if log_dir:
        return os.path.abspath(log_dir)
    if log_dir is None:
        fastvideo_path = Path(fastvideo.__file__).resolve()
        target_path = (fastvideo_path.parent.parent / ".cache" / "logs").resolve()
...
def get_git_commit_hash() -> str:
    try:
        commit_hash = os.environ.get("FASTVIDEO_GIT_COMMIT")
        if not commit_hash:
            commit_hash = (
...
class StageProfiler:
    """
    A unified context manager, records timing information (usually of a single Stage or a step) into a provided
    RequestTimings object (usually from a Req).
    """
    ...
    def __init__(
        ...
        self.logger = logger
        self.start_time = 0.0
        self.log_timing = perf_dump_path_provided or getattr(
            envs, "FASTVIDEO_DIFFUSION_STAGE_LOGGING",
            envs.FASTVIDEO_STAGE_LOGGING)
    ...
    def __enter__(self):
        ...
        if (self.log_timing and self.timings) or self.log_stage_start_end:
            if (os.environ.get("FASTVIDEO_DIFFUSION_SYNC_STAGE_PROFILING",
                               "0") == "1"
...
class PerformanceLogger:
    ...
    def log_request_summary(
        ...
        Note that this accords to the time spent internally in server, postprocess
        is not included.
        """
        formatted_stages = [
            {
        ...
        try:
            ...
            print(f"WARNING: Failed to log performance record: {e}",
                  file=sys.stderr)
