    ...
    def _denoise_t2i(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                attn_metadata = (
                    SDPAMetadata(current_timestep=i, attn_mask=attention_mask_kv)
    ...
    def _denoise_i2i(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with (
                    torch.no_grad(),
                    set_forward_context(current_timestep=i, attn_metadata=cond_meta, forward_batch=batch),
                ):
                    noise_pred = self.transformer(
                        latent_model_input,
                        pos_embeds,
                ...
                if do_cfg:
                    uncond_meta = (
                        SDPAMetadata(current_timestep=i, attn_mask=uncond_mask) if uncond_mask is not None else None
                    )
                    with (
                        torch.no_grad(),
                        set_forward_context(current_timestep=i, attn_metadata=uncond_meta, forward_batch=batch),
                    ):
