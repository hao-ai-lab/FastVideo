...
from fastvideo.pipelines.basic.minimax_h3.types import MiniMaxH3Layout
...
def randn_tensor(
    ...
    generator_device = generator[0].device if isinstance(generator, list) else (generator.device if generator else None)
    if generator_device is not None and generator_device.type != target_device.type and generator_device.type == "cuda":
        raise ValueError(f"Cannot generate a {target_device} tensor from a CUDA generator.")
    if isinstance(generator, list):
        random_device = torch.device("cpu") if generator[0].device.type == "cpu" else target_device
        samples = [
            torch.randn((1, *shape[1:]), generator=generator[index], device=random_device, dtype=dtype)
            for index in range(shape[0])
        ]
        return torch.cat(samples).to(target_device)
    random_device = torch.device(
        "cpu") if generator_device is not None and generator_device.type == "cpu" else target_device
    return torch.randn(shape, generator=generator, device=random_device, dtype=dtype).to(target_device)
...
def _temporal_position_span(num_latent_frames: int) -> float:
    ...
    for index, frames in enumerate(_ROPE_FRAMES_PER_LATENT):
        spans[index::len(_ROPE_FRAMES_PER_LATENT)] *= frames
