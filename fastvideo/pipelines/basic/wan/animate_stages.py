...
v1 scope: a single 77-frame segment. Multi-segment chaining (``refert_num``
temporal guidance) is pipeline-loop work on top of these same stages.
"""
import PIL.Image
import torch
...
def _frames_to_tensor(frames: list[PIL.Image.Image], height: int, width: int,
    ...
    planes = []
    for frame in frames:
        frame = frame.convert("L") if grayscale else frame.convert("RGB")
        frame = resize(frame, height, width)
        tensor = numpy_to_pt(pil_to_numpy(frame))  # [1, C, H, W] in [0, 1]
        planes.append(tensor if grayscale else normalize(tensor))
    return torch.stack(planes, dim=2)  # [1, C, T, H, W]
...
class AnimateLatentPreparationStage(LatentPreparationStage):
    ...
    def forward(self, batch: ForwardBatch, fastvideo_args: FastVideoArgs) -> ForwardBatch:
        ...
        # exactly one extra latent frame, without copying the parent's body.
...
class AnimateConditioningLatentsStage(ImageVAEEncodingStage):
    ...
    def forward(self, batch: ForwardBatch, fastvideo_args: FastVideoArgs) -> ForwardBatch:
        ...
        # First segment: no temporal-guidance frames yet, so mask_len=0 (the
    ...
    def verify_output(self, batch: ForwardBatch, fastvideo_args: FastVideoArgs) -> VerificationResult:
        result = VerificationResult()
        # 4 mask channels + 16 latent channels, T_lat + 1 frames (ref slot first).
        result.add_check(
            "image_latent", batch.image_latent, lambda v: v is not None and v.dim() == 5 and v.shape[1] == 20 and
            batch.num_frames is not None and v.shape[2] == (batch.num_frames - 1) // 4 + 2)
        return result
...
class AnimatePoseVideoEncodingStage(ImageVAEEncodingStage):
    ...
    def verify_output(self, batch: ForwardBatch, fastvideo_args: FastVideoArgs) -> VerificationResult:
        result = VerificationResult()
        result.add_check(
            "pose_latents", batch.pose_latents, lambda v: v is not None and v.dim() == 5 and batch.num_frames is
            not None and v.shape[2] == (batch.num_frames - 1) // 4 + 1)
        return result
...
class AnimateFaceVideoStage(PipelineStage):
    ...
    def verify_output(self, batch: ForwardBatch, fastvideo_args: FastVideoArgs) -> VerificationResult:
        ...
        result.add_check(
            ...
            lambda v: v is not None and v.dim() == 5 and v.shape[1] == 3 and v.shape[2] == batch.num_frames)
        return result
