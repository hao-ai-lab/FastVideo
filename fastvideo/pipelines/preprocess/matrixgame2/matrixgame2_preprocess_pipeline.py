...
class PreprocessPipeline_MatrixGame2(BasePreprocessPipeline):
    ...
    def get_extra_features(self, valid_data: dict[str, Any], fastvideo_args: FastVideoArgs) -> dict[str, Any]:
        ...
        if hasattr(self.get_module("vae"), "shift_factor") and self.get_module("vae").shift_factor is not None:
                    ...
                    if isinstance(action_data, np.ndarray) and action_data.dtype == np.dtype("O"):
                        ...
                        if "mouse" in action_dict:
                            ...
                            assert ms_length >= num_frames, (
                                f"mouse length {ms_length} is smaller than num_frames {num_frames} for {action_path}."
                            )
                            mouse_cond_list.append(action_dict["mouse"][:num_frames])
                    else:
                        kb_length = len(action_data)
                        assert kb_length >= num_frames, (
                            f"keyboard length {kb_length} is smaller than num_frames {num_frames} for {action_path}."
                        )
