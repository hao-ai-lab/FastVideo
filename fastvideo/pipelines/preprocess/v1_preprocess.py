...
from fastvideo.pipelines.preprocess.hyworld.hyworld_preprocess_pipeline import (
    PreprocessPipeline_HYWorld)
...
def main(args) -> None:
    ...
    if args.preprocess_task == "text_only":
    ...
    elif args.preprocess_task == "hyworld":
        # HYWorld uses HunyuanVideo VAE - don't override vae_config
        # Just ensure encoder is loaded for preprocessing
        kwargs = {
            "vae_precision": "fp32",
        }
        ...
        # Full config for video/image processing (WAN-based models)
        kwargs = {
            "vae_precision": "fp32",
    ...
    if args.preprocess_task == "t2v":
        ...
        PreprocessPipeline = PreprocessPipeline_HYWorld
    else:
        raise ValueError(
            f"Invalid preprocess task: {args.preprocess_task}. "
            f"Valid options: t2v, i2v, ode_trajectory, text_only, matrixgame, hyworld")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="t2v",
        choices=["t2v", "i2v", "text_only", "ode_trajectory", "matrixgame", "hyworld"],
