...
class CausalDenoisingStage(DenoisingStage):
    """
    Causal denoising stage.

    This stage performs block-wise temporal denoising to approximate causal inference behavior
    similar to Self-Forcing/pipeline/causal_inference.py while using the standard FastVideo
    transformer interface and scheduler.

    Notes:
    - Uses the same guidance and attention metadata flow as DenoisingStage
    - Processes frames in blocks of size `num_frame_per_block` (defaults to 1)
    - Optionally supports an independent first frame if provided via args
    """
...
class CausalDMDDenosingStage(DenoisingStage):
    ...
    def __init__(self, transformer, scheduler) -> None:
        ...
        # Use FM Euler scheduler for DMD conversion utilities
    ...
    def forward(
        ...
        if not independent_first_frame or (independent_first_frame and batch.image_latent is not None):
            if t % num_frame_per_block != 0:
                raise ValueError("num_frames must be divisible by num_frame_per_block for causal DMD denoising")
            num_blocks = t // num_frame_per_block
            block_sizes = [num_frame_per_block] * num_blocks
            start_index = 0
        else:
            if (t - 1) % num_frame_per_block != 0:
                raise ValueError("(num_frames - 1) must be divisible by num_frame_per_block when independent_first_frame=True")
            num_blocks = (t - 1) // num_frame_per_block
            block_sizes = [1] + [num_frame_per_block] * num_blocks
            start_index = 0
                    ...
                    if i < len(timesteps) - 1:
                        ...
                        noise = torch.randn(video_raw_latent_shape,
                                            ...
                                            generator=(batch.generator[0]
                                                       if isinstance(batch.generator, list) else batch.generator)).to(
                                                self.device)
                        noise_btchw = noise
                        current_latents_btchw = self.scheduler.add_noise(
                            pred_video_btchw.flatten(0, 1), noise_btchw.flatten(0, 1),
                            next_timestep).unflatten(0, pred_video_btchw.shape[:2])
                        current_latents = current_latents_btchw.permute(0, 2, 1, 3, 4)
                    else:
