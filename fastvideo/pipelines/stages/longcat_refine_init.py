    ...
    def forward(
        ...
        if in_memory_stage1 is not None:
            ...
            if len(in_memory_stage1) == 0:
                raise ValueError("stage1_video is empty; expected a non-empty list of frames")
            ...
            if isinstance(in_memory_stage1[0], Image.Image):
                ...
                pil_images = [Image.fromarray(np.array(frame)) for frame in in_memory_stage1]

            logger.info("Initializing LongCat refinement from in-memory stage1_video (%s frames)", len(pil_images))
        else:
            ...
            logger.info("Loaded stage1 video: %s frames @ %s fps", len(pil_images), original_fps)
