...
class SD35LatentPreparationStage(PipelineStage):
    ...
    def forward(self, batch: ForwardBatch,
        ...
        in_channels = int(fastvideo_args.pipeline_config.dit_config.arch_config.in_channels)
...
class SD35DenoisingStage(PipelineStage):
    ...
    def forward(self, batch: ForwardBatch,
        ...
        extra_step_kwargs = self._prepare_extra_func_kwargs(
            self.scheduler.step, {"generator": batch.generator[0] if isinstance(batch.generator, list) else batch.generator}
        )
...
class SD35DecodingStage(PipelineStage):
    ...
    def _denormalize_latents(latents: torch.Tensor, vae) -> torch.Tensor:
        ...
        if sf is None and hasattr(vae, "scaling_factor"):
            sf = getattr(vae, "scaling_factor")
        if sh is None and hasattr(vae, "shift_factor"):
            sh = getattr(vae, "shift_factor")

        if sf is not None:
            latents = latents / (
                sf.to(latents.device, latents.dtype) if isinstance(sf, torch.Tensor) else sf
            )
            if sh is not None:
                latents = latents + (
                    sh.to(latents.device, latents.dtype)
