...
def generate_video_and_latent(pipe, prompt, height, width, num_frames, num_inference_steps, guidance_scale):
    ...
    video, latent, prompt_embed = pipe(
        prompt=prompt,
        height=height,
        width=width,
    ...
    return video[0], latent[0], prompt_embed[0]
...
if __name__ == "__main__":
    ...
    for i, prompt in enumerate(prompt_list):
        video, latent, prompt_embed = generate_video_and_latent(pipe, prompt, args.height, args.width, args.num_frames, args.num_inference_steps, args.guidance_scale)
        # save latent
