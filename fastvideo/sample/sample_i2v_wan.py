...
from fastvideo.models.wan import WanI2V
...
def generate(args):
    ...
    assert "i2v" in args.task, f"Unsupport task: {args.task}"
    ...
    main_print(f"Input image: {args.image}")
    ...
    main_print("Creating WanI2V pipeline.")
    ...
    if rank == 0:
        if args.save_file is None:
            ...
            args.save_file = f"{args.task}_{args.size.replace('*','x') if sys.platform=='win32' else args.size}_{world_size}_{formatted_prompt}_{formatted_time}" + suffix
