...
def test_fullattn_identity_and_preset(identity, monkeypatch):
    monkeypatch.setenv("FASTVIDEO_ATTENTION_BACKEND", "VIDEO_SPARSE_ATTN")
    info = registry._get_config_info(identity)
    ...
    args = FastVideoArgs.from_kwargs(model_path=identity, workload_type="t2v", attention_backend="TORCH_SDPA")
    ...
    assert SamplingParam.from_pretrained(identity) == SamplingParam.from_pretrained(FULL)


@pytest.mark.parametrize("conflict", ["i2v", "VIDEO_SPARSE_ATTN"])
def test_fullattn_rejects_before_loader(identity, conflict, monkeypatch):
    ...
    with pytest.raises(ValueError, match="FullAttn.*" + conflict):
        args = FastVideoArgs.from_kwargs(
            model_path=identity, workload_type="i2v" if conflict == "i2v" else "t2v",
...
def assert_construction(model, expected):
...
def test_recorded_component_constructs_consistently(construction, monkeypatch, requested, environment, expected):
    config = tiny(WanVideoConfig())
    ...
    assert_construction(model, expected)


def test_interleaved_components_do_not_reread_environment(construction, monkeypatch):
    models = []
    ...
    for model_id, requested in cases:
        config = tiny(registry._get_config_info(model_id).pipeline_config_cls().dit_config)
        with selector._component_attention_backend_scope(requested, component="transformer"):
            selector.record_resolved_attention_backend(config)
...
def test_direct_construction_keeps_env_compatibility(construction, monkeypatch):
    monkeypatch.setenv("FASTVIDEO_ATTENTION_BACKEND", "VIDEO_SPARSE_ATTN")
    assert_construction(wan.WanTransformer3DModel(tiny(WanVideoConfig()), {}), VSA)


def test_fullattn_direct_construction_rejects_before_parameters(construction, monkeypatch):
    ...
    with pytest.raises(ValueError, match="FullAttn.*VIDEO_SPARSE_ATTN"):
        wan.WanTransformer3DModel(tiny(config), {})
    spy.assert_not_called()


def test_recorded_auto_survives_serialization(construction, monkeypatch):
    config = tiny(WanVideoConfig())
    selector.record_resolved_attention_backend(config)
    restored = pickle.loads(pickle.dumps(config))
    assert restored._attention_backend_resolved is True
    monkeypatch.setenv("FASTVIDEO_ATTENTION_BACKEND", "VIDEO_SPARSE_ATTN")
    assert_construction(wan.WanTransformer3DModel(restored, {}), SDPA)


def test_legacy_subclass_does_not_inherit_wan_backend_opt_in(construction, monkeypatch):
    ...
    model.config = tiny(WanVideoConfig())
    model.hidden_size = 4
...
def test_constructed_wan_keeps_backend_after_class_wrapping(construction, monkeypatch):
    config = tiny(WanVideoConfig())
    selector.record_resolved_attention_backend(config)
    model = wan.WanTransformer3DModel(config, {})
    ...
    monkeypatch.setenv("FASTVIDEO_ATTENTION_BACKEND", "VIDEO_SPARSE_ATTN")
    assert_construction(model, SDPA)


def test_instance_support_is_used(construction, monkeypatch):
    config = tiny(WanVideoConfig())
...
def test_metadata_uses_actual_fallback_from_instance_support(construction):
    config = tiny(WanVideoConfig())
    config.arch_config._supported_attention_backends = (SDPA,)
    with selector._component_attention_backend_scope(Backend.SAGE_ATTN, component="transformer"):
        selector.record_resolved_attention_backend(config)
    assert_construction(wan.WanTransformer3DModel(config, {}), SDPA)
...
def test_dense_cross_attention_small_cpu_tensor(construction, monkeypatch):
