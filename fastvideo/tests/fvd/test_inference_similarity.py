...
from .utils import calculate_single_fvd
...
if "A40" in device_name:
    ...
    logger.warning(f"Unsupported device for tests: {device_name}")
...
def test_i2v_inference_similarity(prompt, ATTENTION_BACKEND, model_id):
    ...
    to reference videos using FVD (replacing SSIM).
    """
    ...
    logger.info(f"Computing FVD between {reference_video_path} and {generated_video_path}")
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    fvd_score = calculate_single_fvd(reference_video_path, generated_video_path, device)

    logger.info(f"FVD Score: {fvd_score}")
    
    with open(os.path.join(output_dir, "fvd_results.json"), "w") as f:
        json.dump({"fvd": fvd_score, "prompt": prompt, "model": model_id}, f)

    
    max_acceptable_fvd = 1000.0 
    assert fvd_score <= max_acceptable_fvd, f"FVD score {fvd_score} is above threshold {max_acceptable_fvd} for {model_id}"
...
def test_inference_similarity(prompt, ATTENTION_BACKEND, model_id):
    ...
    max_acceptable_fvd = 1000.0
    assert fvd_score <= max_acceptable_fvd, f"FVD score {fvd_score} is above threshold {max_acceptable_fvd} for {model_id} with backend {ATTENTION_BACKEND}"