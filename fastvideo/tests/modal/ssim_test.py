...
def _sync_generated_videos_to_volume(
    ...
    if not os.path.isdir(generated_root):
        print("No generated_videos directory found; skipping raw generated video export.")
        return None
    ...
    print(f"Download command:\n  modal volume get hf-model-weights {relative_dst} ./generated_videos_modal")
    return relative_dst
...
def run_ssim_tests(
    ...
    if sync_generated_to_volume:
        download_src = f"{RAW_GENERATED_VOLUME_ROOT}/{resolved_subdir}/generated_videos"
        print(
