...
def _extract_component_times(result: dict) -> dict[str, float | None]:
    ...
    for stage_name, stage_data in stages.items():
        ...
        if metric_key is None:
            logger.debug("Unmapped stage '%s' (%.3fs) — add to STAGE_METRIC_MAP if needed",
                         stage_name, stage_data.get("execution_time", 0))
            continue
...
def _run_benchmark(cfg):
    ...
    try:
        ...
        for i in range(num_measure):
            ...
            elapsed, peak_mb, component_times = _run_generation(generator, prompt, gen_kwargs)
    ...
    results = {
        ...
        "text_encoder_time_s": _avg_component(all_component_times, "text_encoder_time_s"),
        "dit_time_s":          _avg_component(all_component_times, "dit_time_s"),
        "vae_decode_time_s":   _avg_component(all_component_times, "vae_decode_time_s"),
    }
