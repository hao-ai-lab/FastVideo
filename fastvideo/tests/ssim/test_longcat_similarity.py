...
def _resolve_longcat_paths() -> tuple[str, str | None]:
    """Return (model_path, lora_path_or_none). Skips if required local paths are missing."""
    ...
    default_lora = os.path.join(model_path, "lora", "distilled")
    lora_path = os.getenv("FASTVIDEO_LONGCAT_LORA_PATH", default_lora)

    if lora_path and _looks_like_local_path(lora_path) and not os.path.exists(lora_path):
...
def test_longcat_distill_similarity(prompt: str, ATTENTION_BACKEND: str):
    ...
    model_path, lora_path = _resolve_longcat_paths()
