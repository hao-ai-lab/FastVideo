...
def test_reward_specs_merge_top_level_options_and_inline_overrides() -> None:
    ...
    assert weights == {"hpsv3_general": 0.1, "dynamic_tracking": 0.7}
    assert options["hpsv3_general"] == {"device": "cpu", "max_frames": 4}
    assert options["dynamic_tracking"] == {"frame_pairs": 6, "top_fraction": 0.05}
...
def test_hpsv3_chunks_frames_without_changing_per_video_aggregation(monkeypatch) -> None:
    class FakeInferencer:
        ...
        def reward(self, paths, prompts):
            ...
            values = list(range(self.next_value, self.next_value + len(paths)))
            self.next_value += len(paths)
            return values

    inferencer = FakeInferencer()
    videos = np.zeros((2, 5, 2, 2, 3), dtype=np.uint8)
    monkeypatch.setattr(hpsv3, "_get_inferencer", lambda device: inferencer)
    monkeypatch.setattr(hpsv3, "media_to_uint8_array", lambda media: videos)

    scorer = hpsv3.HPSv3GeneralScorer(device="cpu", max_frames=None, batch_size=4)
    scores = scorer(torch.empty(0), ["first", "second"])

    assert inferencer.batch_sizes == [4, 4, 2]
    assert torch.equal(scores, torch.tensor([2.0, 7.0]))
