...
def test_cosmos2_transformer_video2world():
    ...
    # Use torch attention backend for exact diffusers matching
    cosmos_config = CosmosVideoConfig()
    cosmos_config.arch_config.attention_backend = "torch"
    ...
    args = FastVideoArgs(model_path=transformer_path,
                         ...
                         pipeline_config=PipelineConfig(dit_config=cosmos_config, dit_precision=precision_str))
    ...
    with torch.autocast('cuda', dtype=precision, enabled=False):
        output1 = model1(
            hidden_states=hidden_states,
            encoder_hidden_states=encoder_hidden_states,
            timestep=timestep,
    ...
    # With torch attention backend, outputs should now match closely
    assert max_diff < 1e-1, f"Maximum difference between outputs: {max_diff.item()}"
    # mean diff
    assert mean_diff < 1e-2, f"Mean difference between outputs: {mean_diff.item()}"