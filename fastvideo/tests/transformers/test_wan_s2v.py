...
def _stage_and_batch():
    ...
    model = _tiny_model()
    stage = WanS2VDenoisingStage(transformer=model, scheduler=None)
...
def test_denoising_stage_forwards_the_audio_and_reference_inputs() -> None:
    """The S2V denoising stage must actually hand the model its audio and reference image.
    ...
    kwargs, _ = stage.prepare_model_kwargs(batch, args)
...
def test_cfg_negative_pass_zeroes_the_audio() -> None:
    ...
    cond, uncond = stage.prepare_model_kwargs(batch, args)
