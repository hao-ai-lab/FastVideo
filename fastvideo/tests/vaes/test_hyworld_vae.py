...
# HyWorld uses the same VAE architecture as HunyuanVideo 1.5
...
def test_hyworld_vae():
    device = torch.device("cuda:0")
    precision = torch.float32
    precision_str = "fp32"
    args = FastVideoArgs(model_path=VAE_PATH, pipeline_config=PipelineConfig(vae_config=HyWorldVAEConfig(), vae_precision=precision_str))
    args.device = device
    args.vae_cpu_offload = False

    # HyWorld VAE is compatible with HunyuanVideo15 VAE architecture
