...
- 该文件可以出现 DMD2/critic/fake_score 等算法术语；这些语义不应泄漏到 model plugin/validator。

**依赖与边界**
- ✅ 不 import 任何具体模型/管线实现（Wan/SDXL/...）。
- ✅ 只依赖：
  - `RoleManager`/`RoleHandle`（获取 student/teacher/critic）
  - model plugin 提供的 primitives（通过 `_DMD2Model` Protocol / `ModelBase`）
...
**算法结构（高层）**
1) `prepare_batch()`：交给 model plugin
...
4) backward
   - 由于 Wan 的 forward_context 约束，需要 `model.backward(loss, ctx)`
...
- rollout 的 step list / simulate 逻辑由 method 管理：
  ...
  - `method_config.rollout_mode={simulate|data_latent}`
    - `simulate`：batch 不要求 `vae_latent`（model 用零 latents 构造形状）
    - `data_latent`：batch 必须带 `vae_latent`
  - 可选 `warp_denoising_step`（通过 model.noise_scheduler.timesteps duck-typing）
- model plugin 只提供单步 primitives：
