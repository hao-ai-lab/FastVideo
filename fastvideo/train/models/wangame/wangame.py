...
class WanGameModel(ModelBase):
    ...
    def init_preprocessors(self, training_config: TrainingConfig) -> None:
        ...
        if validation_cfg:
            ...
            if validation_enabled:
                from fastvideo.train.validators.wangame import (
                    WanGameValidator, )

                self.validator = WanGameValidator(training_config=training_config)
    ...
    def prepare_batch(
        ...
        *,
        ...
        image_latents = image_latents[:, :, :tc.data.num_latent_t]
        ...
        if (training_batch.keyboard_cond is not None
                and int(training_batch.keyboard_cond.shape[1]) != int(expected_num_frames)):
            raise ValueError("keyboard_cond temporal dim mismatch: "
                             f"got {int(training_batch.keyboard_cond.shape[1])}, "
                             f"expected {int(expected_num_frames)}")
        if (training_batch.mouse_cond is not None
                and int(training_batch.mouse_cond.shape[1]) != int(expected_num_frames)):
            raise ValueError("mouse_cond temporal dim mismatch: "
