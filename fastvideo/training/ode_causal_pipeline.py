...
class ODEInitTrainingPipeline(TrainingPipeline):
    ...
    def _get_next_batch(self, training_batch) -> tuple[TrainingBatch, torch.Tensor, torch.Tensor]:
        ...
        # Move to device
        device = get_local_torch_device()
        training_batch.encoder_hidden_states = encoder_hidden_states.to(device, dtype=torch.bfloat16)
    ...
    def _step_predict_next_latent(
        ...
        if self._cached_closest_idx_per_dmd is None:
            self._cached_closest_idx_per_dmd = torch.tensor([0, 12, 24, 36, S - 1], dtype=torch.long).cpu()
            # [0, 1, 2, 3], dtype=torch.long).cpu()
            logger.info("self._cached_closest_idx_per_dmd: %s", self._cached_closest_idx_per_dmd)
            logger.info("corresponding timesteps: %s", self.noise_scheduler.timesteps[self._cached_closest_idx_per_dmd])
