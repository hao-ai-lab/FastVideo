...
**Aligning prompts with flow_grpo (same reward curves).** To get the same prompts per batch as flow_grpo, use the same dataset and same sampler seed. (1) **Same dataset:** point FastVideo at flow_grpo’s OCR dataset so both read the same `train.txt`. From the FastVideo repo root run with `FLOW_GRPO_DATASET_OCR=/path/to/flow_grpo/dataset/ocr` (or e.g. `../flow_grpo/dataset/ocr` if flow_grpo is a sibling dir). The runscripts use `RL_DATASET_DIR="${FLOW_GRPO_DATASET_OCR:-data/ocr/}"`. (2) **Same seed:** both use seed 42 and step index = global batch index (FastVideo: `current_step`; flow_grpo: `epoch * num_batches_per_epoch + i`). (3) **Same batch shape:** use the same `train_batch_size`, `rl_num_image_per_prompt` (k), and number of replicas so the sampler’s `m` and per-rank indices match. Then for the same step index, both codebases see the same batch of prompts.
