...
def save_distillation_checkpoint(generator_transformer,
    ...
    if not only_save_generator_weight:
        ...
        if generator_transformer_2 is not None:
            ...
            generator_2_dcp_dir = os.path.join(save_dir, "distributed_checkpoint",
                                             "generator_2")
        ...
        if real_score_transformer_2 is not None:
            ...
            real_score_2_dcp_dir = os.path.join(save_dir, "distributed_checkpoint",
                                              "real_score_2")
    ...
    if rank == 0:
        ...
        if generator_transformer_2 is not None:
            ...
            if rank == 0:
                ...
                logger.info("--> generator_2 distillation checkpoint saved at step %s to %s", 
                           step, weight_path_2)
...
def load_distillation_checkpoint(generator_transformer,
    ...
    if generator_transformer_2 is not None:
        generator_2_dcp_dir = os.path.join(checkpoint_path, "distributed_checkpoint",
                                         "generator_2")
            ...
            if generator_ema_2 is not None:
                try:
                    ...
                    if ema_2_state is not None:
                        ...
                        logger.info("rank: %s, generator_2 EMA state loaded successfully",
                                    rank)
                    else:
                        logger.info("rank: %s, no EMA_2 state found in checkpoint", rank)
                except Exception as e:
                    logger.warning("rank: %s, failed to load EMA_2 state: %s", rank,
                                   str(e))
        else:
    ...
    if fake_score_transformer_2 is not None:
        critic_2_dcp_dir = os.path.join(checkpoint_path, "distributed_checkpoint",
                                      "critic_2")
        if os.path.exists(critic_2_dcp_dir):
            ...
            if fake_score_optimizer_2 is not None:
                critic_2_states["optimizer"] = OptimizerWrapper(fake_score_transformer_2,
    ...
    if real_score_transformer_2 is not None:
        real_score_2_dcp_dir = os.path.join(checkpoint_path, "distributed_checkpoint",
                                          "real_score_2")
