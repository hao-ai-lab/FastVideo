...
class SlidingTileAttentionImpl(AttentionImpl):
    ...
    def __init__(
        ...
        try:
            ...
            raise ValueError(f"FASTVIDEO_ATTENTION_CONFIG file not found: {config_file}")
        except json.JSONDecodeError:
            raise ValueError(f"FASTVIDEO_ATTENTION_CONFIG file is not a valid JSON file: {config_file}")
    ...
    def forward(self,
        ...
        if mask_strategy[0] is not None:
            query = torch.cat([self.tile(query, nccl_info.sp_size), encoder_query], dim=1).transpose(1, 2)
            key = torch.cat([self.tile(key, nccl_info.sp_size), encoder_key], dim=1).transpose(1, 2)
            value = torch.cat([self.tile(value, nccl_info.sp_size), encoder_value], dim=1).transpose(1, 2)
            ...
            raise NotImplementedError("mask_strategy cannot be None for STAttention")
