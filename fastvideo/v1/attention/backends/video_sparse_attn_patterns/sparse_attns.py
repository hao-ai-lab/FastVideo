...
def generate_topk_block_sparse_pattern_torch(block_attn_score, topk):
    ...
    q2k_block_sparse_num = torch.full((bs, h, num_q_blocks), topk, dtype=torch.int32, device=device)
...
def sparse_attn_c_s_p(q, k, v, topk, block_size, compress_attn_weight=None, select_attn_weight=None, return_attention_scores=False):
    ...
    Assume q, k, v is flattend in this way: [batch_size, num_heads, T//block_size[0], H//block_size[1], W//block_size[2], block_size[0], block_size[1], block_size[2]]
    """
    ...
    # compress attn
    q_compress = q.view(batch_size, num_heads, seq_len//block_elements, block_elements, head_dim).mean(dim=3) # [1, 12, 455, 128]
    k_compress = k.view(batch_size, num_heads, seq_len//block_elements, block_elements, head_dim).mean(dim=3)
    v_compress = v.view(batch_size, num_heads, seq_len//block_elements, block_elements, head_dim).mean(dim=3)
    
    output_compress, block_attn_score = torch_attention(q_compress, k_compress, v_compress) # [1, 12, 455, 128]
    
    output_compress = output_compress.view(batch_size, num_heads, seq_len//block_elements, 1, head_dim) # [1, 12, 455, 1, 128]
    output_compress = output_compress.repeat(1, 1, 1, block_elements, 1).view(batch_size, num_heads, seq_len, head_dim) # [1, 12, 29120, 128]
    
    q2k_block_sparse_index, q2k_block_sparse_num, k2q_block_sparse_index, k2q_block_sparse_num = generate_topk_block_sparse_pattern(block_attn_score, topk) #kind of slow # [1, 12, 455, 64] [1, 12, 455]
...
def sparse_attn_c_s(q, k, v, topk, block_size):
    ...
    # compress attn
    q_compress = q.view(batch_size, num_heads, seq_len//block_elements, block_elements, head_dim).mean(dim=3)
    k_compress = k.view(batch_size, num_heads, seq_len//block_elements, block_elements, head_dim).mean(dim=3)
    v_compress = v.view(batch_size, num_heads, seq_len//block_elements, block_elements, head_dim).mean(dim=3)
    
    output_compress, block_attn_score = torch_attention(q_compress, k_compress, v_compress)
    
    output_compress = output_compress.view(batch_size, num_heads, seq_len//block_elements, 1, head_dim)
    output_compress = output_compress.repeat(1, 1, 1, block_elements, 1).view(batch_size, num_heads, seq_len, head_dim)
    
    q2k_block_sparse_index, q2k_block_sparse_num, k2q_block_sparse_index, k2q_block_sparse_num = generate_topk_block_sparse_pattern(block_attn_score, topk)
    # (q, k, v, q2k_block_sparse_index, q2k_block_sparse_num, k2q_block_sparse_index, k2q_block_sparse_num):
...
def generate_stav1_block_sparse_pattern(batch_size, head_size, T, H, W, device, window_size=(1, 1, 1)):
    block_attn_score = torch.zeros(batch_size, head_size, T*H*W, T*H*W, device=device)
    seq_length = T * H * W
    for i in range(seq_length):
        ...
        from_t = max(window_size[0] // 2, min(from_t, (T - 1) - window_size[0] // 2))
        from_h = max(window_size[1] // 2, min(from_h, (H - 1) - window_size[1] // 2))
        from_w = max(window_size[2] // 2, min(from_w, (W - 1) - window_size[2] // 2))
        for j in range(seq_length):
            to_t = j // (H * W)
            to_h = (j % (H * W)) // W
            to_w = j % W
            if abs(from_t - to_t) <= window_size[0]//2 and abs(from_h - to_h) <= window_size[1]//2 and abs(from_w - to_w) <= window_size[2]//2:
