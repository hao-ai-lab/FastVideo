...
class DP_SP_BatchSampler(Sampler[List[int]]):
    ...
    def __init__(
        ...
        if self.drop_last:
            ...
            padding_size = self.num_sp_groups * self.batch_size - (self.dataset_size % (self.num_sp_groups * self.batch_size))
        ...
        logger.info(f"sp_group_local_indices: {len(sp_group_local_indices)}")
...
def get_parquet_files_and_length(path: str) -> Dict[str, int]:
    lengths = []
    file_names = []
    ...
    file_names, lengths = zip(*sorted(zip(file_names, lengths), key=lambda x: x[0]))
...
def read_row_from_parquet_file(parquet_files: List[str], global_row_idx: int, lengths: List[int]) -> Dict[str, Any]:
    ...
    for file_index in range(len(lengths)):
        if cumulative + lengths[file_index] > global_row_idx:
            parquet_file = parquet_files[file_index]
...
class LatentsParquetMapStyleDataset(Dataset):
    ...
    def __init__(
        ...
        if cfg_rate > 0.0:
            raise ValueError("cfg_rate > 0.0 is not supported for now because it will trygger bug when num_data_workers > 0")
        # self.rng = torch.Generator().manual_seed(seed)
        logger.info(f"Initializing LatentsParquetMapStyleDataset with path: {path}")
        ...
        logger.info(f"Dataset initialized with {len(self.parquet_files)} parquet files and {sum(self.lengths)} rows")

    def _get_torch_tensors_from_row_dict(self, row_dict: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        """
    ...
    def get_validation_negative_prompt(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict[str, Any]]:
        """
    ...
    def _pad(self, t: torch.Tensor, padding_length: int) -> torch.Tensor:
        ...
        """
        L, D = t.shape
        if L < padding_length:  # pad
            pad = torch.zeros(
                padding_length - L, D, dtype=t.dtype, device=t.device
            )
...
def main():
    ...
    parser.add_argument(
        "--num_data_workers", type=int, help="Number of DataLoader workers"
    )
    parser.add_argument(
        "--num_epoch", type=int, default=2, help="Number of epoches to benchmark"
    )
    parser.add_argument(
        "--verify_resume", action="store_true", help="Verify resume"
    )
    parser.add_argument(
    ...
    parser.add_argument('--checkpoint_path', type=str, default='dataloader_checkpoint', help='Path to save/load checkpoint')
    ...
    logger.info(f"Initialized distributed environment with world_size={world_size}")
    
    
    # Create DataLoader with proper settings
    dataloader = build_parquet_map_style_dataloader(args.path, args.batch_size, args.num_data_workers)
    logger.info(f"Initialized dataloader with {len(dataloader)} batches")
    

    
    if args.verify_resume:
        for i, (latents, embeddings, masks, data_indices) in enumerate(dataloader):
            logger.info(f"Batch {i} data_indices: {data_indices}")
        ...
        logger.info(f"Rank {get_world_rank()}: Saving dataloader state to {checkpoint_dir}")
        ...
        logger.info(f"Rank {get_world_rank()}: Saved checkpoint in {end_time - begin_time:.2f} seconds")
        ...
        logger.info(f"Rank {get_world_rank()}: Loading dataloader state from {checkpoint_dir}")
        load_states = {"dataloader": dataloader}
        dist_cp.load(load_states, checkpoint_id=checkpoint_dir.as_posix())
        logger.info(f"Rank {get_world_rank()}: Loaded dataloader state from {checkpoint_dir}")
    ...
    for _ in range(args.num_epoch):
        for i, (latents, embeddings, masks, data_indices) in enumerate(dataloader):
            ...
            if get_world_rank() == 0 and (i + 1) % 10 == 0:
                ...
                logger.info(
                    f"Batch {i + 1}/{args.num_batches_per_epoch}, Speed: {samples_per_sec:.2f} samples/sec"
                )
    ...
    if get_world_rank() == 0:
        ...
        logger.info(f"Average speed: {samples_per_sec:.2f} samples/sec")
        logger.info(f"Time per batch: {elapsed / total_batches * 1000:.2f} ms")
