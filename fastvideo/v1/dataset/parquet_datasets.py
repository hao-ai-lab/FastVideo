...
class ParquetVideoTextDataset(IterableDataset):
    ...
    def __init__(self,
        ...
        self.current_batches: Optional[Iterator[Any]] = None
    ...
    def __iter__(
        self
    ) -> Iterator[Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict[str,
                                                                       Any]]]:
        ...
        for i in range(len(batch["vae_latent_bytes"])):
            ...
            if len(text_attention_mask_bytes) > 0 and len(
                ...
                msk = np.frombuffer(text_attention_mask_bytes,
                                    dtype=np.int64).astype(np.bool_)
                msk = msk.reshape(1, -1)
                # Make array writable
                msk = np.copy(msk)
                if msk.shape[1] < 512:
...
def bind_cpu_cores(local_rank: int, cpu_per_process: int = 16) -> None:
