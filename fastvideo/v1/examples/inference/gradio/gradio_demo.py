...
def init_args():
    ...
    parser.add_argument("--model_path", type=str, default="FastVideo/FastHunyuan-diffusers")
...
def generate_video(
    ...
    fastvideo_args = ["--num_inference_steps", str(args.num_inference_steps),
                      "--prompt", args.prompt, "--num_frames", str(args.num_frames), "--guidance_scale", str(args.guidance_scale),
                      "--height", str(args.height), "--width", str(args.width), "--tp_size", str(args.tp_size),
                      "--sp_size", str(args.sp_size), "--model-path", args.model_path]
    


    # utils.launch_distributed(2, fastvideo_args)

    # output_path = os.path.join(tempfile.mkdtemp(), "output.mp4")
    # export_to_video(output, output_path, fps=30)
    generator.generate_video(prompt=args.prompt,
                             num_inference_steps=args.num_inference_steps,
                             num_frames=args.num_frames, tp_size=args.tp_size,
                             sp_size=args.sp_size)

    output_path = os.path.join(args.output_path, args.prompt[:100] + ".mp4")
...
args = FastVideoArgs(model_path="FastVideo/FastHunyuan-diffusers")
