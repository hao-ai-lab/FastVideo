...
class AutoencoderKLWan(nn.Module, ParallelTiledVAE):
    ...
    def __init__(self,
        ...
        self.config.scaling_factor = 1.0 / torch.tensor(self.config.latents_std).view(1, self.config.z_dim, 1, 1, 1)
        self.config.shift_factor = torch.tensor(self.config.latents_mean).view(1, self.config.z_dim, 1, 1, 1)
