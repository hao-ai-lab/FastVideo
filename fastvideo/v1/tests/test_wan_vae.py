...
def test_wan_vae():    
    ...
    input_tensor = torch.randn(batch_size, 3, 81, 32, 32, device=device, dtype=torch.bfloat16)
    ...
    with torch.no_grad():
        ...
        latents_std = 1.0 / torch.tensor(model2.config.latents_std).view(1, model2.config.z_dim, 1, 1, 1).to(
