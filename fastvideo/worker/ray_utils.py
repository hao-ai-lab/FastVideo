...
try:
    ...
    class RayWorkerWrapper(WorkerWrapperBase):
        ...
        def get_node_and_gpu_ids(self) -> tuple[str, list[int]]:
            ...
            if not device_key:
                raise RuntimeError("current platform %s does not support ray.", current_platform.device_name)
...
def assert_ray_available() -> None:
    ...
    if ray is None:
        raise ValueError(f"Failed to import Ray: {ray_import_err}.Please install Ray with `uv pip install ray`.")
