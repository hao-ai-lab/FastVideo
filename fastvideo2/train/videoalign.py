...
"""VideoAlign reward scorers (MQ / VQ / TA) for wan2.1 RL — ported from
fastvideo PR #1476 (``maint/pr1476-runtime-compat`` @518aeab0b) with the
runtime vendored VERBATIM under ``fastvideo2/rl_rewards/VideoAlign``.

Kept from the PR (all load-bearing on current transformers):
  * Qwen2VL state-dict key remap (visual./model.layers. → the 5.x
    ``model.visual``/``model.language_model`` layout) patched into
    ``load_state_dict`` of the reward model and PeftModel;
  * opencv video-reader backend registered when torchvision.io lacks
    ``read_video``;
  * ``embed_tokens`` alias for models that only expose it on
    ``language_model``.

Checkpoint: ``VIDEOALIGN_CHECKPOINT_PATH`` (a local clone of
KwaiVGI/VideoReward). Scores are the reward head's MQ/VQ/TA regression
outputs with ``use_norm=True`` rescaling.
"""
...
_VIDEOALIGN_ROOT = Path(__file__).resolve().parents[1] / "rl_rewards" / "VideoAlign"
...
def media_to_uint8_array(media: "torch.Tensor | np.ndarray") -> np.ndarray:
    """Convert image/video media to uint8 NHWC or NFHWC arrays (verbatim
    PR-branch ``rewards/media.py``)."""
...
def _get_inferencer(
    ...
    if cache_key not in _VIDEOALIGN_INFERENCERS:
        try:
            ...
            raise ImportError("VideoAlign rewards require the vendored runtime under "
                              "fastvideo2/rl_rewards/VideoAlign and a "
