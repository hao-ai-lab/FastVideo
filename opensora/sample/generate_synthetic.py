...
if __name__ == "__main__":
    ...
    for i, prompt in enumerate(prompt_list):
        video, latent = generate_video_and_latent(pipe, prompt, args.height, args.width, args.num_frames, args.num_inference_steps, args.guidance_scale)
        # save latent
        video_name = data[i]["path"].split("/")[-1].split(".")[0]
        latent_name = video_name + ".pt"
        latent_path = os.path.join(args.dataset_output_dir, "latent", latent_name)
        video_path = os.path.join(args.dataset_output_dir, "video", video_name + ".mp4")
        # save latent
        torch.save(latent, latent_path)
        export_to_video(video, video_path, fps=30)
        data[i]["latent_path"] = latent_name
    # save json
