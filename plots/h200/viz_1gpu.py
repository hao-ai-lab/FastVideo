#!/usr/bin/env python3
"""Visualize FastVideo pipeline stage timing breakdown for 1 GPU on H200."""
...
# Parse data rows and find GPU=1 row
times_ms = None
for line in lines[1:]:
    line = line.strip()
    if not line:
...
if times_ms is None:
    raise ValueError("GPU=1 row not found in data.txt")
...
fig.suptitle("LTX2 Pipeline Latency Breakdown (1 H200 GPU)", fontsize=14, fontweight='bold')
...
plt.savefig(os.path.join(script_dir, "timing_1gpu_h200.png"), dpi=150, bbox_inches='tight')
print(f"Saved figure to timing_1gpu_h200.png")
plt.show()
