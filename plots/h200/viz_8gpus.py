#!/usr/bin/env python3
"""Visualize FastVideo pipeline stage timing breakdown for 8 GPUs on H200."""
...
# Parse data rows and find GPU=8 row
times_ms = None
for line in lines[1:]:
    line = line.strip()
    if not line:
...
if times_ms is None:
    raise ValueError("GPU=8 row not found in data.txt")
...
fig.suptitle("LTX2 Pipeline Timing (8 H200 GPUs)", fontsize=14, fontweight='bold')
...
plt.savefig(os.path.join(script_dir, "timing_8gpus_h200.png"), dpi=150, bbox_inches='tight')
print(f"Saved figure to timing_8gpus_h200.png")
plt.show()
