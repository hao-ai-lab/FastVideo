...
def chat_with_gpt(prompt, style_sample):
    response = client.chat.completions.create(
        model="gpt-4-turbo",
        messages=[
            {"role": "system", "content": "Simulate the style of API server lines as shown in the sample."},
...
def remove_numbering(text):
    # Remove any numbering (e.g., "1. ", "2. ") at the beginning of each line
...
if __name__ == "__main__":
    user_prompt = (
        "Generate 100 video prompt lines in a similar style to the examples provided. "
        "Each line should be unique and descriptive, capturing a distinct visual scene or idea."
    )
    response = chat_with_gpt(user_prompt, style_sample)
    cleaned_response = remove_numbering(response)
