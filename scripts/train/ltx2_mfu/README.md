...
The measured optimization source is `20c36acef`. Later commits `0e60a0e9c` and `3f3f06541` isolate compiled validation and unload video-only validation state; a 4x run completed steps 0-50, validations at 0 and 50, and resumed at step 51 without a compile reset or OOM ([W&B run `iopu4dwm`](https://wandb.ai/wlsaidhi/fastvideo_ltx2/runs/iopu4dwm)). Re-gate performance after any source change before replacing the table.
