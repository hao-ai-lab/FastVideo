...
class LTX2FixedArenaZero2:
    ...
    def __init__(
        ...
        *,
        ...
        if any(p.dtype != torch.bfloat16 or not p.is_cuda or not p.is_contiguous() for p in all_params):
            raise ValueError("probe requires contiguous CUDA BF16 parameters")
        ...
        for params, padded_size in zip(groups, padded_sizes, strict=True):
            ...
            with torch.no_grad():
                for param in params:
                    ...
                    param_flat.copy_(param.detach().reshape(-1))
...
def main() -> None:
    ...
    payload = {
        ...
        for key in ("wall_ms", "zero_ms", "forward_ms", "backward_ms", "update_ms", "gpu_ms")
    }
    ...
    if rank == 0:
        ...
        result = {
            ...
            "median_phase_ms": {
                ...
                for key in ("zero_ms", "forward_ms", "backward_ms", "update_ms")
            },
