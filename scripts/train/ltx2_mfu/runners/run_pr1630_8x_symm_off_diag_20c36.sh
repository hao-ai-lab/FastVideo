...
log=/mnt/pr1630_8x_b3_symm_off_20c36_job1629519_node${NODE_RANK}.log
telemetry=/mnt/pr1630_8x_b3_symm_off_20c36_job1629519_node${NODE_RANK}_telemetry.csv
...
unset NCCL_NVLS_ENABLE || true
unset NCCL_MNNVL_ENABLE || true
...
nvidia-smi \
    --query-gpu=timestamp,index,pstate,utilization.gpu,power.draw,clocks.sm \
    --format=csv,noheader \
    --loop-ms=200 > "$telemetry" &
monitor_pid=$!
cleanup() {
...
/mnt/FastVideo/.venv/bin/torchrun \
    ...
    --training.distributed.fsdp_modules_per_group 1 \
    ...
    --training.checkpoint.output_dir /mnt/pr1630_8x_b3_symm_off_20c36_job1629519 \
    > "$log" 2>&1
