...
export TORCHINDUCTOR_CACHE_DIR=/mnt/pr1630_activation_checkpoint_cache_20c36
...
run_variant() {
    ...
    local log=/mnt/pr1630_activation_checkpoint_${label}_20c36.log
    echo "ACTIVATION_CHECKPOINT starting=$label type=$checkpoint_type"
    /mnt/FastVideo/.venv/bin/torchrun \
        --standalone \
        --nproc-per-node 4 \
        "$harness" \
        "${common_args[@]}" \
        --models.student.enable_gradient_checkpointing_type "$checkpoint_type" \
        --training.model.enable_gradient_checkpointing_type "$checkpoint_type" \
        --training.checkpoint.output_dir "/mnt/pr1630_activation_checkpoint_${label}_20c36" \
        > "$log" 2>&1
    echo "ACTIVATION_CHECKPOINT completed=$label"
    grep -E '^(BF16_RESULT|BF16_SEMANTICS) ' "$log"
    sha256sum "$log"
}

run_variant full_a full
run_variant none null
run_variant full_b full
