...
expected_harness_hash=ec4cd5092a691de0f0c630b5ed349f98e7d94d5795dfd11de9e242765bdcb790
harness=/mnt/benchmark_fastvideo_train_pack_ga_ec4c.py
...
run_variant() {
    ...
    local batch_size=$2
    local grad_accum=$3
    local log=/mnt/pr1630_batch2_equal_global_${label}_20c36.log
    FASTVIDEO_BENCH_LOCAL_BATCH_SIZE="$batch_size" \
    FASTVIDEO_BENCH_GRAD_ACCUM_STEPS="$grad_accum" \
        /mnt/FastVideo/.venv/bin/torchrun \
            ...
            --training.checkpoint.output_dir "/mnt/pr1630_batch2_equal_global_${label}_20c36" \
            > "$log" 2>&1
    grep -E '^(BF16_RESULT|BF16_SEMANTICS) ' "$log"
    sha256sum "$log"
}

run_variant control_a_b1_ga2 1 2
run_variant candidate_b2_ga1 2 1
run_variant control_b_b1_ga2 1 2
