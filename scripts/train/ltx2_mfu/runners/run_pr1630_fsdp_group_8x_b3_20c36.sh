...
run_variant() {
    ...
    local log=/mnt/pr1630_fsdp_group_8x_b3_${label}_20c36_job1629519_node${NODE_RANK}.log
    echo "FSDP_GROUP_8X_B3 starting=$label node_rank=$NODE_RANK modules_per_group=$modules_per_group"
    /mnt/FastVideo/.venv/bin/torchrun \
        ...
        --training.checkpoint.output_dir "/mnt/pr1630_fsdp_group_8x_b3_${label}_20c36_job1629519" \
        > "$log" 2>&1
    echo "FSDP_GROUP_8X_B3 completed=$label node_rank=$NODE_RANK"
    if [[ "$NODE_RANK" == 0 ]]; then
