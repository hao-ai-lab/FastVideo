...
common_args=(
    ...
    --models.student.enable_gradient_checkpointing_type full
    --training.model.enable_gradient_checkpointing_type full
    --training.model.enable_torch_compile false
...
run_variant() {
    ...
    local log=/mnt/pr1630_fsdp_group_fullckpt_${label}_20c36.log
    echo "FSDP_GROUP_FULLCKPT starting=$label modules_per_group=$modules_per_group"
    /mnt/FastVideo/.venv/bin/torchrun \
        ...
        --training.checkpoint.output_dir "/mnt/pr1630_fsdp_group_fullckpt_${label}_20c36" \
        > "$log" 2>&1
    echo "FSDP_GROUP_FULLCKPT completed=$label"
