...
run_variant() {
    local label=$1
    local packed=$2
    echo "PACK_GATE starting=$label packed=$packed" >&2
    /mnt/FastVideo/.venv/bin/torchrun \
        ...
        --training.checkpoint.output_dir "/mnt/pr1630_pack_4x_${label}_d016" \
        ...
        > "/mnt/pr1630_pack_4x_${label}_d016.log" 2>&1
    echo "PACK_GATE completed=$label" >&2
}
