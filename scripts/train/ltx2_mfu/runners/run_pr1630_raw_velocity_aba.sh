...
run_variant() {
    local label=$1
    local raw_velocity_flag=$2
    /mnt/FastVideo/.venv/bin/torchrun --standalone --nproc-per-node=4 \
        /mnt/benchmark_fastvideo_train_ltx2_raw_velocity.py \
        $raw_velocity_flag \
        ...
        --training.checkpoint.output_dir "/mnt/pr1630_raw_velocity_${label}" \
        --callbacks.grad_clip.log_grad_norms true \
        --callbacks.validation.every_steps 0 \
        > "/mnt/pr1630_raw_velocity_${label}.log" 2>&1
}

run_variant control_a ""
run_variant candidate --raw-velocity
