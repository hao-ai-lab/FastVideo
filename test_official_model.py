...
snapshot_dir = glob.glob(os.path.expanduser(
...
import json, importlib, sys, types
vae_config_path = os.path.join(vae_dir, "config.json")
with open(vae_config_path) as f:
    vae_config = json.load(f)

auto_map = vae_config.get("auto_map", {})
target = auto_map.get("AutoModel") or auto_map.get("AutoencoderKL") or auto_map.get("Autoencoder")
...
for pyf in sorted(glob.glob(os.path.join(vae_dir, "*.py"))):
    ...
    spec = importlib.util.spec_from_file_location(full_name, pyf, submodule_search_locations=[])
...
official_dir = snapshot_download("Overworld/Waypoint-1-Small", allow_patterns=["transformer/*"])
...
WorldModel = getattr(model_mod, "WorldModel")

# Create model from config
model_cfg = {k: v for k, v in t_config.items() 
...
if any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig2.parameters.values()):
...
class DummyKVCache:
    ...
    def set_frozen(self, frozen): pass
...
if cache_mod is not None:
    ...
    if StaticKVCache:
        try:
            kv_cache = StaticKVCache(transformer.config, max_frames=64, device="cuda", dtype=torch.bfloat16)
...
inputs = tokenizer(prompt, padding="max_length", max_length=512, truncation=True, return_tensors="pt")
...
sigmas = torch.tensor(t_config.get("scheduler_sigmas", [1.0, 0.9483, 0.8380, 0.0]),
...
with torch.no_grad():
    ...
    for step_sig, step_dsig in zip(sigmas, sigmas.diff()):
        ...
        x = x + step_dsig * v
        xf = x.float()
        print(f"Step sigma={step_sig.item():.4f}: v mean={v.float().mean():.4f} std={v.float().std():.4f}  "
              f"x mean={xf.mean():.4f} std={xf.std():.4f}")

    print(f"Denoised: mean={xf.mean():.4f} std={xf.std():.4f} min={xf.min():.4f} max={xf.max():.4f}")

    # Decode
    latent = x.squeeze(1).float()  # [1, 16, 32, 32]
    print(f"VAE input: shape={list(latent.shape)} dtype={latent.dtype}")
    decoded = vae.decode(latent)
    print(f"VAE output: type={type(decoded).__name__} shape={list(decoded.shape)} dtype={decoded.dtype}")
    
    if decoded.dtype == torch.uint8:
        print(f"Pixel stats: mean={decoded.float().mean():.1f} min={decoded.min()} max={decoded.max()}")
        # Per channel
