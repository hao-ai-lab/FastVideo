...
from fastvideo.api.schema import GenerationRequest, InputConfig
...
def test_typed_h3_inputs_reach_forward_batch_without_extension_dicts(monkeypatch) -> None:
    ...
    last_image = "last.png"
    references = [MiniMaxH3Reference(source="reference.wav")]
    ...
    sampling_param = request_to_sampling_param(
        normalize_generation_request(request), model_path="local/minimax-h3-contract"
    )
    batch = ForwardBatch(**shallow_asdict(sampling_param))

    assert batch.pil_image == first_image
    assert batch.last_image == last_image
