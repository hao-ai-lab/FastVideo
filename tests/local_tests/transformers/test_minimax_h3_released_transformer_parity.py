...
"""Released-weight production-loader parity for both MiniMax H3 DiT partitions.
...
def _require_released_assets(partition: str) -> tuple[torch.device, Path]:
    if os.environ.get(RUN_ENV) != "1":
        pytest.skip(f"set {RUN_ENV}=1 on an allocated CUDA node")
    if not torch.cuda.is_available() or not torch.cuda.is_bf16_supported():
        pytest.skip("MiniMax H3 released DiT parity requires a bf16-capable CUDA GPU")
    if not OFFICIAL_SRC.is_dir():
        pytest.skip(f"pinned Diffusers MiniMax H3 checkout is missing: {OFFICIAL_REF_DIR}")
    ...
    if missing:
        pytest.skip(f"MiniMax H3 released {partition} assets are missing: {missing}")
...
def test_minimax_h3_released_transformer_production_loader_parity(partition: str) -> None:
    """Match both released DiT partitions against the pinned official implementation."""
    device, component_dir = _require_released_assets(partition)
