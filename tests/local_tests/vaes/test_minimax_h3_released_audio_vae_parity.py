...
"""Released-weight production-loader parity for the MiniMax H3 audio VAE."""
...
def _require_released_assets() -> tuple[torch.device, Path]:
    if os.environ.get(RUN_ENV) != "1":
        pytest.skip(f"set {RUN_ENV}=1 on an allocated CUDA node")
    if not torch.cuda.is_available():
        pytest.skip("MiniMax H3 released audio VAE parity requires CUDA")
    ...
    if missing:
        pytest.skip(f"MiniMax H3 released audio VAE assets are missing: {missing}")
...
def _make_waveform() -> torch.Tensor:
    ...
    # path for the released 800-sample hop length.
...
def test_minimax_h3_released_audio_vae_production_loader_parity() -> None:
    """Match released posterior, normalization, hop geometry, and deterministic decode."""
    device, component_dir = _require_released_assets()
