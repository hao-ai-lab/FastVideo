...
"""Released-weight production-loader parity for the MiniMax H3 video VAE."""
...
def _require_released_assets() -> tuple[torch.device, Path]:
    ...
    if not torch.cuda.is_available():
        pytest.skip("MiniMax H3 released video VAE parity requires CUDA")
    ...
    if missing:
        pytest.skip(f"MiniMax H3 released video VAE assets are missing: {missing}")
...
def test_minimax_h3_released_video_vae_production_loader_parity() -> None:
    """Match released posterior, normalization, geometry, and deterministic decode."""
