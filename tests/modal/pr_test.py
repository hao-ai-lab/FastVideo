...
def run_encoder_tests():
    run_test("pytest ./tests/encoders -vs")

@app.function(gpu="L40S:1", image=image, timeout=900)
def run_vae_tests():
    run_test("pytest ./tests/vaes -vs")

@app.function(gpu="L40S:1", image=image, timeout=900)
def run_transformer_tests():
    run_test("pytest ./tests/transformers -vs")

@app.function(gpu="L40S:2", image=image, timeout=2700)
def run_ssim_tests():
    run_test("pytest ./tests/ssim -vs")
...
def run_training_tests():
    run_test("wandb login $WANDB_API_KEY && pytest ./tests/training/Vanilla -srP")
...
def run_training_lora_tests():
    run_test("wandb login $WANDB_API_KEY && pytest ./tests/training/lora/test_lora_training.py -srP")
...
def run_training_tests_VSA():
    run_test("wandb login $WANDB_API_KEY && pytest ./tests/training/VSA -srP")

@app.function(gpu="H100:2", image=image, timeout=900)
def run_inference_tests_STA():
    run_test("pytest ./tests/inference/STA -srP")
...
def run_inference_tests_vmoba():
    run_test('python tests/inference/vmoba/test_vmoba_inference.py')

@app.function(gpu="L40S:1", image=image, timeout=3600)
def run_inference_lora_tests():
    run_test("pytest ./tests/inference/lora/test_lora_inference_similarity.py -vs")

@app.function(gpu="L40S:2", image=image, timeout=900)
def run_distill_dmd_tests():
    run_test("pytest ./tests/training/distill/test_distill_dmd.py -vs")

@app.function(gpu="L40S:1", image=image, timeout=900)
def run_unit_test():
    run_test("pytest ./tests/dataset/ ./tests/workflow/ -vs")
