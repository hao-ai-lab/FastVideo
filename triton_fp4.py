...
def initialize_block_scaled(M, N, K, block_scale_type="nvfp4", compute_reference=False):
    ...
    def _expand_A_scale_full(a_scale_fp8):
        ...
        base = a_scale_fp8[..., 0, 0].to(torch.float32)            # [m_chunks, k_chunks]
    ...
    def _expand_B_scale_full(b_scale_fp8):
        ...
        base = b_scale_fp8[..., 0, 0].to(torch.float32)            # [n_chunks, k_chunks]
    ...
    torch.manual_seed(0)
    a_f32_pre = torch.randn((M, K), device=device, dtype=torch.float32) * 0.5
    b_f32_pre = torch.randn((K, N), device=device, dtype=torch.float32) * 0.5
    ...
    a_ref_f32 = a_ref_fp4.to(torch.float32) * a_scale_full          # [M, K]
    b_ref_f32 = b_ref_fp4.to(torch.float32) * b_scale_full          # [K, N]
