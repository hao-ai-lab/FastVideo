...
export default function CreateDatasetModal({
	...
	const [captionFormat, setCaptionFormat] = useState<"json" | "txt" | "csv">(
		"json",
	);
	const [captionMap, setCaptionMap] = useState<Record<string, string> | null>(
		null,
	);
	...
	const [videosTxtFileName, setVideosTxtFileName] = useState<string | null>(
		null,
	);
	const [captionsTxtLines, setCaptionsTxtLines] = useState<string[] | null>(
		null,
	);
	const [captionsTxtFileName, setCaptionsTxtFileName] = useState<
		string | null
	>(null);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (captionFormat === "json" || captionFormat === "csv") {
			...
			captionFormat === "txt" &&
			(videosTxtFileName || captionsTxtFileName)
		) {
								...
								<>
									...
									>
										...
										<UploadZone
											...
											accept=".txt,text/plain"
											value={
												captionsTxtFileName ? "1" : ""
											}
											fileName={
												captionsTxtFileName ?? undefined
											}
											onFileChange={
												handleCaptionsTxtChange
											}
											onClear={() => {
												setCaptionsTxtLines(null);
												setCaptionsTxtFileName(null);
