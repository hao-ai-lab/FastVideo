...
export default function CreateJobModal({
	...
	const [numInferenceSteps, setNumInferenceSteps] = useState(
		defaultOptions.numInferenceSteps,
	);
	...
	const [guidanceScale, setGuidanceScale] = useState(
		defaultOptions.guidanceScale,
	);
	const [guidanceRescale, setGuidanceRescale] = useState(
		defaultOptions.guidanceRescale,
	);
	...
	const [ditCpuOffload, setDitCpuOffload] = useState<boolean>(
		defaultOptions.ditCpuOffload,
	);
	const [textEncoderCpuOffload, setTextEncoderCpuOffload] = useState<boolean>(
		defaultOptions.textEncoderCpuOffload,
	);
	const [vaeCpuOffload, setVaeCpuOffload] = useState<boolean>(
		defaultOptions.vaeCpuOffload,
	);
	const [imageEncoderCpuOffload, setImageEncoderCpuOffload] =
		useState<boolean>(defaultOptions.imageEncoderCpuOffload);
	const [useFsdpInference, setUseFsdpInference] = useState<boolean>(
		defaultOptions.useFsdpInference,
	);
	const [enableTorchCompile, setEnableTorchCompile] = useState<boolean>(
		defaultOptions.enableTorchCompile,
	);
	const [vsaSparsity, setVsaSparsity] = useState<number>(
		defaultOptions.vsaSparsity,
	);
	...
	useEffect(() => {
		if (isOpen) {
			...
			setNumFrames(workloadType === "t2i" ? 1 : defaultOptions.numFrames);
	...
	return (
		<div className={modalStyles.modal} data-modal>
			...
			<div
				className={`${modalStyles.modalContent} ${modalStyles.modalForm}`}
			>
				<button
				...
				>
					<h2>
						New {jobType.charAt(0).toUpperCase() + jobType.slice(1)}{" "}
						Job
						{WORKLOAD_OPTIONS[jobType]?.find(
							(o) => o.type === workloadType,
						)
					...
					<form onSubmit={handleSubmit} autoComplete="off">
						...
						{isInference && (
							<div className={formStyles.formRow}>
								<label htmlFor="modal-negative-prompt">
									Negative Prompt
								</label>
								<textarea
									name="negativePrompt"
						...
						{!isInference && (
							<>
								<div className={formStyles.formRow}>
									...
									<span className={formStyles.helperText}>
										Enter the path to your preprocessed data
										(e.g. parquet output directory)
									</span>
								</div>
								<div className={formStyles.formRow}>
									<label htmlFor="modal-max-train-steps">
										Max Train Steps
									</label>
									<Slider
								...
								<div className={formStyles.formRow}>
									<label htmlFor="modal-train-batch-size">
										Train Batch Size
									</label>
									<Slider
								...
								<div className={formStyles.formRow}>
									<label htmlFor="modal-learning-rate">
										Learning Rate
									</label>
									<input
								...
								<div className={formStyles.formRow}>
									<label htmlFor="modal-num-latent-t">
										Num Latent T
									</label>
									<Slider
						...
						{isInference && (
							<details className={formStyles.advancedSettings}>
								...
								<div className={formStyles.settingsGrid}>
									{workloadType !== "t2i" && (
										<div className={formStyles.formRow}>
											<label htmlFor="modal-num-frames">
												Frames
											</label>
											<Slider
												id="modal-num-frames"
									...
									<div className={formStyles.formRow}>
										<label htmlFor="modal-dit-cpu-offload">
											DiT CPU Offload
										</label>
										<Toggle
									...
									<div className={formStyles.formRow}>
										<label htmlFor="modal-use-fsdp-inference">
											Use FSDP Inference
										</label>
										<Toggle
									...
									<div className={formStyles.formRow}>
										<label htmlFor="modal-vae-cpu-offload">
											VAE CPU Offload
										</label>
										<Toggle
									...
									<div className={formStyles.formRow}>
										<label htmlFor="modal-enable-torch-compile">
											Torch Compile
										</label>
										<Toggle
