...
import { startJob, stopJob, deleteJob, downloadJobVideo } from "@/lib/api";
	...
	return (
		...
		>
			<div className={jobCardStyles.jobHeader}>
				...
				<span
					className={`${badgeStyles.badge} ${badgeStyles[badgeClass as keyof typeof badgeStyles] || badgeStyles.badgePending}`}
				>
					{job.status}
				</span>
			</div>
			...
			<div className={jobCardStyles.jobMeta}>
				{job.job_type === "inference" ? (
					...
					<span>
						{job.workload_type?.replace(/_/g, " ") ?? job.job_type}
					</span>
				)}
				{getElapsedTime() && (
