...
const ActiveDatasetContext = createContext<
	ActiveDatasetContextValue | undefined
>(undefined);
...
export function ActiveDatasetProvider({
	...
	const [activeDataset, setActiveDatasetState] = useState<Dataset | null>(
		null,
	);
...
export function useActiveDataset(): ActiveDatasetContextValue {
	...
	if (!ctx) {
		throw new Error(
			"useActiveDataset must be used within ActiveDatasetProvider",
		);
	}
	return ctx;
}
