...
try:
    from v2.attention.utils.flash_attn_cute import flash_attn_func
...
if fa_version in ("2", "3"):
    ...
    # (registered in `v2.attention.utils.flash_attn_cute`), so a
...
from v2.attention.backends.abstract import (
    AttentionBackend,
    AttentionImpl,
    AttentionMetadata,
...
try:
    from v2.attention.utils.flash_attn_cute import flash_attn_fp4_func
...
class FlashAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        if (attn_metadata is not None and hasattr(attn_metadata, "attn_mask") and attn_metadata.attn_mask is not None):
            from v2.attention.utils.flash_attn_no_pad import (
                flash_attn_no_pad,
