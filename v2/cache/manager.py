"""CacheManager — per-class pools with static budgets, behind one handle (design_v3 §7.2).
...
class CacheManager:
    ...
    def from_card(cls, card) -> "CacheManager":
        ...
        for cc in card.caches.values():
            policies.append(CachePolicy(
                class_name=cc.cache_class, max_bytes=cc.max_bytes, block_bytes=cc.block_bytes,
                eviction=cc.eviction, reuse_across_requests=cc.reuse_across_requests,
    ...
    def stats(self) -> dict[str, Any]:
        return {name: {"used_bytes": p.used_bytes, "hits": p.hits, "misses": p.misses}
