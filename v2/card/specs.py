"""ModelCard and its sub-specs — the Model Plane, the center (design_v3 §4).

> The atomic unit is the (recipe, runtime) pair, owned by a typed ``ModelCard``.

The card is both a **declarative contract** (strict enough to validate before any
GPU touches it — see ``ModelCard.validate``) and a **runtime factory** (it knows how
to instantiate components, bind loops, and resolve caches — see ``card/instance.py``).

design_v3 §18 boundary: ``card/`` imports no product/runtime. It depends only on the
shared leaf modules (``_enums``, ``_types``) and references ``ParallelPlan`` under
``TYPE_CHECKING`` so there is no runtime coupling to ``parallel/``.
"""
...
# --------------------------------------------------------------------------- #
# Cost model — the budget currency, REQUIRED on every LoopSpec (design_v3 §6.1) #
# --------------------------------------------------------------------------- #
@dataclass
class CostModel:
    """Predicted GPU-time per WorkUnit at (shape, precision, policy).

    "The budget currency is predicted GPU-time, not counts." One object, two
    consumers: the scheduler's internal budget and the fleet's routing input
    (design_v3 §6.1, §14). Calibrated online by the Profiler observer (§11).
    """
    kind: WorkUnitKind
    ...
    def predict(self, work_units: float, policy_factor: float = 1.0) -> float:
        """Conservative baseline cost (design_v3 §6.1: admission never optimistic)."""
...
@dataclass
class CacheContract:
    """A cache class the card declares it needs (design_v3 §7.2; designv2 cache plane)."""
    ...
    training_mode_disables_recycle: bool = False           # chunk-KV training mode (§7.2)


@dataclass
class ParityTestSpec:
    """A named tap + tolerance + ladder level (design_v3 §9.1)."""
    name: str
...
@dataclass
class ParitySpec:
    """The (recipe, runtime) honesty contract (design_v3 §9). Measured, never assumed."""
    ...
    interleave_required: bool = True                       # the batch-of-N gate (§9.3) is non-negotiable
...
@dataclass
class DataRef:
    """What a recipe trained on, for governance/reproduction (design_v3 §4.2)."""
    dataset_id: str = ""
    revision: str = ""
    description: str = ""
...
@dataclass
class RecipeSpec:
    """The provenance half of the (recipe, runtime) pair (design_v3 §4.2, §2.1).
...
@dataclass
class ComponentSpec:
    """A weight-bearing (or processing) component (design_v3 §4.3; designv2 §1.2).
    ...
    # omni-ready (design_v3 §4.1; omni-readiness report §4a):
...
@dataclass
class LoopSpec:
    """Describes one iterative computation the card can run (design_v3 §4.3).
    ...
    step_cost_model: CostModel                             # REQUIRED (design_v3 §6.1)
...
class CheckpointManifest:
    """Explicit declared components + key maps — no name-detector guessing (design_v3 §4.1)."""
    upstream_source: str = ""
    revision: str = ""
...
class ModelCard:
    ...
    def validate(self) -> "ModelCard":
        ...
        # 1) recipe.assumes_loop must exist (the (recipe, runtime) binding, §2.1/§4.2)
        if self.recipe.assumes_loop and self.recipe.assumes_loop not in self.loops:
            errs.append(
                f"recipe.assumes_loop={self.recipe.assumes_loop!r} is not a declared loop "
        ...
        if (self.recipe.assumes_precision
                and self.recipe.assumes_precision not in (self.precision.default_dtype,
                                                          *self.precision.component_overrides.values())):
            errs.append(
                f"recipe.assumes_precision={self.recipe.assumes_precision!r} not present in precision contract"
            )
        ...
        for lid, loop in self.loops.items():
            ...
            if loop.step_cost_model is None:
                errs.append(f"loop {lid!r} is missing the REQUIRED step_cost_model (design_v3 §6.1)")
        ...
        if errs:
            raise CardValidationError(
                f"ModelCard {self.model_id!r} failed validation:\n  - " + "\n  - ".join(errs))
        return self
