...
from v2.layers.quantization.base_config import (
    QuantizationConfig,
    QuantizeMethodBase,
)
...
def adjust_scalar_to_fused_array(param: torch.Tensor, loaded_weight: torch.Tensor,
...
class MergedColumnParallelLinear(ColumnParallelLinear):
    ...
    def _load_fused_module_from_checkpoint(self, param: BasevLLMParameter, loaded_weight: torch.Tensor) -> None:
        """
...
class QKVParallelLinear(ColumnParallelLinear):
    ...
    def weight_loader(
        ...
        if output_dim is not None:
            ...
            shard_idx = 0
            param_data = param_data.narrow(output_dim, shard_offset, shard_size)
            shard_idx = tp_rank if loaded_shard_id == "q" else tp_rank // self.num_kv_head_replicas
