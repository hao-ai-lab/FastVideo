...
(cache-dit skip + early-exit; design_v3 §2.2, §9.12). Reuses the Wan T2V program (loop_id
...
def build_adaptive_card(model_id: str = "wan-adaptive", *, cache_threshold: float = 0.02,
    ...
    def loop_factory():
        return CacheDiTDenoiseLoop(loop_id="diffusion_denoise", cfg=cfg, flow_shift=flow,
                                   precision=precision, expert=expert, cost=cost,
                                   cache_threshold=cache_threshold, exit_threshold=exit_threshold)
