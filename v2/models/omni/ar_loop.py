"""ARDecodeLoop — token decode until EOS/max_tokens (design_v3 §4.3, §5; §15b Cosmos3 reasoner).
...
model's ``diffusion_denoise`` loop — one resident MoT instance running both loop types (the §16
claim). All per-request state (the token list, paged-KV namespace) lives in ``LoopState``.
"""
...
class ARDecodeLoop:
    ...
    def next(self, st: LoopState):
        ...
        return WorkPlan(
            ...
            resources=ResourceRequest(compute_seconds=self.cost.predict(len(tokens)),
                                      resident_bytes=8 * len(tokens), peak_activation_bytes=8),
            payload={"branch": "ar", "pos": pos}, run=run, emits=emits, label=f"ar.tok{pos}")
    ...
    def finalize(self, st: LoopState) -> LoopResult:
        ...
        return LoopResult(outputs={"tokens": toks, "text": "tok:" + ",".join(map(str, toks))},
