"""VocoderLoop — streaming code2wav (design_v3 §4.3 ``LoopKind.AUDIO_DECODE``; Qwen-Omni stage 2).

The third loop type in the thinker→talker→vocoder cascade. It consumes the talker's speech-codec
tokens in fixed-size chunks; **each chunk is one ``AUDIO_CHUNK`` WorkUnit** that synthesizes a
waveform segment and streams it. This is the differentiation vllm-omni's own docs name: their
code2wav is one opaque ``LLM_GENERATION`` stage the scheduler never sees inside — here every chunk
is a runtime-visible, step-scheduled, cancellable WorkUnit, on the *same* driven-loop contract the
engine uses for denoise steps and AR tokens. All per-request state lives in ``LoopState``.
"""
...
class VocoderLoop:
    ...
    def init(self, req, model, ctx) -> LoopState:
        ...
        st.scratch["chunks"] = [toks[i:i + self.chunk_tokens]
                                for i in range(0, len(toks), self.chunk_tokens)] or [[1]]
        st.scratch["wave"] = []
    ...
    def next(self, st: LoopState):
        ...
        return WorkPlan(
            loop_id=self.loop_id, instance_id=st.instance_id, kind=WorkUnitKind.AUDIO_CHUNK,
            ...
            payload={"chunk": i}, run=run, emits=emits, label=f"vocoder.chunk{i}")
    ...
    def finalize(self, st: LoopState) -> LoopResult:
        ...
        return LoopResult(outputs={"samples": wave.astype("float32")},
                          metrics={"audio_chunks": float(st.step_idx),
