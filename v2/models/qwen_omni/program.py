...
(``thinker2talker_full_payload``, ``talker2code2wav_full_payload``) — but here they are explicit
slot writes between three driven loops, not opaque stage-boundary callbacks. The talker conditions on
the thinker's **hidden state** (``reasoner_embed``), not only its tokens — the "full payload" path;
the streaming/sync path would pass tokens only (noted inline). Final outputs: ``text`` (thinker) and
...
def build_qwen_omni_program() -> Program:
    return Program(
        program_id="qwen_omni.tts", kind=ProgramKind.INLINE,
        nodes=[
            ComponentNode("tokenize", fn=tokenize_node, writes=("prompt_tokens",)),
            ModelLoopNode("thinker", loop_id="thinker_decode", output_slot="thinker_out",
                          reads=("prompt_tokens",), writes=("thinker_out",)),
            ComponentNode("emit_text", fn=emit_text_node("thinker_out", "text"),
                          reads=("thinker_out",), writes=("text",)),
            ComponentNode("thinker_to_talker", fn=thinker_to_talker_node,
                          reads=("thinker_out",), writes=("talker_prompt_tokens",)),
            ModelLoopNode("talker", loop_id="talker_decode", output_slot="talker_out",
                          reads=("talker_prompt_tokens",), writes=("talker_out",)),
            ComponentNode("talker_to_vocoder", fn=talker_to_vocoder_node,
                          reads=("talker_out",), writes=("speech_tokens",)),
            ModelLoopNode("vocoder", loop_id="vocoder", output_slot="vocoder_out",
                          reads=("speech_tokens",), writes=("vocoder_out",)),
            ComponentNode("emit_audio", fn=emit_audio_node("vocoder_out", "audio"),
                          reads=("vocoder_out",), writes=("audio",)),
        ],
        output_artifacts={"text": "text", "audio": "audio", "speech_tokens": "talker_out"},
    ).validate()
