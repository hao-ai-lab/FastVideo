"""Cosmos25DenoiseLoop — flow-match Euler denoiser with Cosmos2.5's per-frame sigma timestep.

Cosmos-Predict2.5 is a flow-match model (unlike Cosmos-Predict2's EDM denoiser). The sigma schedule and
Euler solver are Wan's (``FlowShiftPolicy.build_schedule`` + ``FLOW_MATCH_STEP``, shift=5.0) and CFG is
``ClassicCFG``, so this loop nearly matches ``WanDenoiseLoop``. The one fork: Wan feeds the DiT a scalar
``timestep = sigma*1000``, but Cosmos2.5 feeds a per-frame 2D ``timestep[B,T]`` of the plain sigma in
...
(``Cosmos25V2WDenoisingStage``) but not in the registered HF presets, so this loop is t2v-only. The hooks
are read from ``ctx.slots`` (None for t2v -> inert), mirroring Wan/Cosmos2, so a later capability can add
the gated injection without touching the t2v path. (BRINGUP: frame-replace needs a request-API extension
to carry the conditioning latents + indicator.)
"""
...
# Cosmos2.5 Wan-style VAE (Cosmos25WanVAE): z_dim=16, 4x temporal, 8x spatial — same geometry as Wan2.1,
# so the reused ``latent_shape`` yields (16, (F-1)//4 + 1, H//8, W//8) on the real backend.
...
class Cosmos25DenoiseLoop:
    ...
    def init(self, req, model, ctx) -> LoopState:
        ...
        # Wan's flow-shift sigma schedule (shift=5.0): 1 -> 0 over num_steps+1 points, warped.
        ...
        st = LoopState(
            ...
            # Model timestep is the plain sigma (not sigma*1000); the Cosmos25DiT adapter
            # broadcasts it to the per-frame [B, T] form the forward needs.
        ...
        # video2world (frame_replace) conditioning: None for the t2v preset -> any later gated injection
        # is inert (pure t2v, matching the fastvideo pipeline with no image/video).
    ...
    def next(self, st: LoopState):
        ...
        # Model timestep is the plain sigma (st.timesteps[i] == st.sigmas[i]); the adapter turns it into
        # the per-frame [B, T] timestep + masks + fps internally.
        ...
        def _velocity(model: Any, x_: Any, sigma_t_: float, pe_: Any, ne_: Any, scale_: float) -> np.ndarray:
            # Conditioned forward + ClassicCFG combine. The dit-call is toy-compatible:
        ...
        def run(model, override=None):
            # Eager thunk: handles the override (cached-prediction) path. The Euler solver dispatches
            # through the platform kernel table (numpy on CPU, the device kernel on GPU).
        ...
        def graph_fn(model, ws):
            # Capturable op-structure: reads every per-step input from the static workspace and writes
        ...
        return WorkPlan(
            ...
            # Flow-match Euler step is a clean capturable op (no host RNG / data-dependent branch), like
            # Wan t2v: branch-set + expert + scheduler-precision discriminate the captured graph.
            capturable=True,
