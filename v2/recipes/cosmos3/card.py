"""Cosmos3 omni ModelCard — one resident MoT transformer serving both und and generation.

ONE resident MoT ``transformer`` is bound to BOTH the ``ar_decode`` loop (the reasoner) and the
``diffusion_denoise`` loop (joint generation) via ``shared_weight_components``. A request runs the
reasoner (prompt upsampling), packs its tokens into conditioning, then the joint denoise — all on the
same resident weights, with every AR token and denoise step a runtime-visible WorkUnit. The diffusion
loop is ``WanDenoiseLoop`` (the same step body the engine serves for Wan) bound to the MoT module.
``sound_vae`` is ``optional_for`` non-t2vs tasks so it loads lazily.
"""
...
def build_cosmos3_card(model_id: str = "cosmos3-vfm") -> ModelCard:
    ...
    components = {
        ...
        ComponentSpec(
            ...
            required_for={"reason", "t2v", "t2vs"}),
        "vae":
        ComponentSpec("vae", kind="vae", factory=lambda inst: ToyVAE(), required_for={"t2v", "t2vs"}),
        "sound_vae":
        ComponentSpec("sound_vae",
                      kind="audio_vae",
                      load_id="fastvideo.models.audio:Cosmos3SoundVAE",
                      optional_for={"reason", "t2v"},
                      required_for={"t2vs"}),
    }
    loops = {
        "ar_decode":
