"""GameCraftDenoiseLoop — a flow-match Euler denoise loop with GameCraft i2v + camera conditioning.

GameCraft (HunyuanGameCraft) is an *interactive* world model: a camera/action-conditioned i2v diffusion
DiT. The sampler is plain flow-match (``FlowMatchEulerDiscreteScheduler``, ``flow_shift=5.0``,
``timestep = σ·1000``) with standard ClassicCFG (``guidance_scale=6.0``, ``guidance=None`` to the DiT — NO
embedded guidance). So the *integrator* is identical to ``WanDenoiseLoop`` (``x_next = x + (σ_next−σ)·v``
via ``FLOW_MATCH_STEP``). The deltas this loop carries (faithful to
``fastvideo/pipelines/stages/gamecraft_denoising.py``):
  ...
  1. **33-channel input is assembled by the DiT adapter, not here.** The DiT forward wants
     ...
     pack ``{"gt_latents","mask","camera_states"}`` to the ``GameCraftDiT`` adapter, which concats. The
     toy ``ToyDiT`` accepts (and ignores) the ``cond`` kwarg, so this loop CPU-verifies unchanged.

  2. **Per-step clean-ref injection (frame_replace).** Official GameCraft overwrites the *conditioned*
     latent frames (``mask>0.5``) with the clean reference latent at EVERY step (not just step 0), so the
     conditioned frames stay noise-free throughout denoising. ``next()`` mutates the working latent before
     the forward when a reference latent is present.

  3. **Dual text encoders.** ``encoder_hidden_states = [LLaMA_states(4096d), CLIP_pooled(768d)]``. The
     loop threads the LLaMA states as the usual ``text_embed`` (cond vs uncond) and the CLIP pooled embed
     through the adapter's ``context=`` slot per branch, so ClassicCFG combines both correctly.

  4. **BRINGUP — camera/action conditioning.** GameCraft is fundamentally CameraNet (Plücker)
     conditioned, with the forward special-casing the latent temporal length (18/9/10) for autoregressive
     multi-chunk generation. v2 has no camera/action request field or Plücker-conversion node yet, so the
     registered path passes ``camera_states=None`` -> the DiT's ``if camera_states is not None`` branch is
     skipped (the standard / degenerate denoise). For pure t2v (no reference image) gt_latents+mask are
     zero -> the 33ch concat reduces to ``[noise | 0 | 0]``, exactly the ``GameCraftDenoisingStage``
     fallback. Wiring real camera/action + the 18/9/10 length special-casing needs a request-API
     extension (see ``program.py`` blockers).

All per-request state lives in ``LoopState`` (interleave-safe). ``next`` is kernel-free; ``advance`` folds
the result + (under ROLLOUT) captures a trajectory slice — same shape as ``WanDenoiseLoop``.
"""
...
class GameCraftDenoiseLoop:
    ...
    def init(self, req, model, ctx) -> LoopState:
        ...
        # concat degenerates to [noise | 0 | 0] and the ref-injection branch is inert (the fastvideo
    ...
    def _inject_ref(self, x: np.ndarray, mask, ref) -> np.ndarray:
        ...
        ``t``). Loop latents here are unbatched ``[C,T,h,w]`` (the toy/CPU geometry), so the mask's temporal
        axis is read at ``mask[0,0,:,0,0]`` when 5D (matching the fastvideo ``[B,1,T,H,W]`` mask), or as a
        1D ``[T]`` vector otherwise. ``ref`` is the clean ref latent ``[C,1,h,w]`` (or ``[C,T,h,w]``)."""
        m = np.asarray(mask)
        ...
        # The conditioned-frame replacement requires the ref frame to match the working latent's per-frame
        # geometry (it always does on the real VAE geometry). If a backend's encode produced a mismatched
        # shape (e.g. the crude CPU toy VAE that does not spatially downsample), skip injection rather than
        # crash — the degenerate denoise still proceeds.
    ...
    def next(self, st: LoopState):
        ...
        # assembles the 33ch concat from it. None entries -> zero gt/mask + no camera inside the adapter.
        ...
        def _velocity(model: Any, x_: np.ndarray, sigma_t_: float, pe_: Any, clip_: Any, ne_: Any, clip_n_: Any,
            ...
            # accepts+ignores both kwargs, so this is CPU-toy compatible.
        ...
        return WorkPlan(
            ...
            # i2v ref-injection mutates host state + threads non-workspace conditioning each step -> the
            # static-buffer capture form would miss it, so this step takes the eager path (like cosmos2's
            # gated frame-injection). Pure-t2v could capture, but keeping one path is simpler + correct.
            capturable=False)
