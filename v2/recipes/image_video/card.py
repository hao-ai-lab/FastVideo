"""Two single-purpose generation cards for the T2I->I2V workflow.
...
Both reuse the unchanged ``WanDenoiseLoop``: the I2V image-conditioning lives in the I2V program (it
folds the conditioning image into ``text_embeds``), so no loop change is needed. A new pipeline is
cards + a program + a workflow, never a new loop or runtime.
"""
...
def _diffusion_card(model_id: str, family: str, loop_id: str, caps: tuple[Capability, ...], tasks: set[str], *,
    ...
    components = {
        "text_encoder":
        ComponentSpec("text_encoder", kind="text_encoder", factory=lambda inst: ToyTextEncoder(), required_for=tasks),
        "transformer":
        ComponentSpec("transformer",
                      kind="dit",
                      factory=lambda inst: ToyDiT(seed=seed),
                      resident_for=[loop_id],
                      required_for=tasks),
        "vae":
        ComponentSpec("vae", kind="vae", factory=lambda inst: ToyVAE(), required_for=tasks),
    }
    loops = {
        loop_id:
        LoopSpec(loop_id,
...
def build_flux_t2i_card(model_id: str = "flux-t2i") -> ModelCard:
    ...
    return _diffusion_card(model_id,
                           "flux",
                           "t2i_denoise",
