"""Matrix-Game 2.0 (Base, distilled) — the INTERACTIVE mouse/keyboard world model ported into the v2
substrate as a self-contained recipe package (bucket-C pattern).

The card declares its torch adapters via ``ComponentSpec.adapter`` (``MatrixGame2CausalDiT`` /
``MatrixGame2CLIPImageEncoder`` in ``v2/platform/backends/torch_matrixgame2.py``) and a new
...
``stamp_wan21_checkpoints``. Registered in ``v2/registry.py`` (the orchestrator adds the entry).
