"""Multi-expert ModelCard — N refiner LMs + one generator, for N-way joint RL.

Generalizes the two-expert ``unified`` card: instead of ``llm`` + ``transformer``, this card has
``n_refiners`` prompt-refiner LMs plus a generator — N+1 disjoint experts on N+1 loops
(``ar_decode_0…n-1`` + ``diffusion_denoise``). It shows that joint RL over more than two experts needs
no new card primitive (a card already holds an arbitrary number of components and loops);
``JointMultiExpertRL`` drives the N updates from one reward by looping over an expert list.

The N refiners' refinements compose into the diffusion conditioning (each adds its embedding offset),
so the generator's output depends on every refiner and one reward trains them all.
"""
...
def build_multi_expert_card(model_id: str = "multi-expert-rl", *, n_refiners: int = 2) -> ModelCard:
    ...
    for i, rid in enumerate(rids):  # N refiner experts + N ar loops
        components[rid] = ComponentSpec(
            ...
            resident_for=[f"ar_decode_{rid}"],
            required_for={"t2v"})
        loops[f"ar_decode_{rid}"] = LoopSpec(f"ar_decode_{rid}",
                                             kind=LoopKind.AR_DECODE,
                                             work_unit_kind=WorkUnitKind.AR_TOKEN,
                                             step_cost_model=ar_cost,
                                             shared_weight_components=[rid],
