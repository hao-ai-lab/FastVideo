"""Omni program nodes shared by Cosmos3 and the bagel/lance MoT.

The ``pack`` node is the seam where the AR (understanding) pathway's tokens become the
conditioning the diffusion (generation) pathway consumes. Both pathways are the SAME resident
MoT module, so this is weight-sharing within a request, not a hand-off between two engines.
"""
from __future__ import annotations

from v2.platform.backends.toy import ToyTokenizer
