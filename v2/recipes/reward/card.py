"""Reward-model ModelCard — a served scorer/verifier for RL.

A reward model is just another card: a `scorer` component + a `score` loop emitting `REWARD_BATCH`
work units. A learned reward (PickScore / CLIP / a VLM judge) is loaded, placed, priced, and scheduled
like any model, and can run on its own role-pool (reward disaggregation).
"""
...
def build_reward_card(model_id: str = "pickscore-reward", *, batch: int = 4) -> ModelCard:
    ...
    loops = {
        "score":
        LoopSpec("score",
                 kind=LoopKind.ENCODER,
                 work_unit_kind=WorkUnitKind.REWARD_BATCH,
                 step_cost_model=cost,
                 shared_weight_components=["scorer"],
                 cache_policy=[],
