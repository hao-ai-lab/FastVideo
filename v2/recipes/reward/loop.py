"""RewardLoop — a served reward model scoring samples as ``REWARD_BATCH`` WorkUnits.

The RL reward is computed by a *model* (PickScore/CLIP/a VLM verifier), not a heuristic. This loop reads
a batch of K rollout media from a slot and emits chunked ``REWARD_BATCH`` work units that score them
through the engine, so reward compute is admitted, priced, and interleaved like a denoise step or an AR
token. A GRPO group's K samples score in one (or a few) batched units.
"""
...
class RewardLoop:
    ...
    def next(self, st: LoopState):
        ...
        return WorkPlan(loop_id=self.loop_id,
                        ...
                        kind=WorkUnitKind.REWARD_BATCH,
                        ...
                        resources=ResourceRequest(compute_seconds=self.cost.predict(len(chunk)),
                                                  resident_bytes=64 * max(1, len(chunk)),
                                                  peak_activation_bytes=64),
                        payload={"reward_batch": i},
                        run=run,
                        label=f"reward.batch{i}")
