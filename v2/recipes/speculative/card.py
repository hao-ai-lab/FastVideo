"""Speculative-decoding card — a draft + target AR pair on one instance."""
...
from v2.platform.backends.toy import ToyDraftModel, ToyTargetModel, ToyTokenizer, _seed_from
from v2.recipes.speculative.loop import SpeculativeARLoop


def build_speculative_card(model_id: str = "spec-decode", *, spec_len: int = 4, draft_agree: float = 0.7) -> ModelCard:
    seed = _seed_from(model_id)
    ar_cost = CostModel(kind=WorkUnitKind.AR_TOKEN, base_seconds=5e-5, per_unit_seconds=1e-7)

    def spec_factory():
        return SpeculativeARLoop(loop_id="spec_decode",
                                 draft_id="draft",
                                 target_id="target",
                                 cost=ar_cost,
                                 spec_len=spec_len,
    ...
    components = {
        "tokenizer":
        ComponentSpec("tokenizer", kind="tokenizer", factory=lambda inst: ToyTokenizer(), required_for={"reason"}),
        "draft":
        ComponentSpec("draft",
                      kind="dit",
                      load_id="fastvideo.models.llm:DraftModel",
                      factory=lambda inst: ToyDraftModel(agree=draft_agree),
                      resident_for=["spec_decode"],
                      required_for={"reason"}),
        "target":
        ComponentSpec(
            "target",
            kind="dit",
            load_id="fastvideo.models.llm:TargetModel",
    ...
    return ModelCard(
        ...
        recipe=RecipeSpec(method="base",
                          assumes_loop="spec_decode",
                          assumes_precision="float32",
                          consistency_required=ConsistencyLevel.C1),
        parity=ParitySpec(consistency_levels=[ConsistencyLevel.C0, ConsistencyLevel.C1], interleave_required=True),
        caches={
