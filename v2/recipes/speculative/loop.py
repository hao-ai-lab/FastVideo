"""SpeculativeARLoop — draft-verify (speculative) decoding.

A cheap draft model proposes K tokens, the target model verifies them, and the loop accepts the longest
matching prefix plus one target correction — a variable number of tokens per round. Two properties:

  * Exact. Every accepted token is one the target would have produced greedily, and the correction is the
    target's own token, so the emitted sequence is identical to the target's standalone greedy decode. The
    speedup is free (same output, fewer sequential target steps).
  * Latency win. The target's per-round forwards are one batched verify step (counted as one
    ``verify_round``); standalone decoding would take one sequential target step per token, so
    ``verify_rounds`` is well below the token count.

It binds two components (``draft`` + ``target``) on one resident instance, and every round is an
``AR_TOKEN`` WorkUnit.
"""
    ...
    def next(self, st: LoopState):
        ...
        def run(model, override=None):
            ...
            for j in range(k):
                ...
                accepted.append(int(target.ar_forward(ctx_tokens + cand)))
                tf += 1  # all k accepted + 1
        ...
        return WorkPlan(loop_id=self.loop_id,
                        ...
                        resources=ResourceRequest(compute_seconds=self.cost.predict(len(ctx_tokens) + k),
                                                  resident_bytes=8 * len(ctx_tokens),
                                                  peak_activation_bytes=8 * k),
                        payload={"round": st.scratch["verify_rounds"]},
                        run=run,
                        emits=emits,
        ...
        return LoopResult(outputs={
            ...
            "text": "tok:" + ",".join(map(str, toks))
                          ...
                          metrics={
                              ...
                              "verify_rounds": float(st.scratch["verify_rounds"]),
