"""Tiled-decode card — a diffusion model whose VAE decode is a scheduled ``vae_tile`` loop, so its
tiles co-schedule with denoise steps under the one WorkUnit budget."""
...
def build_tiled_card(model_id: str = "wan-tiled", *, tile_rows: int = 1) -> ModelCard:
    ...
    def tile_factory():
        return VAETileLoop(loop_id="vae_tile",
                           vae_id="vae",
                           cost=tile_cost,
                           tile_rows=tile_rows,
                           latent_slot="denoise_out")
    ...
    components = {
        ...
        ComponentSpec("transformer",
                      ...
                      required_for={"t2v"}),
        "vae":
        ComponentSpec("vae", kind="vae", factory=lambda inst: ToyVAE(), resident_for=["vae_tile"],
                      required_for={"t2v"}),
    }
    loops = {
        "diffusion_denoise":
        ...
        "vae_tile":
        LoopSpec("vae_tile",
                 kind=LoopKind.VAE_TILE,
                 work_unit_kind=WorkUnitKind.VAE_TILE,
                 step_cost_model=tile_cost,
                 shared_weight_components=["vae"],
                 cache_policy=[],
                 loop_factory=tile_factory),
    }
    return ModelCard(
        model_id=model_id,
        family="wan",
