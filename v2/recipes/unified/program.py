"""Unified LM+generator serve program (UniRL/PromptRL inference path):
...
capture (rollout sampler ≠ serve sampler, a controlled divergence). Same loops, two profiles — the
"train ≡ serve through one loop" invariant, now spanning two loop *types*.
"""
...
from v2.recipes.omni import vae_decode_node
from v2.recipes.unified.card import N_REFINE_ACTIONS
...
def build_unified_program() -> Program:
    return Program(
        program_id="unified.unirl",
        kind=ProgramKind.INLINE,
        nodes=[
            ComponentNode("text_encode", fn=text_encode_node_fn, writes=("text_embeds", "neg_text_embeds")),
            ModelLoopNode("refine",
                          loop_id="ar_decode",
                          output_slot="refine_out",
                          reads=("text_embeds", ),
                          writes=("refine_out", )),
            ComponentNode("apply_refine",
                          fn=apply_refinement_node("refine_out"),
                          reads=("refine_out", "text_embeds"),
                          writes=("text_embeds", "refine_action")),
