...
class ProgramRunner:
    ...
    def __init__(self, engine: Engine, program: Program, instance: Any, request: Any):
        ...
        self.ctx = RuntimeLoopContext(instance,
                ...
                if self._resident_res is None:
                    ...
                    if need > 0:
                        if not self.engine.admission.feasible_resident(need):
                            raise AdmissionInfeasible(f"{node.loop_id}: resident {need}B exceeds pool capacity "
        ...
        while not self.done:
            ...
            if not self.done and self._progress == p0:
                ...
                if stuck > 3:  # fail fast, not a 10M busy-spin
                    raise AdmissionInfeasible(f"program {self.program.program_id} made no progress for several ticks "
...
@dataclass
class Engine:
    """A single-pool engine. Pools are single-node; fleet scale is multiple pools."""
    ...
    def run(self, request: Any) -> Output:
        if request.model_id in self._workflows:  # a workflow servable → orchestrate stages
            ...
            if request.model_id in self._wf_running:  # nested workflows: cycle guard
