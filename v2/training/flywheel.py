"""The (recipe, runtime) flywheel — RL then distill into a faster card.

Reward data → RL → a faster distilled card, with the recipe carrying provenance so the chain is
auditable. Wired end-to-end on the shared loops:

  1. RL the base model (DiffusionNFT) → improved policy weights.
  2. Distill from the RL'd model: a DMD2 student with the RL'd model as its teacher learns to match
     it in fewer steps.
  3. The distilled card's ``RecipeSpec.parents`` records the chain ``base → rl → distilled``.

Both stages drive the same ``diffusion_denoise`` loop the engine serves (no second sampler).
"""
...
def run_flywheel(base_card: Any, *, rl_iters: int = 8, distill_iters: int = 15, batch: dict | None = None) -> dict:
    ...
    rl_card = derive_card(base_card, base_card.model_id + "-rl", method="diffusion_nft", parents=[base_card.model_id])
    ...
    distilled_card = derive_card(base_card,
                                 base_card.model_id + "-rl-distilled",
                                 method="dmd2",
                                 parents=[rl_card.model_id])

    return {
        "rl": nft.student,
        "rl_card": rl_card,
        "distilled": dmd.student,
        "distilled_card": distilled_card,
        "teacher": dmd.teacher,
        "base_card": base_card
    }
