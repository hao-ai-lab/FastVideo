"""WorkflowRLMethod — end-to-end RL over a cross-model workflow.

Train BOTH stages of the T2I→I2V workflow (``flux-t2i`` and ``wan-i2v`` — two separate models) from one
final-video reward. The rollout spans two instances: roll out the T2I diffusion loop with SDE capture →
decode an image → condition and roll out the I2V diffusion loop with SDE capture → decode a video → score
it. One group-relative advantage from the final video drives a FlowGRPO PPO update on each stage's
transformer, with two independent WeightSyncPlans on two different instances.

The interesting part is credit assignment: the reward is at the end (the video) yet credits an earlier
model (the T2I generator), so the T2I stage learns to produce images that lead to better videos. This
shows "rollout == serve + capture" holds for a multi-instance workflow with no new primitive — the method
drives the same shared loops the engine serves and threads the artifact between them as the serve-time
Workflow does.
"""
...
from v2.core.loop.sampler import flow_sde_ml_velocity, flow_sde_step_with_logprob
...
def _flowgrpo_ppo_step(dit: Any,
    ...
    for rec in behavior or []:
        ...
        _, logp_cur, mean_cur, eff_std = sde_step(prev, v_cur, st, sn, prev_sample=sample, noise_scale=noise_scale)
        v_ref = ref_dit(prev, emb, st)
        _, _, mean_ref, _ = sde_step(prev, v_ref, st, sn, prev_sample=sample, noise_scale=noise_scale)
...
class WorkflowRLMethod(TrainingMethod):
    ...
    def get_grad_clip_targets(self, iteration: int = 0) -> dict:
        return {
            "t2i.transformer": self.t2i.component("transformer"),
    ...
    def _rollout_i2v(self, prompt: Any, image: Any, seed: Any) -> tuple[Any, Any, Any]:
        ...
        req = make_request(TaskType.I2V,
                           self.i2v.card.model_id,
                           prompt,
                           diffusion=self._sde_params(num_frames=81, seed=seed))
        res = rollout_loop(self.i2v,
                           "i2v_denoise",
                           req,
                           slots={
                               "text_embeds": emb,
    ...
    def managed_train_step(self, batch: dict, iteration: int) -> tuple[dict, dict]:
        ...
        for pi, prompt in enumerate(prompts):
            ...
            adv = np.clip((rewards - rewards.mean()) / (rewards.std() + 1e-4), -self.adv_clip_max,
                          self.adv_clip_max)  # one final-video advantage
            ...
            for k, (res_t, emb_t, res_i, emb_i, _r) in enumerate(samples):
                ...
                pt, _kt, rt, gt = _flowgrpo_ppo_step(self.t2i.component("transformer"),
                                                     self.t2i_ref.component("transformer"),
                                                     res_t.behavior,
                                                     emb_t,
                                                     a,
                                                     lr=self.t2i_lr,
                ...
                pi_, _ki, ri, gi = _flowgrpo_ppo_step(self.i2v.component("transformer"),
                                                      self.i2v_ref.component("transformer"),
                                                      res_i.behavior,
                                                      emb_i,
                                                      a,
                                                      lr=self.i2v_lr,
...
def build_workflow_rl(t2i_card, i2v_card, **kw) -> WorkflowRLMethod:
    ...
    return WorkflowRLMethod(new_instance(t2i_card), new_instance(i2v_card), new_instance(t2i_card),
                            new_instance(i2v_card), **kw)
