...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize Trajectory from Parquet file")
    ...
    pipeline_config.update_config_from_dict({
        ...
        args.vae_precision,
        "vae_config":
        WanVAEConfig(load_encoder=False, load_decoder=True),
    })
    ...
    for i in range(table.num_rows):
        ...
        latents_np = np.copy(
            np.frombuffer(latents_bytes, dtype=dtype).reshape(shape))
        ...
        if args.decode_steps == "last":
            ...
            indices_to_decode = [
                int(x) for x in args.decode_steps.split(",") if x.strip() != ""
            ]
        indices_to_decode = [i for i in indices_to_decode if 0 <= i < steps]
        if not indices_to_decode:
            raise ValueError(
                f"No valid indices selected for decode_steps='{args.decode_steps}' with steps={steps}"
            )

        for step in tqdm(indices_to_decode,
                         desc=f"Decoding {video_id}",
                         leave=False):
